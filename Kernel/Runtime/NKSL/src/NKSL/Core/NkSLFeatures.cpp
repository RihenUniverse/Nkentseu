...
namespace nkentseu {
	...
	// HLSL — Mesh shader
	// =============================================================================
	NkString GenerateMeshShaderHLSL(const NkString &meshEntrySource, const NkSLMeshShaderLayout &layout,
	...
	// =============================================================================
	NkString GenerateMeshShaderMSL(const NkString &meshEntrySource, const NkSLMeshShaderLayout &layout,
								   const NkSLCompileOptions &opts) {
		NkString out;
	...
	// =============================================================================
	NkString GenerateTessControlHLSL(const NkString &source, const NkSLTessLayout &layout,
									 const NkSLCompileOptions &opts) {
		NkString out;
		...
		nkentseu::NkSnprintf(buf, sizeof(buf),
				 ...
				 "[patchconstantfunc(\"%s\")]\n",
				 layout.domain.CStr(), layout.partitioning.CStr(), layout.outputTopology.CStr(),
				 layout.outputControlPoints, layout.patchConstantFunc.CStr());
		out += NkString(buf);
		out += "VS_Output main(\n";
		out += "    InputPatch<VS_Output, " + NkFormat("{0}", layout.outputControlPoints) + "> patch,\n";
	...
	// =============================================================================
	NkString GenerateTessEvalHLSL(const NkString &source, const NkSLTessLayout &layout,
								  const NkSLCompileOptions &opts) {
		NkString out;
	...
	NkString GenerateBindlessMSL(const NkSLBindlessConfig &cfg) {
		...
		nkentseu::NkSnprintf(buf, sizeof(buf),
				 ...
				 "};\n\n",
				 cfg.maxDescriptors, cfg.maxDescriptors, cfg.maxDescriptors, cfg.maxDescriptors);
		out += NkString(buf);
