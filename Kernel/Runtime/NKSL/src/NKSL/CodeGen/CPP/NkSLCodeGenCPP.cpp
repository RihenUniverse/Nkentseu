...
namespace nkentseu {
	...
	NkString NkSLCodeGenCPP::IntrinsicToCPP(const NkString &name) {
		...
		if (name == "inverse")
			return "NkInverse";
		if (name == "determinant")
			return "NkDeterminant";
		// Texture sampling (implémenté via NkSWTexture2D)
		if (name == "texture")
			return "NkSWTexture_Sample";
		if (name == "textureLod")
			return "NkSWTexture_SampleLod";
		// Constructeurs vec
		if (name == "vec2")
			return "NkVec2f";
		if (name == "vec3")
	...
	// =============================================================================
	NkSLCompileResult NkSLCodeGenCPP::Generate(NkSLProgramNode *ast, NkSLStage stage, const NkSLCompileOptions &opts) {
		mOpts = &opts;
		mStage = stage;
		mOutput = "";
		mErrors.Clear();
	...
	void NkSLCodeGenCPP::GenProgram(NkSLProgramNode *prog) {
		// Taille de workgroup compute (depuis l'AST) — informatif côté software.
	...
	void NkSLCodeGenCPP::GenFunction(NkSLFunctionDeclNode *fn, bool asEntry) {
		if (asEntry) {
			...
			NkString stageName = (mStage == NkSLStage::NK_VERTEX)	  ? "vertex"
								 : (mStage == NkSLStage::NK_FRAGMENT) ? "fragment"
																	  : "compute";
			EmitLine("// Entry point: " + stageName);
			if (mStage == NkSLStage::NK_COMPUTE)
				EmitLine("// (compute: la taille de workgroup local_size est gérée "
						 "par le dispatcher software, pas dans la signature)");
	...
	NkString NkSLCodeGenCPP::GenExpr(NkSLNode *node) {
		...
		switch (node->kind) {
			...
			case NkSLNodeKind::NK_EXPR_IDENT: {
				...
				if (id->name == "gl_Position")
					return "output.position";
				if (id->name == "gl_FragCoord")
					return "input.fragCoord";
				if (id->name == "gl_FragDepth")
					return "output.depth";
				if (id->name == "gl_VertexID")
					return "vertexIndex";
				return id->name;
			}
	...
	NkString NkSLCodeGenCPP::GenCall(NkSLCallNode *call) {
		...
		if (name == "NkSWTexture_Sample" && call->args.Size() >= 2) {
			return "NkSWTexture_Sample(" + GenExpr(call->args[0]) + ", " + GenExpr(call->args[1]) + ")";
		}
