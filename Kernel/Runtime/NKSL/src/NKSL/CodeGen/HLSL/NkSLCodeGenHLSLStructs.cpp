...
namespace nkentseu {
	...
	static const HLSLSemanticRule kSemanticRules[] = {{"position", false, "POSITION", "SV_Position", "SV_Position"},
	...
	// =============================================================================
	NkString GetHLSLSemantic(NkSLVarDeclNode *v, NkSLStage stage, bool isVertexInput, bool isFragmentOut,
	...
	void NkSLCodeGenHLSL::GenInputOutputStructs() {
		mInputStructName = ""; // réinitialisé par programme (sera fixé ci-dessous)
		// ── Struct d'entrée du vertex ─────────────────────────────────────────────
		// TOUJOURS émise (même sans attributs) : un VS fullscreen (skybox / post-process
		// via gl_VertexID) n'a pas d'attribut mais référence input._VertexID. Sans struct
		// ni paramètre `input`, fxc émet `X3004 undeclared identifier 'input'`.
		if (mStage == NkSLStage::NK_VERTEX) {
			EmitLine("struct VS_Input");
			EmitLine("{");
			IndentPush();
			// Sémantiques d'ENTRÉE VS = celles de l'input layout du renderer (NkRender3D) :
			//   POSITION0 · NORMAL0 · TANGENT0 · TEXCOORD0(uv) · TEXCOORD1(uv2) · COLOR0.
			// Les noms du shader sont préfixés "a" (aPos/aNormal/aUV/aColor) → on classe par
			// sous-chaîne, et les UV reçoivent un index TEXCOORD courant (PAS la @location,
			// sinon aUV→TEXCOORD3 ≠ layout TEXCOORD0 → vColor/vUV garbage → matériaux noirs).
			int texIdx = 0;
		...
		// TOUJOURS émise (même sans varyings) : un FS fullscreen peut référencer
		// gl_FragCoord (→ input._Position). _Position : SV_Position est toujours valide.
	...
	// =============================================================================
	NkString NkSLCodeGenHLSL::SemanticFor(NkSLVarDeclNode *v, NkSLStage stage, bool isInput, bool isFragOut,
										  int autoIndex) {
		(void)isFragOut;
		if (v->binding.HasLocation()) {
			char buf[32];
			NkString name = v->name.ToLower();
			if (name == "position" || name == "pos" || name.StartsWith("apos") || name.StartsWith("vpos")) {
				if (!isInput)
					return "SV_Position";
				return "POSITION";
			}
		...
		if (n == "normal" || n == "anormal" || n == "vnormal")
			return "NORMAL";
		if (n == "tangent" || n == "atangent")
			return "TANGENT";
		if (n == "bitangent" || n == "abitangent")
			return "BINORMAL";
		if (n == "color" || n == "acolor" || n == "vcolor" || n.StartsWith("col"))
			return "COLOR";
		if (n == "uv" || n == "texcoord" || n == "vuv" || n.StartsWith("uv"))
			return "TEXCOORD0";
		if (n == "uv1" || n == "texcoord1")
			return "TEXCOORD1";
		if (n == "uv2" || n == "texcoord2")
			return "TEXCOORD2";
	...
	NkString NkSLCodeGen_MSL::BuildEntryPointSignature(NkSLFunctionDeclNode *fn) {
		...
		if (mStage == NkSLStage::NK_VERTEX)
			...
			qualifier = "fragment ";
		else if (mStage == NkSLStage::NK_COMPUTE)
			qualifier = "kernel ";
