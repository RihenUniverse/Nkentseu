...
namespace nkentseu {
	...
	// =============================================================================
	NkString NkSLCodeGenGLSLVulkan::BuiltinToVkGLSL(const NkString &name, NkSLStage stage) {
	...
	// =============================================================================
	NkString NkSLCodeGenGLSLVulkan::LayoutQualifier(const NkSLBinding &b, bool isBlock, bool isStorage,
													bool isPushConstant, const char *extra) {
		if (isPushConstant) {
	...
	// Generate
	// =============================================================================
	NkSLCompileResult NkSLCodeGenGLSLVulkan::Generate(NkSLProgramNode *ast, NkSLStage stage,
	...
	void NkSLCodeGenGLSLVulkan::GenVarDecl(NkSLVarDeclNode *v, bool isGlobal) {
		...
		// Type SANS dimension d'array + array postfixée C-style `type nom[N]` (cohérent
		// avec le générateur GL ; évite que TypeToString perde l'array des structs).
		NkString typeStr =
	...
	void NkSLCodeGenGLSLVulkan::GenBlock(NkSLBlockDeclNode *b) {
		...
		if (isPush) {
			...
			NkString layoutStr = LayoutQualifier(b->binding, true, isStorage, false, isStorage ? "std430" : "std140");
			Emit(layoutStr);
		}
