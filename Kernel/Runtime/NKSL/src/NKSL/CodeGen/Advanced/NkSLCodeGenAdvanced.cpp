...
namespace nkentseu {
	...
	// Texture calls HLSL
	// =============================================================================
	static NkString TranslateTextureCallHLSL(const NkString &funcName, const NkVector<NkString> &args) {
		if (args.Empty())
			return funcName + "()";
		...
		if (funcName == "textureLodOffset" && args.Size() >= 4)
			return tex + ".SampleLevel(" + smp + ", " + args[1] + ", " + args[2] + ", " + args[3] + ")";
		...
		if (funcName == "textureGrad" && args.Size() >= 4)
			return tex + ".SampleGrad(" + smp + ", " + args[1] + ", " + args[2] + ", " + args[3] + ")";
		...
		if (funcName == "imageAtomicCompSwap" && args.Size() >= 4)
			return "InterlockedCompareExchange(" + args[0] + "[" + args[1] + "], " + args[2] + ", " + args[3] + ")";
	...
	NkString NkSLCodeGenHLSL::GenCall(NkSLCallNode *call) {
		...
		for (auto *a : call->args)
			strArgs.PushBack(GenExpr(a));

		if (name == "texture" || name == "textureLod" || name == "textureLodOffset" || name == "textureOffset" ||
			name == "texelFetch" || name == "textureGather" || name == "textureGrad" || name == "textureSize" ||
			name == "imageLoad" || name == "imageStore" || name.StartsWith("imageAtomic")) {
	...
	// Texture calls MSL
	// =============================================================================
	static NkString TranslateTextureCallMSL(const NkString &funcName, const NkVector<NkString> &args) {
		if (args.Empty())
			return funcName + "()";
		NkString tex = args[0] + "_tex";
		NkString smp = args[0] + "_smp";
		...
		if (funcName == "textureGather" && args.Size() >= 2)
			return tex + ".gather(" + smp + ", " + args[1] + ")";
		if (funcName == "textureGrad" && args.Size() >= 4)
			return tex + ".sample(" + smp + ", " + args[1] + ", gradient2d(" + args[2] + ", " + args[3] + "))";
		if (funcName == "textureSize")
		...
		if (funcName == "imageAtomicMin" && args.Size() >= 3)
			return "atomic_fetch_min_explicit((volatile device atomic_int*)&" + args[0] + "[" + args[1] + "], " +
				   args[2] + ", memory_order_relaxed)";
		if (funcName == "imageAtomicMax" && args.Size() >= 3)
			return "atomic_fetch_max_explicit((volatile device atomic_int*)&" + args[0] + "[" + args[1] + "], " +
				   args[2] + ", memory_order_relaxed)";
		if (funcName == "imageAtomicExchange" && args.Size() >= 3)
			return "atomic_exchange_explicit((volatile device atomic_int*)&" + args[0] + "[" + args[1] + "], " +
				   args[2] + ", memory_order_relaxed)";
	...
	void NkSLCodeGenHLSL::GenGeometryShaderPrefix(NkSLStage stage) {
		if (stage != NkSLStage::NK_GEOMETRY)
			return;
	...
	void NkSLCodeGenHLSL::GenTessellationShaders(NkSLProgramNode *prog) {
		if (mStage == NkSLStage::NK_TESS_CONTROL) {
			...
			EmitNewLine();
			EmitLine(
				"HS_CONSTANT_DATA PatchConstantFunc(InputPatch<VS_OUT, 4> patch, uint patchID : SV_PrimitiveID) {");
			IndentPush();
		...
		switch (target) {
			...
			case NkSLTarget::NK_MSL_SPIRV_CROSS: {
				if (opts.preferSpirvCrossForMSL) {
					...
					if (!res.success) {
						NkSLCodeGen_MSL ng;
						res = ng.Generate(ast, stage, opts);
					}
				} else {
					NkSLCodeGen_MSL gen;
					res = gen.Generate(ast, stage, opts);
