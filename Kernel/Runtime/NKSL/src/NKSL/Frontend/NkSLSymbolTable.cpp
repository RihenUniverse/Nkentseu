...
// NkSLSymbolTable.cpp
// =============================================================================
#include "NKSL/Frontend/NkSLSymbolTable.h"
...
namespace nkentseu {
	...
	NkSLBaseType NkSLResolvedType::ScalarBase() const {
		if (base >= NkSLBaseType::NK_IVEC2 && base <= NkSLBaseType::NK_IVEC4)
			return NkSLBaseType::NK_INT;
		if (base >= NkSLBaseType::NK_UVEC2 && base <= NkSLBaseType::NK_UVEC4)
			return NkSLBaseType::NK_UINT;
		if (base >= NkSLBaseType::NK_VEC2 && base <= NkSLBaseType::NK_VEC4)
			return NkSLBaseType::NK_FLOAT;
		if (base >= NkSLBaseType::NK_DVEC2 && base <= NkSLBaseType::NK_DVEC4)
			return NkSLBaseType::NK_DOUBLE;
		if (base >= NkSLBaseType::NK_MAT2 && base <= NkSLBaseType::NK_MAT4)
			return NkSLBaseType::NK_FLOAT;
		if (base >= NkSLBaseType::NK_DMAT2 && base <= NkSLBaseType::NK_DMAT4)
			return NkSLBaseType::NK_DOUBLE;
		return base;
	}
	...
	bool NkSLScope::Define(const NkSLSymbol &sym) {
		if (symbols.Find(sym.name))
			return false; // déjà défini dans ce scope
	...
	void NkSLSymbolTable::PushScope(bool isFunction, bool isLoop, NkSLResolvedType returnType) {
	...
	const NkSLFunctionOverload *NkSLSymbolTable::ResolveFunction(const NkString &name,
																 const NkVector<NkSLResolvedType> &argTypes) {
		NkSLSymbol *sym = Resolve(name);
		if (!sym || sym->kind != NkSLSymbolKind::NK_FUNCTION && sym->kind != NkSLSymbolKind::NK_BUILTIN_FUNC)
			return nullptr;
		...
		for (auto &overload : sym->overloads) {
			if (overload.paramTypes.Size() != argTypes.Size())
				continue;
			uint32 score = 0;
			bool valid = true;
	...
	// =============================================================================
	bool NkSLSymbolTable::IsAssignable(const NkSLResolvedType &from, const NkSLResolvedType &to) {
		if (from == to)
			return true;
		return IsImplicitlyConvertible(from, to);
	}

	bool NkSLSymbolTable::IsImplicitlyConvertible(const NkSLResolvedType &from, const NkSLResolvedType &to) {
		// int → float, int → uint (avec warning), float → double
		if (from.base == NkSLBaseType::NK_INT && to.base == NkSLBaseType::NK_FLOAT)
			return true;
		if (from.base == NkSLBaseType::NK_INT && to.base == NkSLBaseType::NK_UINT)
			return true;
		if (from.base == NkSLBaseType::NK_FLOAT && to.base == NkSLBaseType::NK_DOUBLE)
			return true;
		if (from.base == NkSLBaseType::NK_INT && to.base == NkSLBaseType::NK_DOUBLE)
			return true;
		// ivec → vec (même dimension)
		if (from.base == NkSLBaseType::NK_IVEC2 && to.base == NkSLBaseType::NK_VEC2)
			return true;
		if (from.base == NkSLBaseType::NK_IVEC3 && to.base == NkSLBaseType::NK_VEC3)
			return true;
		if (from.base == NkSLBaseType::NK_IVEC4 && to.base == NkSLBaseType::NK_VEC4)
			return true;
		return false;
	}
	...
	NkSLResolvedType NkSLSymbolTable::BinaryResultType(const NkString &op, const NkSLResolvedType &lhs,
		...
		if (rhs.IsVector() && lhs.IsScalar())
			return rhs;
		if (lhs.IsVector() && rhs.IsScalar())
			return lhs;

		// scalar op matrix → matrix
		if (rhs.IsMatrix() && lhs.IsScalar())
			return rhs;
		...
		// int op float → float
		if (IsImplicitlyConvertible(lhs, rhs))
			return rhs;
		if (IsImplicitlyConvertible(rhs, lhs))
			return lhs;
	...
	void NkSLSymbolTable::RegisterBuiltinFunctions() {
		...
		auto defFunc = [&](const NkString &name, std::initializer_list<NkSLBaseType> params, NkSLBaseType ret) {
		...
		defFunc("textureLod", {NkSLBaseType::NK_SAMPLER2D, NkSLBaseType::NK_VEC2, NkSLBaseType::NK_FLOAT},
				NkSLBaseType::NK_VEC4);
		defFunc("textureLod", {NkSLBaseType::NK_SAMPLER3D, NkSLBaseType::NK_VEC3, NkSLBaseType::NK_FLOAT},
				NkSLBaseType::NK_VEC4);
		defFunc("textureOffset", {NkSLBaseType::NK_SAMPLER2D, NkSLBaseType::NK_VEC2, NkSLBaseType::NK_IVEC2},
				NkSLBaseType::NK_VEC4);
		defFunc("textureGrad",
				{NkSLBaseType::NK_SAMPLER2D, NkSLBaseType::NK_VEC2, NkSLBaseType::NK_VEC2, NkSLBaseType::NK_VEC2},
				NkSLBaseType::NK_VEC4);
		defFunc("textureGather", {NkSLBaseType::NK_SAMPLER2D, NkSLBaseType::NK_VEC2}, NkSLBaseType::NK_VEC4);
		defFunc("textureGather", {NkSLBaseType::NK_SAMPLER2D, NkSLBaseType::NK_VEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_VEC4);
		...
		defFunc("texelFetch", {NkSLBaseType::NK_SAMPLER2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_VEC4);
		...
		defFunc("textureLod", {NkSLBaseType::NK_SAMPLER1D, NkSLBaseType::NK_FLOAT, NkSLBaseType::NK_FLOAT},
				NkSLBaseType::NK_VEC4);
		defFunc("textureLod", {NkSLBaseType::NK_SAMPLER_CUBE_ARRAY, NkSLBaseType::NK_VEC4, NkSLBaseType::NK_FLOAT},
				NkSLBaseType::NK_VEC4);
		...
		defFunc("texelFetch", {NkSLBaseType::NK_SAMPLER2DMS, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_VEC4);

		// ── Image load/store ───────────────────────────────────────────────────────
		defFunc("imageLoad", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2}, NkSLBaseType::NK_VEC4);
		defFunc("imageStore", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_VEC4},
				NkSLBaseType::NK_VOID);
		defFunc("imageAtomicAdd", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("imageAtomicMin", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("imageAtomicMax", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("imageAtomicAnd", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("imageAtomicOr", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("imageAtomicXor", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("imageAtomicExchange", {NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("imageAtomicCompSwap",
				{NkSLBaseType::NK_IMAGE2D, NkSLBaseType::NK_IVEC2, NkSLBaseType::NK_INT, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		...
		defFunc("imageStore", {NkSLBaseType::NK_IMAGE3D, NkSLBaseType::NK_IVEC3, NkSLBaseType::NK_VEC4},
				NkSLBaseType::NK_VOID);
		...
		defFunc("imageStore", {NkSLBaseType::NK_IIMAGE3D, NkSLBaseType::NK_IVEC3, NkSLBaseType::NK_IVEC4},
				NkSLBaseType::NK_VOID);
		defFunc("imageLoad", {NkSLBaseType::NK_UIMAGE3D, NkSLBaseType::NK_IVEC3}, NkSLBaseType::NK_UVEC4);
		defFunc("imageStore", {NkSLBaseType::NK_UIMAGE3D, NkSLBaseType::NK_IVEC3, NkSLBaseType::NK_UVEC4},
				NkSLBaseType::NK_VOID);
		defFunc("imageLoad", {NkSLBaseType::NK_IMAGE2D_ARRAY, NkSLBaseType::NK_IVEC3}, NkSLBaseType::NK_VEC4);
		defFunc("imageStore", {NkSLBaseType::NK_IMAGE2D_ARRAY, NkSLBaseType::NK_IVEC3, NkSLBaseType::NK_VEC4},
				NkSLBaseType::NK_VOID);
		...
		defFunc("imageStore", {NkSLBaseType::NK_IMAGE1D, NkSLBaseType::NK_INT, NkSLBaseType::NK_VEC4},
				NkSLBaseType::NK_VOID);
		...
		defFunc("imageStore", {NkSLBaseType::NK_IMAGE_CUBE, NkSLBaseType::NK_IVEC3, NkSLBaseType::NK_VEC4},
				NkSLBaseType::NK_VOID);
		defFunc("imageSize", {NkSLBaseType::NK_IMAGE_CUBE}, NkSLBaseType::NK_IVEC2);

		// ── Constructeurs vecteurs / matrices ──────────────────────────────────────
		for (auto t : {NkSLBaseType::NK_VEC2, NkSLBaseType::NK_VEC3, NkSLBaseType::NK_VEC4, NkSLBaseType::NK_IVEC2,
					   NkSLBaseType::NK_IVEC3, NkSLBaseType::NK_IVEC4, NkSLBaseType::NK_UVEC2, NkSLBaseType::NK_UVEC3,
					   NkSLBaseType::NK_UVEC4, NkSLBaseType::NK_MAT2, NkSLBaseType::NK_MAT3, NkSLBaseType::NK_MAT4}) {
			NkSLSymbol *sym = mCurrent->symbols.Find(BaseTypeNameGLSL(t));
			if (!sym) {
				NkSLSymbol s;
				s.name = BaseTypeNameGLSL(t);
		...
		defFunc("bitfieldExtract", {NkSLBaseType::NK_INT, NkSLBaseType::NK_INT, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
		defFunc("bitfieldInsert",
				{NkSLBaseType::NK_INT, NkSLBaseType::NK_INT, NkSLBaseType::NK_INT, NkSLBaseType::NK_INT},
				NkSLBaseType::NK_INT);
