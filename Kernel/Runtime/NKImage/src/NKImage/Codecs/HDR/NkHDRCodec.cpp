...
namespace nkentseu {
	...
	static NKIMG_INLINE void RGBE2F(uint8 r, uint8 g, uint8 b, uint8 e, float32 *out) noexcept {
		if (!e) {
	...
	static bool ParseDimLine(const char *line, int32 &w, int32 &h, bool &flipX, bool &flipY,
	...
	NkImage NkHDRCodec::Decode(const uint8 *data, usize size) noexcept {
		...
		for (int32 iter = 0; iter < 64 && !s.IsEOF(); ++iter) {
			if (!ReadHdrLine(s, line, sizeof(line)) || line[0] == 0)
				break;
		...
		for (int32 y = 0; y < h; ++y) {
			...
			if (img.IsHDR()) {
				...
				for (int32 x = 0; x < w; ++x) {
					...
					float32 fc[3] = {p[0] / 255.f, (ch > 1 ? p[1] : p[0]) / 255.f, (ch > 2 ? p[2] : p[0]) / 255.f};
					F2RGBE(fc, scan + x * 4);
				}
			}
	...
	bool NkHDRCodec::EncodeToMemory(const NkImage &img, uint8 *&out, usize &outSize) noexcept {
		NkImageStream s;
		if (!EncodeToStream(img, s))
			return false;
