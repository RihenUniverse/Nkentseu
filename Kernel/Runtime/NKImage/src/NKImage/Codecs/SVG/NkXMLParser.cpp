...
namespace nkentseu {
	...
	bool NkXMLParser::Match(Ctx &c, const char *str) noexcept {
		...
		if (c.pos + len > c.size)
			return false;
		if (NkCompare(c.data + c.pos, str, len) != 0)
			return false;
	...
	const char *NkXMLParser::ReadName(Ctx &c) noexcept {
		...
		return c.arena->Dup(reinterpret_cast<const char *>(c.data + start), static_cast<int32>(c.pos - start));
	}

	// Résout les entités XML dans une chaîne brute
	const char *NkXMLParser::DecodeEntities(const char *raw, int32 len, NkXMLArena &arena) noexcept {
		if (!raw)
			return nullptr;
		...
		for (int32 i = 0; i < len;) {
			...
			// Entité
			int32 j = i + 1;
			while (j < len && raw[j] != ';' && j - i < 12)
				++j;
			if (j >= len || raw[j] != ';') {
				dst[di++] = raw[i++];
				continue;
			}
			const char *ent = raw + i + 1;
			const int32 elen = j - (i + 1);
			i = j + 1;
			// Entités prédéfinies
			if (elen == 3 && ::strncmp(ent, "amp", 3) == 0) {
				dst[di++] = '&';
			} else if (elen == 2 && ::strncmp(ent, "lt", 2) == 0) {
				dst[di++] = '<';
			} else if (elen == 2 && ::strncmp(ent, "gt", 2) == 0) {
				dst[di++] = '>';
			} else if (elen == 4 && ::strncmp(ent, "quot", 4) == 0) {
				dst[di++] = '"';
			} else if (elen == 4 && ::strncmp(ent, "apos", 4) == 0) {
				dst[di++] = '\'';
			} else if (elen == 5 && ::strncmp(ent, "nbsp", 5) == 0) {
				dst[di++] = (char)0xC2;
				dst[di++] = (char)0xA0;
			} // U+00A0 UTF-8
			else if (ent[0] == '#') {
	...
	const char *NkXMLParser::ReadText(Ctx &c, bool inCDATA) noexcept {
		...
		if (inCDATA) {
			while (c.pos + 2 < c.size) {
				if (c.data[c.pos] == ']' && c.data[c.pos + 1] == ']' && c.data[c.pos + 2] == '>')
					break;
				if (c.data[c.pos] == '\n')
					++c.line;
				++c.pos;
	...
	bool NkXMLParser::ParseText(Ctx &c, NkXMLNode *parent) noexcept {
		...
		for (const char *p = text; *p && allWS; ++p)
			if (*p != ' ' && *p != '\t' && *p != '\n' && *p != '\r')
				allWS = false;
		if (allWS)
			return true; // texte blanc seul → ignore
