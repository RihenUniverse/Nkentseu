...
#define NK_TEST(name)                                                                                                  \
	{                                                                                                                  \
		...
		bool _ok = true;
#define NKENTSEU_ASSERT_TRUE(cond)                                                                                     \
	if (!(cond)) {                                                                                                     \
		printf("  FAIL: %s : " #cond "\n", _testName);                                                                 \
		_ok = false;                                                                                                   \
	}
#define NKENTSEU_ASSERT_NEAR(a, b, eps)                                                                                \
	if (fabsf((a) - (b)) > (eps)) {                                                                                    \
		printf("  FAIL: %s : " #a " ~= " #b " (got %f vs %f)\n", _testName, (float)(a), (float)(b));                   \
		_ok = false;                                                                                                   \
	}
#define NK_END_TEST()                                                                                                  \
	if (_ok) {                                                                                                         \
...
static void TestAudioAnalyzer() {
	...
	FftResult fft = AudioAnalyzer::ComputeFft(sine, 2048);
	float32 bands[10] = {};
	AudioAnalyzer::ComputeBandEnergies(fft.magnitudes.Data(), fft.fftSize, fft.sampleRate, bands, 10);
...
static void TestAudioEngine() {
	...
	AudioHandle h = AudioEngine::Instance().PlayProcedural([&](float32 *buf, int32 frames, int32 ch) {
		for (int32 i = 0; i < frames * ch; ++i)
			buf[i] = 0.0f;
		callbackCount++;
	});
	NKENTSEU_ASSERT_TRUE(h.IsValid());
