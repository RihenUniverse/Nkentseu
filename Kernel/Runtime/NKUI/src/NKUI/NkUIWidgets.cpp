...
namespace nkentseu {
	namespace nkui {
		...
		template <typename T, uint32 N> struct NkWidgetStateStore {
				NkUIID keys[N] = {};
				T values[N] = {};
				bool used[N] = {};
		};
		...
		static int32 NkDecodeUTF8Codepoint(const char *s, uint32 &cp) noexcept {
			...
			if ((c0 & 0xF0u) == 0xE0u && s[1] && s[2]) {
				cp = ((c0 & 0x0Fu) << 12) | ((static_cast<uint8>(s[1]) & 0x3Fu) << 6) |
					 (static_cast<uint8>(s[2]) & 0x3Fu);
				return 3;
			}
			if ((c0 & 0xF8u) == 0xF0u && s[1] && s[2] && s[3]) {
				cp = ((c0 & 0x07u) << 18) | ((static_cast<uint8>(s[1]) & 0x3Fu) << 12) |
					 ((static_cast<uint8>(s[2]) & 0x3Fu) << 6) | (static_cast<uint8>(s[3]) & 0x3Fu);
				return 4;
			}
			cp = '?';
			return 1;
		...
		static float32 NkMeasureWrappedTextHeight(const NkUIFont &font, const char *text, float32 wrapWidth,
		...
		static float32 NkMeasureUTF8Range(const NkUIFont &font, const char *text, int32 startByte, int32 endByte,
		...
		static int32 NkFindCaretFromX(const NkUIFont &font, const char *text, int32 startByte, int32 endByte, float32 x,
									  float32 charSpacing) noexcept {
			if (!text)
				return 0;
			if (x <= 0.f)
		...
		static int32 NkBuildTextLines(const NkUIFont &font, const char *text, bool wrap, float32 wrapWidth,
									  float32 charSpacing, NkTextLineInfo *outLines, int32 maxLines,
									  float32 *outMaxWidth) noexcept {
			if (!outLines || maxLines <= 0)
				return 0;
			if (wrapWidth < 1.f)
				wrapWidth = 1.f;
		...
		static int32 NkFindLineIndexForCaret(const NkTextLineInfo *lines, int32 lineCount, int32 caretByte) noexcept {
			...
			for (int32 i = 0; i < lineCount; ++i) {
				if (caretByte >= lines[i].start && caretByte <= lines[i].end)
					return i;
			}
			return lineCount - 1;
		}

		static NkVec2 NkCaretPixelFromLines(const NkUIFont &font, const char *text, const NkTextLineInfo *lines,
											int32 lineCount, int32 caretByte, float32 lineHeight,
											float32 charSpacing) noexcept {
			if (!lines || lineCount <= 0)
				return {0.f, 0.f};
		...
		static NkUITextInputOptions ResolveTextInputOptions(const NkUITextInputOptions *opts) noexcept {
			...
			if (o.repeatDelay < 0.f)
				o.repeatDelay = 0.f;
			if (o.repeatRate < 0.01f)
				o.repeatRate = 0.01f;
			if (o.multilineLineSpacing < 1.f)
				o.multilineLineSpacing = 1.f;
			if (o.multilineCharSpacing < 0.f)
				o.multilineCharSpacing = 0.f;
			return o;
		}

		static bool ConsumeRepeat(const NkUIContext &ctx, NkKey key, bool allowRepeat, float32 delay, float32 rate,
		...
		NkUIWidgetState NkUI::CalcState(NkUIContext &ctx, NkUIID id, NkRect r, bool enabled) noexcept {
			...
			if (!enabled)
				return NkUIWidgetState::NK_DISABLED;
		...
		void NkUI::DrawWidgetBg(NkUIDrawList &dl, const NkUITheme &t, NkRect r, NkUIWidgetType type,
		...
		void NkUI::DrawLabel(NkUIDrawList &dl, NkUIFont &f, const NkUITheme &t, NkRect r, const char *text,
		...
		void NkUI::EndRow(NkUIContext &ctx, NkUILayoutStack &ls) noexcept {
			...
			NkUILayoutNode *parent = ls.Top();
			if (parent) {
				NkUILayout::AdvanceItem(
					ctx, ls, {node->bounds.x, node->bounds.y, node->bounds.w, h + ctx.theme.metrics.itemSpacing});
			} else {
		...
		bool NkUI::BeginScrollRegion(NkUIContext &ctx, NkUILayoutStack &ls, const char *id, NkRect rect,
		...
		void NkUI::EndScrollRegion(NkUIContext &ctx, NkUILayoutStack &ls) noexcept {
			...
			if (node->scrollY < 0.f)
				node->scrollY = 0.f;
			if (node->scrollY > maxY)
				node->scrollY = maxY;
			if (node->scrollX < 0.f)
				node->scrollX = 0.f;
			if (node->scrollX > maxX)
				node->scrollX = maxX;

			// Barre verticale
			if (node->contentSize.y > node->bounds.h) {
				NkRect track = {node->bounds.x + node->bounds.w - m.scrollbarWidth, node->bounds.y, m.scrollbarWidth,
								node->bounds.h};
				NkUILayout::DrawScrollbar(ctx, *ctx.dl, true, track, node->contentSize.y, node->bounds.h, node->scrollY,
										  NkHashInt(node->id, 1));
				if (node->scrollYPtr)
					*node->scrollYPtr = node->scrollY;
			}
			// Barre horizontale
			if (node->contentSize.x > node->bounds.w) {
				NkRect track = {node->bounds.x, node->bounds.y + node->bounds.h - m.scrollbarWidth, node->bounds.w,
								m.scrollbarWidth};
				NkUILayout::DrawScrollbar(ctx, *ctx.dl, false, track, node->contentSize.x, node->bounds.w,
										  node->scrollX, NkHashInt(node->id, 2));
				if (node->scrollXPtr)
					*node->scrollXPtr = node->scrollX;
			}
			ls.Pop();
		}

		bool NkUI::BeginSplit(NkUIContext &ctx, NkUILayoutStack &ls, const char *id, NkRect rect, bool vertical,
							  float32 *ratio) noexcept {
			NkUILayoutNode *node = ls.Push();
			if (!node)
				return false;
			ResetLayoutNode(*node);
		...
		bool NkUI::BeginSplitPane2(NkUIContext &ctx, NkUILayoutStack &ls) noexcept {
			NkUILayoutNode *node = ls.Top();
			if (!node || node->type != NkUILayoutType::NK_SPLIT)
				return false;
		...
		void NkUI::Text(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *text,
		...
		void NkUI::TextSmall(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font,
							 const char *text) noexcept {
			Text(ctx, ls, dl, font, text, ctx.theme.colors.textSecondary);
		}

		void NkUI::TextColored(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, NkColor col,
							   const char *text) noexcept {
			Text(ctx, ls, dl, font, text, col);
		}

		void NkUI::TextWrapped(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font,
							   const char *text) noexcept {
			NkUILayoutNode *n = ls.Top();
			const float32 x = n ? (n->cursor.x - n->scrollX) : ctx.cursor.x;
			const float32 y = n ? (n->cursor.y - n->scrollY) : ctx.cursor.y;
			...
			font.RenderTextWrapped(dl,
								   {r.x + ctx.GetPaddingX(), r.y + ctx.GetPaddingY(), r.w - ctx.GetPaddingX() * 2, r.h},
		...
		bool NkUI::Button(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
		...
		bool NkUI::ButtonSmall(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font,
		...
		bool NkUI::ButtonImage(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, uint32 texId, NkVec2 size,
							   NkColor tint) noexcept {
		...
		bool NkUI::Checkbox(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
							bool &value) noexcept {
			const NkUILabelParts lp = NkParseLabel(label);
			const NkUIID id = ResolveLabelID(ctx, ls, lp);
			...
			if (!ctx.CallShapeOverride(NkUIWidgetType::NK_CHECKBOX, r,
				...
				dl.AddRect(box,
						   HasState(state, NkUIWidgetState::NK_FOCUSED) ? ctx.theme.colors.borderFocus
				...
				// Libellé
				font.RenderText(dl, {r.x + sz + ctx.GetPaddingX(), TextBaselineY(r, font)}, lp.label,
		...
		bool NkUI::Toggle(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
						  bool &value) noexcept {
			const NkUILabelParts lp = NkParseLabel(label);
			const NkUIID id = ResolveLabelID(ctx, ls, lp);
			...
			dl.AddCircleFilled({thumb.x + thumb.w * 0.5f, thumb.y + thumb.h * 0.5f}, thumb.w * 0.5f, NkColor::White);
			font.RenderText(dl, {r.x + tw2 + ctx.GetPaddingX(), TextBaselineY(r, font)}, lp.label,
							ctx.theme.colors.textPrimary);
			NkUILayout::AdvanceItem(ctx, ls, r);
			return changed;
		}
		...
		bool NkUI::SliderFloat(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font,
							   const char *label, float32 &value, float32 vmin, float32 vmax, const char *fmt,
							   float32 width, const NkUISliderValueOptions *valueOpts) noexcept {
			const NkUILabelParts lp = NkParseLabel(label);
			const NkUIID id = ResolveLabelID(ctx, ls, lp);
			...
			if (opts.placement == NkUISliderValuePlacement::NK_LEFT_OF_SLIDER)
				reservedLeft = valueExtra;
			if (opts.placement == NkUISliderValuePlacement::NK_RIGHT_OF_SLIDER)
				reservedRight = valueExtra;
			...
			if (opts.placement == NkUISliderValuePlacement::NK_ABOVE_THUMB)
				valuePadTop = lineH + opts.gap;
			if (opts.placement == NkUISliderValuePlacement::NK_BELOW_THUMB)
				valuePadBottom = lineH + opts.gap;
			...
			NkRect sliderR = {r.x + reservedLeft, r.y + valuePadTop, r.w - labelW - reservedLeft - reservedRight,
							  sliderH};
			if (sliderR.w < 10.f)
				sliderR.w = 10.f;
			...
			if (!ctx.CallShapeOverride(NkUIWidgetType::NK_SLIDER_FLOAT, r, state, lp.label, t)) {
				...
				if (fill.w > 0)
					dl.AddRectFilled(fill, ctx.theme.colors.sliderThumb, th * 0.5f);
				// Thumb
				const float32 tw2 = ctx.theme.metrics.sliderThumbW;
				const NkVec2 thumbC = {sliderR.x + sliderR.w * t, sliderR.y + sliderR.h * 0.5f};
				const NkColor thumbC2 =
					(HasState(state, NkUIWidgetState::NK_ACTIVE) || HasState(state, NkUIWidgetState::NK_HOVERED))
						? ctx.theme.colors.accentHover
						: ctx.theme.colors.sliderThumb;
		...
		bool NkUI::SliderInt(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
							 int32 &value, int32 vmin, int32 vmax, float32 width,
							 const NkUISliderValueOptions *valueOpts) noexcept {
			float32 fv = static_cast<float32>(value);
			const bool r = SliderFloat(ctx, ls, dl, font, label, fv, static_cast<float32>(vmin),
									   static_cast<float32>(vmax), "%.0f", width, valueOpts);
		...
		bool NkUI::DragFloat(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
							 float32 &value, float32 speed, float32 vmin, float32 vmax) noexcept {
			const NkUILabelParts lp = NkParseLabel(label);
			const NkUIID id = ResolveLabelID(ctx, ls, lp);
		...
		bool NkUI::DragInt(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
						   int32 &value, float32 speed, int32 vmin, int32 vmax) noexcept {
			float32 fv = static_cast<float32>(value);
			const bool r =
				DragFloat(ctx, ls, dl, font, label, fv, speed, static_cast<float32>(vmin), static_cast<float32>(vmax));
			value = static_cast<int32>(fv + 0.5f);
			if (value < vmin)
				value = vmin;
			if (value > vmax)
		...
		bool NkUI::InputText(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
							 char *buf, int32 bufSize, float32 width, const NkUITextInputOptions *optsPtr) noexcept {
			if (!buf || bufSize <= 0)
				return false;
			...
			if (ctx.IsActive(id) && !ctx.input.mouseDown[0])
				ctx.ClearActive();
			...
			if (ctx.IsFocused(id)) {
				if (ctx.input.IsKeyPressed(NkKey::NK_ESCAPE) || ctx.input.IsKeyPressed(NkKey::NK_TAB)) {
					...
					const bool moveLeft =
						ConsumeRepeat(ctx, NkKey::NK_LEFT, true, opts.repeatDelay, opts.repeatRate, rep.left);
					const bool moveRight =
						ConsumeRepeat(ctx, NkKey::NK_RIGHT, true, opts.repeatDelay, opts.repeatRate, rep.right);
					if (moveLeft) {
					...
					const bool eraseBack = ConsumeRepeat(ctx, NkKey::NK_BACK, opts.repeatBackspace, opts.repeatDelay,
														 opts.repeatRate, rep.backspace);
					const bool eraseDel = ConsumeRepeat(ctx, NkKey::NK_DELETE, opts.repeatDelete, opts.repeatDelay,
														opts.repeatRate, rep.del);
					if (eraseBack) {
			...
			if (ctx.IsFocused(id)) {
				...
				if (caretX > rightLimit)
					edit.scrollX = caretX - (visibleW - 2.f);
			}
			if (edit.scrollX < 0.f)
				edit.scrollX = 0.f;
			const float32 maxScrollX = (textW > visibleW) ? (textW - visibleW + 2.f) : 0.f;
			if (edit.scrollX > maxScrollX)
				edit.scrollX = maxScrollX;
			...
			if (hasText) {
				font.RenderText(dl, {textRect.x - edit.scrollX, TextBaselineY(inputR, font)}, displayText, textCol,
								-1.f, false);
			}
			if (ctx.IsFocused(id) && static_cast<int32>(ctx.time * 2.f) % 2 == 0) {
				...
				const float32 descRaw =
					(font.metrics.descender >= 0.f) ? font.metrics.descender : -font.metrics.descender;
				const float32 desc = (descRaw > 0.f) ? descRaw : (font.size * 0.2f);
				const float32 glyphH = asc + desc;
			...
			if (lp.label[0]) {
				font.RenderText(dl, {r.x + inputW + ctx.GetPaddingX() * 0.5f, TextBaselineY(r, font)}, lp.label,
								ctx.theme.colors.textPrimary);
			}
			NkUILayout::AdvanceItem(ctx, ls, r);
			return changed;
		...
		bool NkUI::InputInt(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
							int32 &value, const NkUITextInputOptions *opts) noexcept {
			const NkUILabelParts lp = NkParseLabel(label);
			const NkUIID id = ResolveLabelID(ctx, ls, lp);
			...
			const bool textChanged =
				InputText(ctx, ls, dl, font, label, st.buf, static_cast<int32>(sizeof(st.buf)), 0.f, opts);
			if (ctx.IsFocused(id))
				st.editing = true;
			else
				st.editing = false;
		...
		bool NkUI::InputMultiline(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font,
			...
			int32 lineCount = NkBuildTextLines(font, buf, opts.multilineWrap, inner.w, opts.multilineCharSpacing, lines,
											   kMaxLines, &contentW);
			if (lineCount <= 0) {
				lines[0] = {0, 0, 0.f};
				lineCount = 1;
			...
			if (canInteract && ctx.IsHovered(r) && ctx.ConsumeMouseClick(0)) {
				...
				edit.caretByte = NkFindCaretFromX(font, buf, lines[lineIdx].start, lines[lineIdx].end, localX,
												  opts.multilineCharSpacing);
				edit.preferredX = -1.f;
			}
			if (ctx.IsActive(id) && !ctx.input.mouseDown[0])
				ctx.ClearActive();
			...
			if (ctx.IsFocused(id)) {
				if (ctx.input.IsKeyPressed(NkKey::NK_ESCAPE)) {
					...
					const bool moveRight =
						ConsumeRepeat(ctx, NkKey::NK_RIGHT, true, opts.repeatDelay, opts.repeatRate, rep.right);
					const bool moveUp =
						ConsumeRepeat(ctx, NkKey::NK_UP, true, opts.repeatDelay, opts.repeatRate, rep.up);
					const bool moveDown =
						ConsumeRepeat(ctx, NkKey::NK_DOWN, true, opts.repeatDelay, opts.repeatRate, rep.down);
					...
					if (moveUp || moveDown) {
						const NkVec2 caretPos = NkCaretPixelFromLines(font, buf, lines, lineCount, edit.caretByte,
																	  lineH, opts.multilineCharSpacing);
						...
						edit.caretByte = NkFindCaretFromX(font, buf, lines[targetLine].start, lines[targetLine].end,
														  targetX, opts.multilineCharSpacing);
					}
					...
					if (opts.allowEnterNewLine && !insertedNewLineFromChars) {
						const bool addEnter =
							ConsumeRepeat(ctx, NkKey::NK_ENTER, true, opts.repeatDelay, opts.repeatRate, rep.enter);
						if (addEnter) {
							int32 newCaret = edit.caretByte;
							if (NkInsertCharAt(buf, bufSize, edit.caretByte, '\n', &newCaret)) {
								edit.caretByte = newCaret;
			...
			for (int32 pass = 0; pass < 3; ++pass) {
				lineCount = NkBuildTextLines(font, buf, opts.multilineWrap, viewW, opts.multilineCharSpacing, lines,
											 kMaxLines, &contentW);
				if (lineCount <= 0) {
					lines[0] = {0, 0, 0.f};
					lineCount = 1;
			...
			const NkVec2 caretPos =
				NkCaretPixelFromLines(font, buf, lines, lineCount, edit.caretByte, lineH, opts.multilineCharSpacing);
			if (ctx.IsFocused(id)) {
				if (!opts.multilineWrap) {
					if (caretPos.x < edit.scrollX)
						edit.scrollX = caretPos.x;
					const float32 right = edit.scrollX + viewW - 2.f;
					if (caretPos.x > right)
						edit.scrollX = caretPos.x - (viewW - 2.f);
				} else {
					edit.scrollX = 0.f;
				}

				if (caretPos.y < edit.scrollY)
					edit.scrollY = caretPos.y;
				const float32 caretBottom = caretPos.y + lineH;
				const float32 viewBottom = edit.scrollY + viewH;
				if (caretBottom > viewBottom)
					edit.scrollY = caretBottom - viewH;
			}
			...
			if (edit.scrollY < 0.f)
				edit.scrollY = 0.f;
			if (edit.scrollY > maxScrollY)
				edit.scrollY = maxScrollY;
			...
			for (int32 lineIdx = 0; lineIdx < lineCount; ++lineIdx) {
				...
				if (lineTop > textRect.y + textRect.h)
					break;
				if (lineTop + lineH < textRect.y)
					continue;
			...
			if (showHScroll) {
				...
				NkUILayout::DrawScrollbar(ctx, dl, false, track, contentW, textRect.w, edit.scrollX,
										  NkHashInt(id, 202));
			}
		...
		bool NkUI::Combo(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
						 int32 &selected, const char *const *items, int32 numItems, float32 width) noexcept {
			...
			if (selected >= 0 && selected < numItems) {
				font.RenderText(dl, {comboR.x + ctx.GetPaddingX(), TextBaselineY(comboR, font)}, items[selected],
								ctx.theme.colors.inputText,
								comboR.w - ctx.theme.metrics.comboArrowW - ctx.GetPaddingX(), true);
			}
			// Flèche
			dl.AddArrow({comboR.x + comboR.w - ctx.theme.metrics.comboArrowW * 0.5f, comboR.y + comboR.h * 0.5f}, 8.f,
						1, ctx.theme.colors.textSecondary);
			...
			if (popupOpen) {
				...
				for (int32 i = 0; i < numItems; ++i) {
					const NkRect item = {popup.x + 2, popup.y + 2 + i * ctx.GetItemHeight(), popup.w - 4,
										 ctx.GetItemHeight()};
					const bool hov = popupCanInteract && ctx.IsHovered(item);
					if (hov)
						ctx.dl->AddRectFilled(item, ctx.theme.colors.accent, ctx.theme.metrics.cornerRadiusSm);
					font.RenderText(*ctx.dl, {item.x + ctx.GetPaddingX(), TextBaselineY(item, font)}, items[i],
		...
		bool NkUI::ListBox(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
						   int32 &selected, const char *const *items, int32 numItems, int32 visibleCount,
			...
			const bool multiSelect =
				(opts && opts->multiSelect && opts->selectedMask && opts->selectedMaskCount >= numItems);
			...
			for (int32 i = 0; i < numItems; ++i) {
				...
				if (item.y + item.h < r.y || item.y > r.y + r.h)
					continue;
				...
				font.RenderText(dl, {item.x + ctx.GetPaddingX(), TextBaselineY(item, font)}, items[i],
								(sel || hov) ? ctx.theme.colors.textOnAccent : ctx.theme.colors.textPrimary);
			...
			if (contentH > r.h) {
				const NkRect track = {r.x + r.w - ctx.theme.metrics.scrollbarWidth, r.y,
									  ctx.theme.metrics.scrollbarWidth, r.h};
		...
		void NkUI::ProgressBar(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, float32 fraction, NkVec2 size,
							   const char *overlay) noexcept {
			const float32 w = size.x > 0 ? size.x : 200.f;
			...
			if (overlay && *overlay) {
				...
				if (!drawFont) {
					...
					if (fallbackFont.size < 6.f)
						fallbackFont.size = 6.f;
		...
		void NkUI::ProgressBarVertical(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, float32 fraction,
									   NkVec2 size, const char *overlay) noexcept {
		...
		bool NkUI::ColorButton(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font,
			...
			dl.AddRect(box,
					   HasState(state, NkUIWidgetState::NK_HOVERED) ? ctx.theme.colors.borderHover
																	: ctx.theme.colors.border,
					   1.f, ctx.theme.metrics.cornerRadiusSm);
			if (lp.label[0]) {
				font.RenderText(dl, {box.x + size + ctx.GetPaddingX(), TextBaselineY(r, font)}, lp.label,
								ctx.theme.colors.textPrimary);
			}
			NkUILayout::AdvanceItem(ctx, ls, r);
		...
		static float32 TreeBaseY(const NkRect &r, const NkUIFont &f) noexcept {
			...
			const float32 lh =
				f.metrics.lineHeight > 0.f ? f.metrics.lineHeight : asc + (desc > 0.f ? desc : f.size * 0.2f);
			return r.y + (r.h - lh) * .5f + asc;
		}

		bool NkUI::TreeNode(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font, const char *label,
			...
			NkUILayoutNode *n = ls.Top();
			float32 availW = n ? (n->bounds.x + n->bounds.w - (n->cursor.x - n->scrollX))
			...
			const NkColor lblCol = HasState(state, NkUIWidgetState::NK_DISABLED) ? ctx.theme.colors.textDisabled
																				 : ctx.theme.colors.textPrimary;
			const float32 textX = r.x + ctx.theme.metrics.treeIndent * 0.35f + 14.f;
			font.RenderText(dl, {textX, TreeBaseY(r, font)}, lp.label, lblCol, r.w - (textX - r.x) - 4.f, true);
		...
		bool NkUI::BeginTable(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &, const char *id,
							  int32 columns, float32 width) noexcept {
			if (sTableDepth >= 64)
				return false;
		...
		void NkUI::TableHeader(NkUIContext &ctx, NkUILayoutStack &ls, NkUIDrawList &dl, NkUIFont &font,
							   const char *label) noexcept {
			NkRect r = NkUILayout::NextItemRect(ctx, ls, 0, ctx.GetItemHeight());
			font.RenderText(dl, {r.x + ctx.GetPaddingX(), TextBaselineY(r, font)}, label,
							ctx.theme.colors.textOnAccent);
			NkUILayout::AdvanceItem(ctx, ls, r);
		}
		...
		void NkUI::Tooltip(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const char *text) noexcept {
			...
			if (sTooltipHoverTime < ctx.theme.metrics.tooltipDelay)
				return;
			...
			const float32 lh =
				font.metrics.lineHeight > 0.f ? font.metrics.lineHeight : asc + (desc > 0.f ? desc : font.size * 0.2f);
			...
			if (tx + bw > ctx.viewW)
				tx = ctx.input.mousePos.x - bw - 6.f;
			if (ty + th > ctx.viewH)
				ty = ctx.input.mousePos.y - th - 4.f;
			if (tx < 2.f)
				tx = 2.f;
			if (ty < 2.f)
				ty = 2.f;
