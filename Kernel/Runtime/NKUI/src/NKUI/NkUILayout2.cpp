...
namespace nkentseu {
	namespace nkui {
		...
		bool NkUILayoutSerializer::Save(const NkUIWindowManager &wm, const NkUIDockManager &dm,
										const char *path) noexcept {
			char *json = nullptr;
			usize len = 0;
			if (!SaveToMemory(wm, dm, json, len) || !json)
				return false;
		...
		bool NkUILayoutSerializer::SaveToMemory(const NkUIWindowManager &wm, const NkUIDockManager &dm, char *&outJson,
			...
			buf[pos] = 0;
			outJson = buf;
			outLen = static_cast<usize>(pos);
			return true;
		}

		bool NkUILayoutSerializer::Load(NkUIWindowManager &wm, NkUIDockManager &dm, const char *path) noexcept {
		...
		bool NkUILayoutSerializer::LoadFromMemory(NkUIWindowManager &wm, NkUIDockManager &dm, const char *json,
												  usize /*len*/) noexcept {
			if (!json)
				return false;
			...
			while (p && *p) {
				...
				findFloat(objBuf, "x", x);
				findFloat(objBuf, "y", y);
				findFloat(objBuf, "w", w);
				findFloat(objBuf, "h", h);
				findFloat(objBuf, "scrollY", sy);
				findBool(objBuf, "open", open);
				findBool(objBuf, "collapsed", col);
				findBool(objBuf, "maximized", max);
		...
		NkUIColorPickerFull::State *NkUIColorPickerFull::FindOrCreate(NkUIID id) noexcept {
			for (int32 i = 0; i < sNumStates; ++i)
				if (sStates[i].id == id)
					return &sStates[i];
			if (sNumStates >= 16)
				return &sStates[0];
			State &s = sStates[sNumStates++];
			s.id = id;
			s.h = 0;
			s.s = 1;
			s.v = 1;
			s.a = 1;
			::strcpy(s.hexBuf, "#FFFFFFFF");
			return &s;
		}

		void NkUIColorPickerFull::RGBtoHSV(uint8 r, uint8 g, uint8 b, float32 &h, float32 &s, float32 &v) noexcept {
			const float32 rf = r / 255.f, gf = g / 255.f, bf = b / 255.f;
			const float32 mx = rf > gf ? rf : gf;
			const float32 mx2 = mx > bf ? mx : bf;
			const float32 mn = rf < gf ? rf : gf;
			const float32 mn2 = mn < bf ? mn : bf;
			v = mx2;
		...
		void NkUIColorPickerFull::HSVtoRGB(float32 h, float32 s, float32 v, uint8 &r, uint8 &g, uint8 &b) noexcept {
			if (s <= 0) {
				r = g = b = static_cast<uint8>(v * 255);
				return;
			}
			h *= 6;
			const int32 i = static_cast<int32>(h);
			const float32 f = h - i;
			...
			r = static_cast<uint8>(rf * 255);
			g = static_cast<uint8>(gf * 255);
			b = static_cast<uint8>(bf * 255);
		}

		void NkUIColorPickerFull::DrawSVSquare(NkUIContext &ctx, NkUIDrawList &dl, NkRect r, float32 h, float32 &s,
											   float32 &v) noexcept {
			...
			if (ctx.IsHovered(r) && ctx.ConsumeMouseClick(0))
				ctx.SetActive(id);
			if (ctx.IsActive(id)) {
		...
		void NkUIColorPickerFull::DrawHueBar(NkUIContext &ctx, NkUIDrawList &dl, NkRect r, float32 &h) noexcept {
			const int32 steps = 18;
			for (int32 i = 0; i < steps; ++i) {
				const float32 hf = static_cast<float32>(i) / steps;
				uint8 cr, cg, cb;
				HSVtoRGB(hf, 1, 1, cr, cg, cb);
		...
		void NkUIColorPickerFull::DrawAlphaBar(NkUIContext &ctx, NkUIDrawList &dl, NkRect r, float32 h, float32 s,
											   float32 v, float32 &a) noexcept {
			// Damier
			const int32 cs = 8;
			for (int32 y = 0; y < static_cast<int32>(r.h / cs) + 1; ++y)
				for (int32 x = 0; x < static_cast<int32>(r.w / cs) + 1; ++x) {
					...
					dl.AddRectFilled({r.x + x * cs, r.y + y * cs, static_cast<float32>(cs), static_cast<float32>(cs)},
									 c);
				}
		...
		void NkUIColorPickerFull::DrawPreview(NkUIDrawList &dl, NkRect r, NkColor oldC, NkColor newC) noexcept {
		...
		bool NkUIColorPickerFull::Draw(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUILayoutStack &ls,
			...
			uint8 cr, cg, cb;
			HSVtoRGB(st->h, st->s, st->v, cr, cg, cb);
			...
			const bool changed =
				(newColor.r != color.r || newColor.g != color.g || newColor.b != color.b || newColor.a != color.a);
			if (changed)
				color = newColor;
			return changed;
		}
		...
		bool NkUIFontNKFont::Load(const uint8 *, usize, float32, NkUIFontAtlas &, NkUIFont &) noexcept {
			return false;
		}

		bool NkUIFontNKFont::LoadFile(const char *, float32, NkUIFontAtlas &, NkUIFont &) noexcept {
			return false;
		}

		bool NkUIFontNKFont::AddGlyphRange(NkUIFont &, NkUIFontAtlas &, uint32, uint32) noexcept {
			return false;
		}
		...
		bool NkUIOpenGLRenderer::Init(int32 w, int32 h) noexcept {
			...
			glVertexAttribPointer(2, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(NkUIVertex),
								  (void *)offsetof(NkUIVertex, col));
			glBindVertexArray(0);
		...
		void NkUIOpenGLRenderer::SubmitDrawList(const NkUIDrawList &dl, int32 fbW, int32 fbH) noexcept {
			if (!dl.vtx || !dl.idx || dl.cmdCount == 0)
				return;
			glBindVertexArray(mVAO);
			glBindBuffer(GL_ARRAY_BUFFER, mVBO);
			...
			glUniform2f(glGetUniformLocation(mShader, "uViewport"), static_cast<float32>(fbW),
						static_cast<float32>(fbH));
			...
			for (uint32 ci = 0; ci < dl.cmdCount; ++ci) {
				...
				if (cmd.type == NkUIDrawCmdType::NK_CLIP_RECT) {
					...
					glScissor(static_cast<int32>(cr.x), fbH - static_cast<int32>(cr.y + cr.h), static_cast<int32>(cr.w),
		...
		void NkUIOpenGLRenderer::FreeTexture(uint32 id) noexcept {
			glDeleteTextures(1, &id);
		}

#else
		// Stubs quand OpenGL n'est pas disponible
		bool NkUIOpenGLRenderer::Init(int32, int32) noexcept {
			return false;
		}
		...
		uint32 NkUIOpenGLRenderer::UploadTexture(const uint8 *, int32, int32, int32) noexcept {
			return 0;
		}
		...
		uint32 NkUIOpenGLRenderer::CompileShader(uint32, const char *) noexcept {
			return 0;
		}

		uint32 NkUIOpenGLRenderer::LinkProgram(uint32, uint32) noexcept {
			return 0;
		}
#endif
	} // namespace nkui
