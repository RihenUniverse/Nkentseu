...
namespace nkentseu {
	namespace nkui {
		...
		namespace {
			...
			static bool PointInQuad(NkVec2 p, NkVec2 a, NkVec2 b, NkVec2 c, NkVec2 d) noexcept {
			...
			static void DrawAxisLabel(NkUIDrawList &dl, NkVec2 pos, const char *txt, NkColor col) noexcept {
		...
		void NkUIGizmo::DrawGrid2D(const NkUIGridConfig &cfg, NkUIDrawList &dl, NkRect viewport,
								   NkVec2 originPx) noexcept {
			if (!cfg.enabled || !cfg.showPlane2D)
				return;
			...
			// Axes
			if (cfg.showAxisX)
				dl.AddLine({viewport.x, originPx.y}, {viewport.x + viewport.w, originPx.y}, cfg.axisXColor, 1.5f);
			if (cfg.showAxisY)
				dl.AddLine({originPx.x, viewport.y}, {originPx.x, viewport.y + viewport.h}, cfg.axisYColor, 1.5f);
		...
		void NkUIGizmo::DrawGrid3D(const NkUIGridConfig &cfg, NkUIDrawList &dl, const NkUIGizmo3DDesc &desc,
			...
			if (cfg.showPlaneXZ) {
				...
				for (int32 i = -nLines / 2; i <= nLines / 2; ++i) {
					...
					dl.AddLine({desc.originPx.x - dx.x * half + offZx, desc.originPx.y - dx.y * half + offZy},
							   {desc.originPx.x + dx.x * half + offZx, desc.originPx.y + dx.y * half + offZy}, lc, 1.f);

					// Ligne parallèle à Z (à distance i * step le long de X)
					const float32 offXx = dx.x * fi * step;
					const float32 offXy = dx.y * fi * step;
					dl.AddLine({desc.originPx.x - dz.x * half + offXx, desc.originPx.y - dz.y * half + offXy},
							   {desc.originPx.x + dz.x * half + offXx, desc.originPx.y + dz.y * half + offXy}, lc, 1.f);
				}
			}
			...
			if (cfg.showPlaneYZ) {
				if (cfg.solid) {
					const NkVec2 c0{desc.originPx.x - dy.x * half - dz.x * half,
									desc.originPx.y - dy.y * half - dz.y * half};
					const NkVec2 c1{desc.originPx.x + dy.x * half - dz.x * half,
									desc.originPx.y + dy.y * half - dz.y * half};
					const NkVec2 c2{desc.originPx.x + dy.x * half + dz.x * half,
									desc.originPx.y + dy.y * half + dz.y * half};
					const NkVec2 c3{desc.originPx.x - dy.x * half + dz.x * half,
									desc.originPx.y - dy.y * half + dz.y * half};
					dl.AddTriangleFilled(c0, c1, c2, cfg.solidColor);
					dl.AddTriangleFilled(c0, c2, c3, cfg.solidColor);
				}
				for (int32 i = -nLines / 2; i <= nLines / 2; ++i) {
					...
					dl.AddLine({desc.originPx.x - dy.x * half + odx, desc.originPx.y - dy.y * half + ody},
							   {desc.originPx.x + dy.x * half + odx, desc.originPx.y + dy.y * half + ody}, lc, 1.f);
					const float32 oex = dy.x * fi * step, oey = dy.y * fi * step;
					dl.AddLine({desc.originPx.x - dz.x * half + oex, desc.originPx.y - dz.y * half + oey},
							   {desc.originPx.x + dz.x * half + oex, desc.originPx.y + dz.y * half + oey}, lc, 1.f);
				}
			}
			...
			if (cfg.showAxisX)
				dl.AddLine({desc.originPx.x - dx.x * axExtent, desc.originPx.y - dx.y * axExtent},
						   {desc.originPx.x + dx.x * axExtent, desc.originPx.y + dx.y * axExtent}, cfg.axisXColor,
						   1.8f);
			if (cfg.showAxisY)
				dl.AddLine({desc.originPx.x - dy.x * axExtent, desc.originPx.y - dy.y * axExtent},
						   {desc.originPx.x + dy.x * axExtent, desc.originPx.y + dy.y * axExtent}, cfg.axisYColor,
						   1.8f);
			if (cfg.showAxisZ)
				dl.AddLine({desc.originPx.x - dz.x * axExtent, desc.originPx.y - dz.y * axExtent},
						   {desc.originPx.x + dz.x * axExtent, desc.originPx.y + dz.y * axExtent}, cfg.axisZColor,
						   1.8f);
		...
		NkUIGizmoResult NkUIGizmo::Manipulate2D(NkUIContext &ctx, NkUIDrawList &dl, const NkUIGizmo2DDesc &desc,
												const NkUIGizmoConfig &cfg, NkUIGizmoState &state,
			...
			const NkVec2 origin{desc.originPx.x + transform.position.x, desc.originPx.y + transform.position.y};
			...
			if (state.active && ctx.input.mouseDown[0]) {
				const NkVec2 dir = (state.activeAxis == NKGIZMO_AX_X) ? NkVec2{1.f, 0.f} : NkVec2{0.f, -1.f};
				float32 d = Dot2(ctx.input.mouseDelta, dir);

				if (cfg.mode == NkUIGizmoMode::NK_TRANSLATE) {
					if (cfg.snap.enabled) {
						float32 &acc = (state.activeAxis == NKGIZMO_AX_X) ? state.snapAccumX : state.snapAccumY;
						acc += d;
					...
					if (state.activeAxis == NKGIZMO_AX_X) {
						...
						out.deltaPosition.x = d;
					} else {
						transform.position.y -= d;
						out.deltaPosition.y = -d;
					}
			...
			const NkColor cX = (state.activeAxis == NKGIZMO_AX_X || hovAxis == NKGIZMO_AX_X) ? cfg.colorHL : cfg.colorX;
			const NkColor cY = (state.activeAxis == NKGIZMO_AX_Y || hovAxis == NKGIZMO_AX_Y) ? cfg.colorHL : cfg.colorY;
			...
			if (axX) {
				...
				if (cfg.showAxisLabels)
					DrawAxisLabel(dl, {endX.x + 7.f, endX.y - 7.f}, "X", cX);
			}
			if (axY) {
				dl.AddLine(origin, endY, cY, cfg.axisThickness);
				dl.AddCircleFilled(endY, cfg.handleRadius, cY);
				if (cfg.showAxisLabels)
					DrawAxisLabel(dl, {endY.x + 7.f, endY.y - 7.f}, "Y", cY);
			}
			if (cfg.showCenter)
		...
		NkUIGizmoResult NkUIGizmo::Manipulate3D(NkUIContext &ctx, NkUIDrawList &dl, const NkUIGizmo3DDesc &desc,
												const NkUIGizmoConfig &cfg, NkUIGizmoState &state,
												NkUIGizmoTransform &transform) noexcept {
			NkUIGizmoResult out{};
			...
			if (!state.active && InVP(desc.viewport, mp)) {
				...
				if (cfg.showPlanes && cfg.mode == NkUIGizmoMode::NK_TRANSLATE) {
					if (PointInQuad(mp, pXY0, pXY1, pXY2, pXY3))
						hovAxis = NKGIZMO_AX_XY;
					else if (PointInQuad(mp, pXZ0, pXZ1, pXZ2, pXZ3))
						hovAxis = NKGIZMO_AX_XZ;
					else if (PointInQuad(mp, pYZ0, pYZ1, pYZ2, pYZ3))
						hovAxis = NKGIZMO_AX_YZ;
				}

				// Centre translate libre
				if (hovAxis == NK_GIZMO_AXIS_NONE && cfg.mode == NkUIGizmoMode::NK_TRANSLATE && cfg.showCenterMove &&
					Len2({mp.x - O.x, mp.y - O.y}) < cR * 2.f)
					hovAxis = NKGIZMO_AX_UNI;

				// Centre scale uniforme
				if (hovAxis == NK_GIZMO_AXIS_NONE && cfg.mode == NkUIGizmoMode::NK_SCALE && cfg.showCenterScale) {
				...
				if (cfg.mode == NkUIGizmoMode::NK_ROTATE && hovAxis == NK_GIZMO_AXIS_NONE) {
					...
					if (hovAxis == NK_GIZMO_AXIS_NONE)
						hArc(dy, NKGIZMO_AX_Y);
					if (hovAxis == NK_GIZMO_AXIS_NONE)
						hArc(dz, NKGIZMO_AX_Z);
				}
			}
			...
			if (state.active && ctx.input.mouseDown[0]) {
				...
				if (cfg.mode == NkUIGizmoMode::NK_TRANSLATE) {
					...
					if (ax == NKGIZMO_AX_X) {
						out.deltaRotationDeg.x = deg;
						state.accumX += deg;
						transform.rotationDeg.x += deg;
						out.totalRotationDeg.x = state.accumX;
					} else if (ax == NKGIZMO_AX_Y) {
						out.deltaRotationDeg.y = deg;
						state.accumY += deg;
						transform.rotationDeg.y += deg;
						out.totalRotationDeg.y = state.accumY;
					} else {
						out.deltaRotationDeg.z = deg;
						state.accumZ += deg;
						transform.rotationDeg.z += deg;
						out.totalRotationDeg.z = state.accumZ;
					}
				} else { // Scale
					...
					if (cfg.snap.enabled && unif) {
						state.dragAccum += sx;
						const float32 s = SnapV(state.dragAccum, cfg.snap.scaleStep);
						sx = sy = sz = s - (state.dragAccum - sx);
						state.dragAccum = s;
					} else if (cfg.snap.enabled) {
						auto sn = [&](float32 &v, float32 &acc) {
							acc += v;
							const float32 s = SnapV(acc, cfg.snap.scaleStep);
							v = s - (acc - v);
							acc = s;
						};
						sn(sx, state.snapAccumX);
						sn(sy, state.snapAccumY);
						sn(sz, state.snapAccumZ);
					}
			...
			auto drawPlane = [&](NkVec2 a, NkVec2 b, NkVec2 c, NkVec2 d, NkColor col, uint8 pid) {
			...
			auto rotArc = [&](NkVec2 axDir, NkColor col, uint8 ax2) {
				...
				dl.AddArc(ac, arcR * 0.82f, baseA - sweep * 0.5f, baseA + sweep * 0.5f, col, cfg.axisThickness + 1.f,
						  24);
				...
				if (cfg.showRotationAngle && state.active && state.activeAxis == ax2) {
					...
					if (::fabsf(totalDeg) > 0.5f) {
						...
						dl.AddArc(ac, arcR * 0.82f, (p0 < p1 ? p0 : p1), (p0 < p1 ? p1 : p0), col.WithAlpha(160),
								  cfg.axisThickness + 3.f, 24);
					}
				}
			};
			...
			// Snap grid visuel
			auto drawSnapGrid = [&]() {
				if (!cfg.showSnapGrid || !cfg.snap.enabled || !state.active)
					return;
				if (cfg.mode != NkUIGizmoMode::NK_TRANSLATE)
					return;
				const uint8 ax2 = state.activeAxis;
				// Seulement pour les axes simples — comparaisons explicites
				if (ax2 != NKGIZMO_AX_X && ax2 != NKGIZMO_AX_Y && ax2 != NKGIZMO_AX_Z)
					return;
				...
				for (int32 i = -nL; i <= nL; ++i) {
					...
					dl.AddLine({lp.x - perp.x * arm, lp.y - perp.y * arm}, {lp.x + perp.x * arm, lp.y + perp.y * arm},
