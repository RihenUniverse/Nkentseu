...
namespace nkentseu {
	namespace nkui {
		...
		NkUIViewport3D::ProjectCtx NkUIViewport3D::MakeProjectCtx(const NkVP3DCamera &cam, NkRect viewport) noexcept {
			ProjectCtx pc;
		...
		NkVec2 NkUIViewport3D::Project(const ProjectCtx &pc, float32 wx, float32 wy, float32 wz) noexcept {
			...
			if (pc.isPerspective) {
				...
				return {pc.center.x + x1 * d * pc.scale, pc.center.y - y2 * d * pc.scale};
			} else {
				// Orthographique
				return {pc.center.x + x1 * pc.scale, pc.center.y - y2 * pc.scale};
			}
		}

		NkVec2 NkUIViewport3D::ProjectDir(const ProjectCtx &pc, float32 dx, float32 dy, float32 dz) noexcept {
		...
		NkUIViewport3D::Layout NkUIViewport3D::ComputeLayout(NkRect r, const NkVP3DConfig &cfg) noexcept {
			Layout ly;
			...
			ly.details = {r.x + r.w - ow, contentY + contentH * 0.45f, ow, contentH * 0.55f};
		...
		void NkUIViewport3D::DrawCube(NkUIDrawList &dl, NkRect clip, const ProjectCtx &pc, float32 px, float32 py,
									  float32 pz, float32 sx, float32 sy, float32 sz, NkColor col,
									  float32 lw) noexcept {
			const float32 hx = sx * 0.5f, hy = sy * 0.5f, hz = sz * 0.5f;
			NkVec2 v[8];
			v[0] = Project(pc, px - hx, py - hy, pz - hz);
			v[1] = Project(pc, px + hx, py - hy, pz - hz);
			v[2] = Project(pc, px + hx, py + hy, pz - hz);
			v[3] = Project(pc, px - hx, py + hy, pz - hz);
			v[4] = Project(pc, px - hx, py - hy, pz + hz);
			v[5] = Project(pc, px + hx, py - hy, pz + hz);
			v[6] = Project(pc, px + hx, py + hy, pz + hz);
			v[7] = Project(pc, px - hx, py + hy, pz + hz);
			const int32 edges[12][2] = {{0, 1}, {1, 2}, {2, 3}, {3, 0}, {4, 5}, {5, 6},
										{6, 7}, {7, 4}, {0, 4}, {1, 5}, {2, 6}, {3, 7}};
			dl.PushClipRect(clip, true);
			for (const auto &e : edges)
				dl.AddLine(v[e[0]], v[e[1]], col, lw);
			dl.PopClipRect();
		}

		void NkUIViewport3D::DrawSphere(NkUIDrawList &dl, NkRect clip, const ProjectCtx &pc, float32 px, float32 py,
										float32 pz, float32 r, NkColor col, float32 lw) noexcept {
			const int32 seg = 24;
			dl.PushClipRect(clip, true);
			for (int32 plane = 0; plane < 3; ++plane) {
				...
				for (int32 k = 0; k <= seg; ++k) {
					...
					NkVec2 pt;
					if (plane == 0)
						pt = Project(pc, px + ca, py + sa, pz);
					else if (plane == 1)
						pt = Project(pc, px + ca, py, pz + sa);
					else
						pt = Project(pc, px, py + ca, pz + sa);
					if (k > 0)
						dl.AddLine(prev, pt, col, lw);
					prev = pt;
				}
			}
			dl.PopClipRect();
		...
		void NkUIViewport3D::DrawCylinder(NkUIDrawList &dl, NkRect clip, const ProjectCtx &pc, float32 px, float32 py,
										  float32 pz, float32 rx, float32 ry, NkColor col, float32 lw) noexcept {
			const int32 seg = 16;
			...
			for (int32 k = 0; k <= seg; ++k) {
				...
				if (k > 0) {
					dl.AddLine(top[k - 1], top[k], col, lw);
					dl.AddLine(bot[k - 1], bot[k], col, lw);
				}
			}
			// Colonnes verticales sur 8 points
			for (int32 k = 0; k < seg; k += 2)
				dl.AddLine(top[k], bot[k], col, lw);
			dl.PopClipRect();
		}

		void NkUIViewport3D::DrawCone(NkUIDrawList &dl, NkRect clip, const ProjectCtx &pc, float32 px, float32 py,
									  float32 pz, float32 r, float32 h, NkColor col, float32 lw) noexcept {
			const int32 seg = 16;
			...
			for (int32 k = 0; k <= seg; ++k) {
				...
				if (k > 0) {
					dl.AddLine(base[k - 1], base[k], col, lw);
				}
				if (k % 4 == 0)
					dl.AddLine(tip, base[k], col, lw);
			}
			dl.PopClipRect();
		}

		void NkUIViewport3D::DrawPlane(NkUIDrawList &dl, NkRect clip, const ProjectCtx &pc, float32 px, float32 py,
									   float32 pz, float32 sx, float32 sz, NkColor col, float32 lw) noexcept {
			...
			dl.PushClipRect(clip, true);
			dl.AddLine(v0, v1, col, lw);
			dl.AddLine(v1, v2, col, lw);
			dl.AddLine(v2, v3, col, lw);
			dl.AddLine(v3, v0, col, lw);
			dl.AddLine(v0, v2, col.WithAlpha(100), lw * 0.6f);
			dl.AddLine(v1, v3, col.WithAlpha(100), lw * 0.6f);
			dl.PopClipRect();
		}

		void NkUIViewport3D::DrawLightIcon(NkUIDrawList &dl, const ProjectCtx &pc, float32 px, float32 py, float32 pz,
			...
			if (type == NkVP3DObjectShape::NK_VP3D_LIGHT_POINT) {
				...
				for (int32 i = 0; i < 8; ++i) {
					...
					dl.AddLine({c.x + ::cosf(a) * (r + 2.f), c.y + ::sinf(a) * (r + 2.f)},
							   {c.x + ::cosf(a) * (r + 6.f), c.y + ::sinf(a) * (r + 6.f)}, col, 1.2f);
				}
			} else {
				// Directionnel : flèche vers le bas
				dl.AddLine({c.x, c.y + r}, {c.x, c.y + r + 12.f}, col, 1.5f);
				dl.AddTriangleFilled({c.x, c.y + r + 16.f}, {c.x - 4.f, c.y + r + 10.f}, {c.x + 4.f, c.y + r + 10.f},
									 col);
			}
		}

		void NkUIViewport3D::DrawCameraIcon(NkUIDrawList &dl, const ProjectCtx &pc, float32 px, float32 py, float32 pz,
			...
			// Viseur
			dl.AddTriangleFilled({c.x + 12.f, c.y - 5.f}, {c.x + 20.f, c.y - 8.f}, {c.x + 20.f, c.y + 8.f},
								 col.WithAlpha(160));
			dl.AddTriangleFilled({c.x + 12.f, c.y + 5.f}, {c.x + 20.f, c.y - 8.f}, {c.x + 20.f, c.y + 8.f},
								 col.WithAlpha(160));
		}

		void NkUIViewport3D::DrawEmptyIcon(NkUIDrawList &dl, const ProjectCtx &pc, float32 px, float32 py, float32 pz,
										   NkColor col) noexcept {
			const NkVec2 c = Project(pc, px, py, pz);
			const float32 s = 8.f;
		...
		void NkUIViewport3D::DrawObjectShape(NkUIDrawList &dl, NkRect viewport, const NkVP3DObject &obj,
											 const ProjectCtx &pc, bool selected, bool hovered, const NkVP3DConfig &cfg,
											 NkVP3DShadingMode shading) noexcept {
			(void)shading;
			(void)cfg;
			const NkColor baseCol = obj.color;
			const NkColor wireCol = selected ? kSelectionHL : hovered ? NkColor{255, 220, 150, 220} : obj.wireColor;
		...
		float32 NkUIViewport3D::PickObject(const NkVP3DObject &obj, const ProjectCtx &pc, NkVec2 mp) noexcept {
			const NkVec2 c = Project(pc, obj.transform.position.x, obj.transform.position.y, obj.transform.position.z);
			const float32 dist = Len2({mp.x - c.x, mp.y - c.y});
			const float32 pickR =
				::fmaxf(16.f, ::fmaxf(obj.transform.scale.x, ::fmaxf(obj.transform.scale.y, obj.transform.scale.z)) *
								  pc.scale * 0.6f);
		...
		const char *NkUIViewport3D::ShapeName(NkVP3DObjectShape s) noexcept {
			switch (s) {
				...
				case NkVP3DObjectShape::NK_VP3D_CONE:
					return "Cone";
				case NkVP3DObjectShape::NK_VP3D_TORUS:
					return "Torus";
				case NkVP3DObjectShape::NK_VP3D_LIGHT_POINT:
					return "Point Light";
				case NkVP3DObjectShape::NK_VP3D_LIGHT_DIR:
		...
		void NkUIViewport3D::DrawShapeIcon(NkUIDrawList &dl, NkRect r, NkVP3DObjectShape s, NkColor col) noexcept {
			...
			switch (s) {
				...
				case NkVP3DObjectShape::NK_VP3D_CAMERA:
					...
					dl.AddTriangleFilled({cx + sz * 0.7f, cy - sz * 0.6f}, {cx + sz * 1.3f, cy - sz * 0.4f},
										 {cx + sz * 0.7f, cy + sz * 0.6f}, col.WithAlpha(160));
					break;
				case NkVP3DObjectShape::NK_VP3D_EMPTY:
		...
		int32 NkUIViewport3D::AddObject(NkVP3DState &state, const char *name, NkVP3DObjectShape shape,
										NkColor color) noexcept {
			if (state.numObjects >= NkVP3DState::MAX_OBJECTS)
				return -1;
		...
		void NkUIViewport3D::FocusSelected(NkVP3DState &state, const NkVP3DConfig &cfg) noexcept {
			...
			for (int32 k = 0; k < state.numSelected; ++k) {
				...
				const float32 r = ::fmaxf(o.transform.scale.x, ::fmaxf(o.transform.scale.y, o.transform.scale.z));
				if (r > maxR)
					maxR = r;
			}
			...
			state.camera.pivotX = cx;
			state.camera.pivotY = cy;
			state.camera.pivotZ = cz;
		...
		void NkUIViewport3D::DrawToolbar(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect r,
										 NkVP3DConfig &cfg, NkVP3DState &state) noexcept {
			// Fond toolbar
			...
			auto btn = [&](const char *label, float32 bw, bool active, bool enabled = true) -> bool {
				...
				if (active)
					dl.AddRect(br, kAccentHover.WithAlpha(160), 1.f, 3.f);
				else
					dl.AddRect(br, kPanelBorder.WithAlpha(active ? 200 : 100), 1.f, 3.f);
				...
				font.RenderText(dl, {tx, ty}, label, enabled ? kTextPrimary : kTextDisabled, bw, false);
				cx += bw + 3.f;
			...
			// ── Séparateur ─────────────────────────────────────────────────
			auto sep = [&]() {
				dl.AddLine({cx + 2.f, by + 4.f}, {cx + 2.f, by + bh - 4.f}, kPanelBorder.WithAlpha(160), 1.f);
				cx += 8.f;
			};
			...
			if (icoBtn(bh, false, [&](NkRect ir, bool hov) {
					...
					dl.AddRectFilled({ir.x + ir.w * 0.55f, ir.y + ir.h * 0.60f, ir.w * 0.15f, ir.h * 0.35f}, c, 1.f);
				})) {
			}
			...
			if (icoBtn(bh, state.snapEnabled, [&](NkRect ir, bool hov) {
					...
					dl.AddArc({cx2, cy2 + ir.h * 0.05f}, ir.h * 0.32f, NKUI_PI * 0.05f, NKUI_PI * 0.95f, c, 2.f, 10);
					dl.AddLine({cx2 - ir.h * 0.32f, cy2 + ir.h * 0.05f}, {cx2 - ir.h * 0.32f, cy2 + ir.h * 0.35f}, c,
							   2.f);
					dl.AddLine({cx2 + ir.h * 0.32f, cy2 + ir.h * 0.05f}, {cx2 + ir.h * 0.32f, cy2 + ir.h * 0.35f}, c,
							   2.f);
					dl.AddLine({cx2 - ir.h * 0.32f, cy2 + ir.h * 0.35f}, {cx2 - ir.h * 0.18f, cy2 + ir.h * 0.35f}, c,
							   2.f);
					dl.AddLine({cx2 + ir.h * 0.32f, cy2 + ir.h * 0.35f}, {cx2 + ir.h * 0.18f, cy2 + ir.h * 0.35f}, c,
							   2.f);
				})) {
			...
			const char *viewLabels[] = {"Persp", "Front", "Back", "Left", "Right", "Top", "Bot"};
			const NkVP3DCameraMode viewModes[] = {
				NkVP3DCameraMode::NK_VP3D_PERSPECTIVE,	NkVP3DCameraMode::NK_VP3D_ORTHO_FRONT,
				NkVP3DCameraMode::NK_VP3D_ORTHO_BACK,	NkVP3DCameraMode::NK_VP3D_ORTHO_LEFT,
				NkVP3DCameraMode::NK_VP3D_ORTHO_RIGHT,	NkVP3DCameraMode::NK_VP3D_ORTHO_TOP,
			...
			if (btn("Grid", 40.f, cfg.showGrid))
				cfg.showGrid = !cfg.showGrid;
			if (btn("Stats", 44.f, cfg.showStats))
				cfg.showStats = !cfg.showStats;
			...
			// ── DROITE : Add Object ────────────────────────────────────────
			{
				...
				font.RenderText(
					dl, {addX + (addW - tw) * 0.5f, by + (bh - font.metrics.lineHeight) * 0.5f + font.metrics.ascender},
		...
		void NkUIViewport3D::DrawGrid(NkUIDrawList &dl, NkRect r, const NkVP3DConfig &cfg, const NkVP3DState &state,
			...
			for (int32 i = -nCoarse * nFine; i <= nCoarse * nFine; ++i) {
				if (i % nFine == 0)
					continue; // ligne principale → traitée après
				const float32 t = fineU * static_cast<float32>(i);
				const float32 ext = nCoarse * coarseU;
				const float32 fade = 1.f - Clamp01(::fabsf(t) / ext * 1.2f);
				if (fade < 0.05f)
					continue;
				const NkColor fc = kGridLineSub.WithAlpha(static_cast<uint8>(kGridLineSub.a * fade));
			...
			for (int32 i = -nCoarse; i <= nCoarse; ++i) {
				if (i == 0)
					continue; // axes traités séparément
			...
			dl.AddLine(Project(pc, -axExt, 0.f, 0.f), Project(pc, axExt, 0.f, 0.f), kAxisX.WithAlpha(180), 1.8f);
			dl.AddLine(Project(pc, 0.f, -axExt, 0.f), Project(pc, 0.f, axExt, 0.f), kAxisY.WithAlpha(180), 1.8f);
			dl.AddLine(Project(pc, 0.f, 0.f, -axExt), Project(pc, 0.f, 0.f, axExt), kAxisZ.WithAlpha(180), 1.8f);
		...
		void NkUIViewport3D::DrawMiniAxes(NkUIDrawList &dl, NkRect viewport, const ProjectCtx &pc) noexcept {
		...
		void NkUIViewport3D::DrawOrientationCube(NkUIDrawList &dl, NkUIFont &font, NkRect viewport,
			...
			NkVec2 v[8];
			v[0] = proj(-1.f, -1.f, -1.f);
			v[1] = proj(1.f, -1.f, -1.f);
			v[2] = proj(1.f, 1.f, -1.f);
			v[3] = proj(-1.f, 1.f, -1.f);
			v[4] = proj(-1.f, -1.f, 1.f);
			v[5] = proj(1.f, -1.f, 1.f);
			v[6] = proj(1.f, 1.f, 1.f);
			v[7] = proj(-1.f, 1.f, 1.f);
		...
		void NkUIViewport3D::DrawStats(NkUIDrawList &dl, NkUIFont &font, NkRect viewport,
		...
		void NkUIViewport3D::DrawNavigationHelp(NkUIDrawList &dl, NkUIFont &font, NkRect viewport) noexcept {
			const char *help = "LMB: Orbit  RMB: Pan  Wheel: Zoom  "
			...
			dl.AddRectFilled(
				{x - padH, y - padH - font.metrics.ascender, tw + padH * 2.f, font.metrics.lineHeight + padH * 2.f},
				{0, 0, 0, 130}, 3.f);
		...
		void NkUIViewport3D::DrawSelectionOverlay(NkUIDrawList &dl, NkUIFont &font, NkRect viewport,
												  const NkVP3DState &state, const ProjectCtx &pc) noexcept {
			if (state.numSelected == 0)
				return;
		...
		void NkUIViewport3D::HandleNavigation(NkUIContext &ctx, NkRect r, NkVP3DConfig &cfg, NkVP3DState &state,
											  bool gizmoActive, float32 dt) noexcept {
			...
			if (ctx.input.IsMouseDown(1)) {
				if (!state.panning && !ctx.input.IsMouseClicked(1))
					state.panning = true;
			}
		...
		void NkUIViewport3D::HandleKeyboardShortcuts(NkUIContext &ctx, NkVP3DConfig &cfg, NkVP3DState &state,
													 NkRect viewport) noexcept {
			if (!InRect(viewport, ctx.input.mousePos))
				return;
			(void)cfg;
			...
			if (ctx.input.IsKeyPressed(NkKey::NK_NUMPAD_0)) {
				state.camera.mode = NkVP3DCameraMode::NK_VP3D_ORTHO_FRONT;
				state.camera.yaw = 0;
				state.camera.pitch = 0;
			}
			if (ctx.input.IsKeyPressed(NkKey::NK_NUMPAD_3)) {
				state.camera.mode = NkVP3DCameraMode::NK_VP3D_ORTHO_RIGHT;
				state.camera.yaw = 90;
				state.camera.pitch = 0;
			}
			if (ctx.input.IsKeyPressed(NkKey::NK_NUMPAD_7)) {
				state.camera.mode = NkVP3DCameraMode::NK_VP3D_ORTHO_TOP;
				state.camera.yaw = 0;
		...
		void NkUIViewport3D::DrawObjects(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect r,
										 NkVP3DConfig &cfg, NkVP3DState &state, const ProjectCtx &pc, bool leftClick,
			...
			for (int32 oi = 0; oi < state.numObjects; ++oi) {
				...
				if (isSel || isHov) {
					const NkVec2 c =
						Project(pc, obj.transform.position.x, obj.transform.position.y, obj.transform.position.z);
					if (InRect(r, c)) {
						const float32 tw = font.MeasureWidth(obj.name);
						font.RenderText(dl, {c.x - tw * 0.5f, c.y - 22.f + font.metrics.ascender}, obj.name,
										isSel ? kSelectionHL : NkColor{200, 200, 200, 160}, tw + 4.f, false);
					}
				}
			}
			...
			if (leftClick) {
				...
				if (bestPickIdx >= 0) {
					...
					int32 selPos = -1;
					for (int32 k = 0; k < state.numSelected; ++k)
						if (state.selectedIdx[k] == bestPickIdx) {
							alreadySel = true;
							selPos = k;
							break;
						}
		...
		void NkUIViewport3D::DrawGizmo(NkUIContext &ctx, NkUIDrawList &dl, NkRect r, NkVP3DConfig &cfg,
									   NkVP3DState &state, const ProjectCtx &pc, NkVP3DResult &result) noexcept {
			if (state.numSelected == 0)
				return;
			...
			NkUIGizmoResult gr = NkUIGizmo::Manipulate3D(ctx, dl, gdesc, state.gizmoCfg, obj.gizmoState, obj.transform);
		...
		NkVP3DResult NkUIViewport3D::DrawViewport(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id,
												  NkRect r, NkVP3DConfig &cfg, NkVP3DState &state,
												  float32 dt) noexcept {
			NkVP3DResult result{};
			...
			dl.AddTriangleFilled({r.x, r.y}, {r.x + r.w, r.y}, {r.x, r.y + r.h * 0.4f}, cfg.bgGradTop.WithAlpha(60));
			...
			if (state.numSelected > 0) {
				const int32 idx = state.selectedIdx[0];
				if (idx >= 0 && idx < state.numObjects)
					gizmoActive = state.objects[idx].gizmoState.active || state.objects[idx].gizmoState.hovered;
			}
			...
			const bool leftClick =
				InRect(r, ctx.input.mousePos) && ctx.input.IsMouseClicked(0) && !gizmoActive && !state.orbiting;
			...
			// Overlays
			if (cfg.showMiniAxes)
				DrawMiniAxes(dl, r, pc);
			if (cfg.showOrientationCube)
				DrawOrientationCube(dl, font, r, viewCam);
			if (cfg.showStats)
				DrawStats(dl, font, r, state);
		...
		void NkUIViewport3D::DrawOutliner(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect r,
										  NkVP3DConfig &cfg, NkVP3DState &state) noexcept {
			(void)cfg;
			// Fond panneau
			...
			font.RenderText(dl, {r.x + 8.f, r.y + (titleH - font.metrics.lineHeight) * 0.5f + asc}, "Outliner",
							kTextPrimary, r.w - 10.f, false);
			...
			if (fHov)
				dl.AddRectFilled(filterR, kHoverBg, 3.f);
			dl.AddLine({filterR.x + 3.f, filterR.y + 5.f}, {filterR.x + filterR.w - 3.f, filterR.y + 5.f}, kTextSecond,
					   1.2f);
			dl.AddLine({filterR.x + 5.f, filterR.y + 9.f}, {filterR.x + filterR.w - 5.f, filterR.y + 9.f}, kTextSecond,
					   1.2f);
			dl.AddLine({filterR.x + 7.f, filterR.y + 13.f}, {filterR.x + filterR.w - 7.f, filterR.y + 13.f},
					   kTextSecond, 1.2f);
			...
			for (int32 oi = 0; oi < state.numObjects; ++oi) {
				...
				if (hov)
					state.outlinerHovered = oi;

				// Fond ligne
				if (isSel)
					dl.AddRectFilled(row, kSelectBg.WithAlpha(160), 2.f);
				else if (hov)
					dl.AddRectFilled(row, kHoverBg, 2.f);
		...
		void NkUIViewport3D::DrawDetails(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect r,
										 NkVP3DConfig &cfg, NkVP3DState &state) noexcept {
			(void)cfg;
			dl.AddRectFilled(r, kPanelBg);
			dl.AddRect(r, kPanelBorder.WithAlpha(160), 1.f);
			...
			dl.AddRectFilled(titleR, kSectionBg);
			dl.AddLine({r.x, r.y + titleH}, {r.x + r.w, r.y + titleH}, kPanelBorder.WithAlpha(150), 1.f);
			font.RenderText(dl, {r.x + 8.f, r.y + (titleH - lh) * 0.5f + asc}, "Details", kTextPrimary, r.w - 10.f,
							false);

			if (state.numSelected == 0) {
				font.RenderText(dl, {r.x + 8.f, r.y + titleH + 12.f + asc}, "No selection", kTextDisabled, r.w - 10.f,
								false);
				return;
			}
			...
			// ── Nom de l'objet ─────────────────────────────────────────────
			{
				...
				font.RenderText(dl, {r.x + ph, y + (20.f - lh) * 0.5f + asc}, obj.name, kSelectionHL, fieldW, true);
				font.RenderText(
					dl, {r.x + r.w - font.MeasureWidth(ShapeName(obj.shape)) - ph, y + (20.f - lh) * 0.5f + asc},
					ShapeName(obj.shape), kTextSecond, fieldW, false);
				y += 22.f;
			...
			auto row3 = [&](const char *label, float32 &x, float32 &yy, float32 &z, NkColor col) {
				...
				struct {
						...
						float32 *val;
						NkColor col2;
				} comps[] = {{"X", &x, kAxisX}, {"Y", &yy, kAxisY}, {"Z", &z, kAxisZ}};
				float32 fx = r.x + ph;
				for (auto &c2 : comps) {
			...
			// Position
			{
				...
				struct {
						...
						float32 val;
						NkColor col2;
				} comps[] = {{"X", px2, kAxisX}, {"Y", py2, kAxisY}, {"Z", pz2, kAxisZ}};
			...
			// Rotation
			{
				...
				struct {
						...
						float32 val;
						NkColor col2;
				} comps[] = {{"X", obj.transform.rotationDeg.x, kAxisX},
			...
			// Scale
			{
				...
				struct {
						...
						float32 val;
						NkColor col2;
				} comps[] = {{"X", obj.transform.scale.x, kAxisX},
			...
			y += 4.f;
			{
				...
				const float32 tw = font.MeasureWidth(lbl);
				font.RenderText(dl, {resetR.x + (fieldW - tw) * 0.5f, resetR.y + (22.f - lh) * 0.5f + asc}, lbl,
								kTextPrimary, fieldW, false);
		...
		void NkUIViewport3D::DrawStatusBar(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect r,
										   const NkVP3DConfig &cfg, const NkVP3DState &state) noexcept {
			(void)cfg;
			(void)ctx;
			dl.AddRectFilled(r, kToolbarBg);
			...
			auto stat = [&](const char *label, NkColor col = kTextSecond) {
				...
				dl.AddLine({cx - 8.f, r.y + 4.f}, {cx - 8.f, r.y + r.h - 4.f}, kPanelBorder.WithAlpha(100), 1.f);
			};
			...
			// Mode
			const char *modeNames[] = {"Perspective",	"Front (Ortho)", "Back (Ortho)",  "Left (Ortho)",
									   "Right (Ortho)", "Top (Ortho)",	 "Bottom (Ortho)"};
		...
		NkVP3DResult NkUIViewport3D::Draw(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id, NkRect rect,
										  NkVP3DConfig &cfg, NkVP3DState &state, float32 dt) noexcept {
			NkVP3DResult result{};
			...
			// Viewport 3D
			{
				...
				if (vr.event != NkVP3DEvent::NK_VP3D_NONE)
					result = vr;
			}
			...
			dl.AddLine({ly.outliner.x, ly.outliner.y}, {ly.outliner.x, ly.outliner.y + ly.outliner.h + ly.details.h},
					   kPanelBorder, 1.f);
