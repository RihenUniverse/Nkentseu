...
namespace nkentseu {
	namespace nkui {
		...
		static NKUI_INLINE int32 Utf8BackSpace(char *s, int32 len) noexcept {
			...
			while (i > 0 && (static_cast<uint8>(s[i]) & 0xC0u) == 0x80u)
				--i;
			s[i] = '\0';
			return i;
		}
		...
		static NKUI_INLINE int32 Utf8Append(char *buf, int32 len, int32 maxLen, uint32 cp) noexcept {
			char enc[4];
			const int32 nb = Utf8Encode(cp, enc);
			if (len + nb >= maxLen)
				return -1;
		...
		NkUIFSProvider NkUIFSProvider::NativeProvider() noexcept {
			...
			p.list = [](const char *path, NkUIFileEntry *out, int32 maxE, void *) -> int32 {
				...
				do {
					...
					e.modifiedTime =
						(mt.QuadPart > 116444736000000000ULL) ? (mt.QuadPart - 116444736000000000ULL) / 10000000ULL : 0;
			...
			p.stat = [](const char *path, NkUIFileEntry *out, void *) -> bool {
				...
				if (h == INVALID_HANDLE_VALUE)
					return false;
				::FindClose(h);
				::strncpy(out->name, fd.cFileName, 255);
				::strncpy(out->path, path, 511);
				out->type = (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? NkUIFileType::NK_FT_DIRECTORY
																			 : NkUIFileType::NK_FT_FILE;
				ULARGE_INTEGER sz;
				sz.LowPart = fd.nFileSizeLow;
				sz.HighPart = fd.nFileSizeHigh;
		...
		NkUIFSProvider NkUIFSProvider::Null() noexcept {
			return {};
		}
		...
		void NkUIFileBrowser::PushHistory(NkUIFileBrowserState &s, const char *path) noexcept {
			if (s.historyPos < s.historyDepth)
				s.historyDepth = s.historyPos;
		...
		void NkUIFileBrowser::SortEntries(NkUIFileBrowserState &s, NkUIFBSortCol col, bool asc) noexcept {
			const int32 n = s.numEntries;
			for (int32 i = 0; i < n - 1; ++i)
				for (int32 j = 0; j < n - 1 - i; ++j) {
					NkUIFileEntry &a = s.entries[j];
					NkUIFileEntry &b = s.entries[j + 1];
		...
		void NkUIFileBrowser::Navigate(NkUIFileBrowserState &s, const char *path) noexcept {
			...
			s.renaming = false;
			s.creatingDir = false;
			s.creatingFile = false;
			s.searchBuffer[0] = '\0';
		}

		void NkUIFileBrowser::Refresh(NkUIFileBrowserState &s, const NkUIFSProvider &p,
		...
		void NkUIFileBrowser::AddBookmark(NkUIFileBrowserConfig &cfg, const char *label, const char *path,
										  bool special) noexcept {
			if (cfg.numBookmarks >= NkUIFileBrowserConfig::MAX_BOOKMARKS)
				return;
			auto &bk = cfg.bookmarks[cfg.numBookmarks++];
			::strncpy(bk.label, label, 63);
			::strncpy(bk.path, path, 511);
			bk.isSpecial = special;
		}

		void NkUIFileBrowser::RemoveBookmark(NkUIFileBrowserConfig &cfg, int32 idx) noexcept {
			if (idx < 0 || idx >= cfg.numBookmarks)
				return;
			for (int32 i = idx; i < cfg.numBookmarks - 1; ++i)
				cfg.bookmarks[i] = cfg.bookmarks[i + 1];
			--cfg.numBookmarks;
		}
		...
		NkColor NkUIFileBrowser::IconColorForEntry(const NkUIFileEntry &e) noexcept {
			...
			if (e.type == NkUIFileType::NK_FT_SYMLINK)
				return {180, 120, 255, 255};
			const char *dot = ::strrchr(e.name, '.');
		...
		NkColor NkUIFileBrowser::ExtensionAccentColor(const char *x) noexcept {
			...
			// Code/Shaders
			if (!::strcasecmp(x, "cpp") || !::strcasecmp(x, "h") || !::strcasecmp(x, "c") || !::strcasecmp(x, "hpp") ||
				!::strcasecmp(x, "cs") || !::strcasecmp(x, "py") || !::strcasecmp(x, "js") || !::strcasecmp(x, "ts") ||
				!::strcasecmp(x, "lua") || !::strcasecmp(x, "glsl") || !::strcasecmp(x, "hlsl") ||
				!::strcasecmp(x, "wgsl") || !::strcasecmp(x, "vert") || !::strcasecmp(x, "frag") ||
				!::strcasecmp(x, "comp"))
		...
		void NkUIFileBrowser::DrawIconFolder(NkUIDrawList &dl, NkRect r, NkColor col, bool open) noexcept {
			...
			if (open) { // ouvert : ligne de séparation
				dl.AddLine({bx + 2.f, by + bh * 0.5f}, {bx + bw - 2.f, by + bh * 0.5f}, col.WithAlpha(120), 1.f);
			}
		}
		...
		void NkUIFileBrowser::DrawIconSymlink(NkUIDrawList &dl, NkRect r, NkColor col) noexcept {
			...
			dl.AddLine({ax + sz * 0.2f, ay + sz * 0.8f}, {ax + sz * 0.8f, ay + sz * 0.2f}, {col.r, col.g, col.b, 230},
					   1.5f);
		...
		void NkUIFileBrowser::DrawIconTrash(NkUIDrawList &dl, NkRect r, NkColor col, bool full) noexcept {
			...
			if (full) {
				...
				dl.AddLine({bx + bw * 0.82f, by - bh * 0.20f}, {bx + bw * 0.82f, by - bh * 0.06f}, {255, 255, 255, 200},
						   1.2f);
			}
		}
		...
		void NkUIFileBrowser::DrawIconRefresh(NkUIDrawList &dl, NkRect r, NkColor col) noexcept {
			...
			dl.AddTriangleFilled(
				{ex, ey}, {ex - tx * rad * 0.38f + ty * rad * 0.20f, ey - ty * rad * 0.38f - tx * rad * 0.20f},
		...
		void NkUIFileBrowser::DrawIconSearch(NkUIDrawList &dl, NkRect r, NkColor col) noexcept {
			...
			dl.AddLine({cx + rad * ::cosf(a), cy + rad * ::sinf(a)}, {r.x + r.w * 0.86f, r.y + r.h * 0.86f}, col, 2.f);
		}

		void NkUIFileBrowser::DrawEntryIcon(NkUIDrawList &dl, NkRect r, const NkUIFileEntry &e, float32) noexcept {
			const NkColor col = IconColorForEntry(e);
			if (e.type == NkUIFileType::NK_FT_DIRECTORY)
				DrawIconFolder(dl, r, col);
			else if (e.type == NkUIFileType::NK_FT_SYMLINK)
				DrawIconSymlink(dl, r, col);
			else
				DrawIconFile(dl, r, col);
		}

		// Miniature enrichie pour le mode grille
		void NkUIFileBrowser::DrawFileThumbnail(NkUIDrawList &dl, NkRect r, const NkUIFileEntry &e) noexcept {
			const NkColor col = IconColorForEntry(e);
		...
		NkUIFileBrowser::Layout NkUIFileBrowser::ComputeLayout(NkRect rect, const NkUIFileBrowserConfig &cfg) noexcept {
			Layout ly;
		...
		void NkUIFileBrowser::DrawBackground(NkUIDrawList &dl, NkRect r, const NkUIContext &ctx) noexcept {
		...
		void NkUIFileBrowser::DrawToolbar(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const Layout &ly,
										  NkUIFileBrowserConfig &cfg, NkUIFileBrowserState &state,
										  const NkUIFSProvider &prov) noexcept {
			const NkRect &r = ly.toolbar;
			dl.AddRectFilled(r, ctx.theme.colors.bgHeader);
			dl.AddLine({r.x, r.y + r.h}, {r.x + r.w, r.y + r.h}, ctx.theme.colors.separator.WithAlpha(80), 1.f);
			...
			auto iconBtn = [&](float32 w, bool en, auto drawFn) -> bool {
				...
				const NkColor bg = hov ? ctx.theme.colors.buttonHover : ctx.theme.colors.bgSecondary.WithAlpha(80);
			...
			if (iconBtn(bw, true, [&](NkRect ir, NkColor c, bool) {
				...
				char par[512];
				::strncpy(par, state.currentPath, 511);
				char *sl = nullptr;
				for (char *pp = par; *pp; ++pp)
					if (*pp == '/' || *pp == '\\')
						sl = pp;
				if (sl && sl != par) {
					*sl = '\0';
					if (::strlen(par) >= ::strlen(cfg.rootPath))
						Navigate(state, par);
				}
			}

			// ↻ Actualiser
			if (iconBtn(bw, true, [&](NkRect ir, NkColor c, bool) { DrawIconRefresh(dl, ir, c); })) {
				state.needsRefresh = true;
			}

			// ── Séparateur visuel ──────────────────────────────────────────────────
			dl.AddLine({cx + 1.f, cy + 4.f}, {cx + 1.f, cy + bh - 4.f}, ctx.theme.colors.separator.WithAlpha(70), 1.f);
			cx += 6.f;
			...
			if (cfg.allowDelete || cfg.allowPermanentDelete) {
				dl.AddLine({cx + 1.f, cy + 4.f}, {cx + 1.f, cy + bh - 4.f}, ctx.theme.colors.separator.WithAlpha(70),
						   1.f);
				cx += 6.f;
			}
			...
			if (cfg.allowDelete)
				if (iconBtn(bw, state.numSelected > 0, [&](NkRect ir, NkColor c, bool hov) {
					...
					if (sp && sp[0])
						::strncpy(state.confirmDeletePath, sp, 511);
				}
			...
			if (cfg.showViewToggle) {
				...
				if (iconBtn(bw, true, [&](NkRect ir, NkColor c, bool) {
						if (isList)
							dl.AddRectFilled(ir, ctx.theme.colors.accent.WithAlpha(50), 2.f);
				...
				if (iconBtn(bw, true, [&](NkRect ir, NkColor c, bool) {
						...
						dl.AddRectFilled(
							{ir.x + ir.w * 0.08f + sz + 2.f, ir.y + ir.h * 0.32f, ir.w * 0.46f, ir.h * 0.12f},
							isTile ? ctx.theme.colors.accent : c, 1.f);
						dl.AddRectFilled(
							{ir.x + ir.w * 0.08f + sz + 2.f, ir.y + ir.h * 0.56f, ir.w * 0.30f, ir.h * 0.10f},
			...
			// ── Barre de recherche ─────────────────────────────────────────────────
			{
				...
				if (sw > 60.f) {
					...
					if (state.searchBuffer[0])
						font.RenderText(dl, {textX, textBl}, state.searchBuffer, ctx.theme.colors.inputText, maxTW,
										false);
					else if (!foc)
						font.RenderText(dl, {textX, textBl}, "Rechercher...", ctx.theme.colors.textDisabled, maxTW,
										false);
					...
					if (foc) {
						...
						if (state.searchBuffer[0]) {
							...
							if (xh)
								dl.AddRectFilled(xr, ctx.theme.colors.danger.WithAlpha(80), 3.f);
							const float32 m = 3.f;
		...
		void NkUIFileBrowser::DrawBreadcrumb(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const Layout &ly,
											 NkUIFileBrowserState &state) noexcept {
			const NkRect &r = ly.breadcrumb;
			dl.AddRectFilled(r, ctx.theme.colors.bgHeader);
			dl.AddLine({r.x, r.y + r.h}, {r.x + r.w, r.y + r.h}, ctx.theme.colors.separator.WithAlpha(60), 1.f);
			...
			auto drawSeg = [&](const char *label, const char *navPath, bool isLast) {
				...
				if (isLast)
					dl.AddRectFilled(btn, ctx.theme.colors.buttonBg.WithAlpha(110), 3.f);
				else if (hov)
					dl.AddRectFilled(btn, ctx.theme.colors.buttonHover, 3.f);
				font.RenderText(dl, {cx + ph, bly}, label,
								isLast ? ctx.theme.colors.textPrimary : ctx.theme.colors.accent, tw, false);
				if (hov && !isLast && ctx.ConsumeMouseClick(0))
					Navigate(state, navPath);
				cx += bw;
				if (!isLast) {
					// Chevron ›
					const float32 sw = font.MeasureWidth(" \xe2\xba\x85 "); // › UTF-8
					font.RenderText(dl, {cx, bly}, " \xe2\xba\x85 ", ctx.theme.colors.textDisabled, sw, false);
					cx += sw;
				}
			};

#if defined(_WIN32)
			const bool atComp = (state.currentPath[0] == '\0' || ::strcmp(state.currentPath, "Computer") == 0);
			...
			while (*p) {
				...
				::memcpy(seg, p, static_cast<size_t>(len));
				seg[len] = '\0';
#if defined(_WIN32)
				if (accum[0])
					::strncat(accum, "\\", 1);
		...
		void NkUIFileBrowser::DrawBookmarkPanel(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const Layout &ly,
												const NkUIFileBrowserConfig &cfg,
												NkUIFileBrowserState &state) noexcept {
			const NkRect &r = ly.bookmarkPanel;
			dl.AddRectFilled(r, ctx.theme.colors.bgSecondary);
			dl.AddLine({r.x + r.w, r.y}, {r.x + r.w, r.y + r.h}, ctx.theme.colors.separator.WithAlpha(80), 1.f);
			...
			// ── Titre section ────────────────────────────────────────────────────────
			font.RenderText(dl, {r.x + ph, y + asc}, "Acc\xc3\xa8s rapide", ctx.theme.colors.textDisabled,
							r.w - ph * 2.f, false);
			y += ih;
			...
			for (int32 i = 0; i < cfg.numBookmarks; ++i) {
				...
				font.RenderText(
					dl, {r.x + 22.f, y + (ih - font.metrics.lineHeight) * 0.5f + asc}, cfg.bookmarks[i].label,
					active ? ctx.theme.colors.textOnAccent : ctx.theme.colors.textSecondary, r.w - 28.f, true);

				if (hov && ctx.ConsumeMouseClick(0))
					Navigate(state, cfg.bookmarks[i].path);
				y += ih;
			}
			...
			if (addH)
				dl.AddRectFilled(addRow, ctx.theme.colors.buttonHover, 3.f);
			...
			font.RenderText(dl, {r.x + 22.f, y + (ih - font.metrics.lineHeight) * 0.5f + asc}, "Marquer ici",
							addH ? ctx.theme.colors.accent : ctx.theme.colors.textDisabled, r.w - 28.f, false);
		...
		void NkUIFileBrowser::DrawColumnHeaders(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const Layout &ly,
												const NkUIFileBrowserConfig &cfg,
												NkUIFileBrowserState &state) noexcept {
			...
			const ColDef cols[] = {
				...
				{cfg.showModifiedDate ? "Modifi\xc3\xa9" : "", cx + state.colNameW + szW, dtW,
				 NkUIFBSortCol::NK_FBS_DATE},
				{cfg.showTypeCol ? "Type" : "", cx + state.colNameW + szW + dtW, tpW, NkUIFBSortCol::NK_FBS_TYPE},
			};
			for (const auto &col : cols) {
				if (!col.lbl || !col.lbl[0] || col.w < 4.f)
					continue;
				const NkRect hr = {col.x, r.y, col.w, r.h};
				const bool hov = NkRectContains(hr, ctx.input.mousePos);
				if (hov)
					dl.AddRectFilled(hr, ctx.theme.colors.buttonHover.WithAlpha(55));
				const bool active = (state.sortCol == col.sc);
				font.RenderText(dl, {col.x + ph, bly}, col.lbl,
								active ? ctx.theme.colors.accent : ctx.theme.colors.textSecondary, col.w - ph - 14.f,
								false);
				if (active)
					dl.AddArrow({col.x + col.w - 10.f, r.y + r.h * 0.5f}, 5.f, state.sortAscending ? 3 : 1,
								ctx.theme.colors.accent);
				if (hov && ctx.ConsumeMouseClick(0)) {
					if (state.sortCol == col.sc)
						state.sortAscending = !state.sortAscending;
					else {
						state.sortCol = col.sc;
						state.sortAscending = true;
					}
					SortEntries(state, state.sortCol, state.sortAscending);
				}
				dl.AddLine({col.x, r.y + 4.f}, {col.x, r.y + r.h - 4.f}, ctx.theme.colors.separator.WithAlpha(45), 1.f);
			}
		}
		...
		static void DrawTextField(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect inp, char *buf,
								  int32 maxLen, bool focused) noexcept {
			dl.AddRectFilled(inp, ctx.theme.colors.inputBg, 3.f);
			dl.AddRect(inp, focused ? ctx.theme.colors.accent : ctx.theme.colors.border, focused ? 2.f : 1.f, 3.f);
			...
			if (focused) {
				bool dummy = true;
				TextFieldInput(ctx, buf, maxLen, dummy);
				(void)maxLen;
			}
		}
		...
		NkUIFileBrowserResult NkUIFileBrowser::DrawEntryRow(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
			...
			// Fond
			if (selected)
				dl.AddRectFilled(row, ctx.theme.colors.accent.WithAlpha(72), 2.f);
			else if (hovered)
				dl.AddRectFilled(row, ctx.theme.colors.buttonHover.WithAlpha(50), 2.f);
			...
			if (cfg.showFileSize && state.colSizeW > 0.f) {
				if (e.type != NkUIFileType::NK_FT_DIRECTORY) {
					char szBuf[32];
					FormatSize(e.size, szBuf, 32);
					font.RenderText(dl, {nX + state.colNameW + ph, bly}, szBuf, ctx.theme.colors.textSecondary,
									state.colSizeW - ph * 2.f, false);
				}
			}
			// ── Date ─────────────────────────────────────────────────────────────────
			if (cfg.showModifiedDate && state.colDateW > 0.f) {
				char dtBuf[32];
				FormatDate(e.modifiedTime, dtBuf, 32);
			...
			if (cfg.showTypeCol && state.colTypeW > 0.f) {
				...
				font.RenderText(dl, {nX + state.colNameW + state.colSizeW + state.colDateW + ph, bly},
								dot ? dot + 1 : "\xe2\x80\x94", ctx.theme.colors.textDisabled,
								state.colTypeW - ph * 2.f, false);
			}
			...
			if (hovered) {
				if (ctx.input.IsMouseClicked(0)) {
					...
					if (multi && ctrl) {
						...
						for (int32 k = 0; k < state.numSelected; ++k)
							if (state.selected[k] == idx) {
								for (int32 j = k; j < state.numSelected - 1; ++j)
									state.selected[j] = state.selected[j + 1];
								--state.numSelected;
								found = true;
								break;
							}
				...
				if (cfg.allowDnD && !state.dndActive && ctx.input.IsMouseDown(0)) {
					...
					if (d.x * d.x + d.y * d.y > 25.f) {
						...
						state.dndSourceIdx = idx;
					}
				}
			}

			// DnD cible (dossier survolé)
			if (state.dndActive && hovered && state.dndSourceIdx != idx && e.type == NkUIFileType::NK_FT_DIRECTORY) {
		...
		NkUIFileBrowserResult NkUIFileBrowser::DrawListView(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
			...
			auto drawInline = [&](bool &creating, char *buf, int32 bufMax, bool isDir) {
				...
				// Fond
				dl.AddRectFilled({listR.x, listR.y + createH, listR.w, rowH}, ctx.theme.colors.accent.WithAlpha(20));
				// Icône
				const NkRect ic = {inp.x, inp.y, rowH - 4.f, rowH - 4.f};
				if (isDir)
					DrawIconNewFolder(dl, ic, ctx.theme.colors.accent);
				else
					DrawIconNewFile(dl, ic, ctx.theme.colors.accent);
				// Champ texte
			...
			for (int32 i = 0; i < state.numEntries; ++i) {
				...
				if (!cfg.showHidden && e.isHidden)
					continue;
				if (state.searchBuffer[0] && !Utf8ContainsI(e.name, state.searchBuffer))
					continue;
				if (!MatchesFilter(e, cfg.filterExt))
					continue;
				++vis;
			}
			...
			if (NkRectContains(r, ctx.input.mousePos) && !ctx.wheelConsumed && ctx.input.mouseWheel != 0.f) {
				state.scrollY -= ctx.input.mouseWheel * rowH * 3.f;
				ctx.wheelConsumed = true;
			}
			const float32 maxSc = (state.contentH > r.h) ? state.contentH - r.h : 0.f;
			if (state.scrollY < 0.f)
				state.scrollY = 0.f;
			if (state.scrollY > maxSc)
				state.scrollY = maxSc;
			...
			if (!state.renaming && !state.creatingDir && !state.creatingFile && !state.searchFocused) {
				...
				if (ctx.input.IsKeyPressed(NkKey::NK_DOWN) && state.numSelected > 0) {
					int32 c = state.selected[0] + 1;
					if (c >= state.numEntries)
						c = state.numEntries - 1;
					state.numSelected = 1;
					state.selected[0] = c;
					const float32 by = createH + (c + 1) * rowH - state.scrollY;
					if (by > r.h)
						state.scrollY = createH + (c + 1) * rowH - r.h;
				}
				if (ctx.input.IsKeyPressed(NkKey::NK_ENTER) && state.numSelected > 0) {
					...
					if (e.type == NkUIFileType::NK_FT_DIRECTORY) {
						...
						tmp.event = NkUIFileBrowserEvent::NK_FB_FILE_SELECTED;
						::strncpy(tmp.path, e.path, 511);
					}
					result = tmp;
				}
				if (ctx.input.ctrl && ctx.input.IsKeyPressed(NkKey::NK_A)) {
					state.numSelected = 0;
					for (int32 i = 0; i < state.numEntries && state.numSelected < NkUIFileBrowserState::MAX_SELECTED;
						 ++i)
			...
			for (int32 i = 0; i < state.numEntries; ++i) {
				...
				if (rr.event != NkUIFileBrowserEvent::NK_FB_NONE)
					result = rr;
				y += rowH;
			}
			...
			if (state.contentH > r.h) {
				...
				dl.AddRectFilled(thumb, (aSB || hSB) ? ctx.theme.colors.scrollThumbHov : ctx.theme.colors.scrollThumb,
								 4.f);
				if (hSB && ctx.ConsumeMouseClick(0))
					ctx.SetActive(sbId);
				if (aSB) {
					float32 rel = (ctx.input.mousePos.y - r.y - tH * 0.5f) / (r.h - tH);
		...
		NkUIFileBrowserResult NkUIFileBrowser::DrawGridView(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
															NkUIID id, const Layout &ly, NkUIFileBrowserConfig &cfg,
															NkUIFileBrowserState &state,
															const NkUIFSProvider &prov) noexcept {
			NkUIFileBrowserResult result{};
			...
			if (isTile) {
				cellW = r.w - kSBW;
				cellH = cfg.rowHeight * 2.0f;
			} else {
				const float32 sz =
					(cfg.viewMode == NkUIFBViewMode::NK_FBVM_ICONS_SMALL) ? cfg.iconSmallSize : cfg.iconGridSize;
				cellW = sz + 8.f;
				cellH = sz + font.metrics.lineHeight + 10.f;
			}
			...
			// Scroll molette
			if (NkRectContains(r, ctx.input.mousePos) && !ctx.wheelConsumed && ctx.input.mouseWheel != 0.f) {
				state.gridScrollY -= ctx.input.mouseWheel * cellH;
				ctx.wheelConsumed = true;
			}
			const float32 maxSc = (state.contentH > r.h) ? state.contentH - r.h : 0.f;
			if (state.gridScrollY < 0.f)
				state.gridScrollY = 0.f;
			if (state.gridScrollY > maxSc)
				state.gridScrollY = maxSc;
			...
			for (int32 i = 0; i < state.numEntries; ++i) {
				...
				if (gy + cellH >= listR.y && gy <= listR.y + listR.h) {
					...
					if (isTile) {
						// ── Mode Tuiles ──────────────────────────────────────────────
						if (sel)
							dl.AddRectFilled(cell, ctx.theme.colors.accent.WithAlpha(72), 4.f);
						else if (hov)
							dl.AddRectFilled(cell, ctx.theme.colors.buttonHover.WithAlpha(55), 4.f);
						...
						// ── Mode Grille ──────────────────────────────────────────────
						if (sel)
							dl.AddRectFilled(cell, ctx.theme.colors.accent.WithAlpha(80), 4.f);
						else if (hov)
							dl.AddRectFilled(cell, ctx.theme.colors.buttonHover.WithAlpha(60), 4.f);
						...
						font.RenderText(dl, {tx, nameY + font.metrics.ascender}, e.name,
										sel ? ctx.theme.colors.textOnAccent : ctx.theme.colors.textPrimary, cellW - 4.f,
										true);
					}
					...
					if (hov) {
						if (ctx.input.IsMouseClicked(0)) {
							...
							if (multi && ctrl) {
								bool found = false;
								for (int32 kk = 0; kk < state.numSelected; ++kk)
									if (state.selected[kk] == i) {
										for (int32 jj = kk; jj < state.numSelected - 1; ++jj)
											state.selected[jj] = state.selected[jj + 1];
										--state.numSelected;
										found = true;
										break;
									}
								...
								state.numSelected = 0;
								for (int32 kk = from;
									 kk <= to && state.numSelected < NkUIFileBrowserState::MAX_SELECTED; ++kk)
									state.selected[state.numSelected++] = kk;
							} else {
								state.numSelected = 1;
								state.selected[0] = i;
		...
		void NkUIFileBrowser::DrawStatusBar(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, const Layout &ly,
											const NkUIFileBrowserConfig &cfg,
		...
		NkUIFileBrowserResult NkUIFileBrowser::DrawSaveRow(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID,
														   const Layout &ly, NkUIFileBrowserConfig &cfg,
														   NkUIFileBrowserState &state) noexcept {
			NkUIFileBrowserResult result{};
			if (!ly.hasSave)
				return result;
		...
		NkUIFileBrowserResult NkUIFileBrowser::DrawConfirmButtons(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
			...
			// ── Annuler ───────────────────────────────────────────────────────────────
			{
				...
				font.RenderText(
					dl,
					{btn.x + (bw - font.MeasureWidth(lbl)) * 0.5f, btn.y + (bh - font.metrics.lineHeight) * 0.5f + asc},
					lbl, ctx.theme.colors.textPrimary, bw, false);
				if (hov && ctx.ConsumeMouseClick(0))
					result.event = NkUIFileBrowserEvent::NK_FB_CANCELLED;
			}
			...
			// ── OK ─────────────────────────────────────────────────────────────────────
			{
				...
				const char *oklbl = (cfg.mode == NkUIFBMode::NK_FBM_SAVE)		  ? "Enregistrer"
									: (cfg.mode == NkUIFBMode::NK_FBM_SELECT_DIR) ? "S\xc3\xa9lectionner"
																				  : "Ouvrir";
				font.RenderText(dl,
								{btn.x + (bw - font.MeasureWidth(oklbl)) * 0.5f,
				...
				if (hov && ctx.ConsumeMouseClick(0)) {
					if (cfg.mode == NkUIFBMode::NK_FBM_SAVE && state.saveFilenameBuffer[0]) {
						...
						for (int32 k = 0; k < state.numSelected && k < NkUIFileBrowserResult::MAX_SELECTED; ++k)
							::strncpy(result.paths[result.numPaths++], state.entries[state.selected[k]].path, 511);
						if (result.numPaths > 0) {
							::strncpy(result.path, result.paths[0], 511);
							result.event = NkUIFileBrowserEvent::NK_FB_FILES_SELECTED;
						}
					} else {
						const char *sp = GetSelectedPath(state);
						if (sp && sp[0]) {
							::strncpy(result.path, sp, 511);
							result.event = NkUIFileBrowserEvent::NK_FB_FILE_SELECTED;
						}
					}
				}
		...
		void NkUIFileBrowser::DrawDeleteOverlay(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect rect,
												NkUIFileBrowserState &state, NkUIFileBrowserResult &result) noexcept {
			if (!state.confirmDeletePending)
				return;
			...
			const char *title = state.confirmPermanentDelete ? "Suppression deinitive" : "Deplacer vers la corbeille?";
			...
			if (state.confirmPermanentDelete) {
				...
				font.RenderText(dl, {textX, panel.y + 18.f + lh * 2.f + 14.f + asc}, warn, NkColor{255, 160, 60, 220},
								pw - 32.f, false);
			}
			...
			// Non
			{
				...
				dl.AddRectFilled(btn, hov ? ctx.theme.colors.buttonHover : ctx.theme.colors.buttonBg, kBtnR);
				dl.AddRect(btn, ctx.theme.colors.border, 1.f, kBtnR);
				font.RenderText(dl, {btn.x + (bw - font.MeasureWidth("Non")) * 0.5f, btn.y + (bh - lh) * 0.5f + asc},
								"Non", ctx.theme.colors.textPrimary, bw, false);
				if (hov && ctx.ConsumeMouseClick(0))
			...
			{
				const NkColor okBg =
					state.confirmPermanentDelete ? NkColor{200, 40, 40, 255} : NkColor{180, 40, 40, 255};
				const NkColor okHov =
					state.confirmPermanentDelete ? NkColor{230, 60, 60, 255} : NkColor{210, 60, 60, 255};
				...
				font.RenderText(dl, {btn.x + (bw - font.MeasureWidth(lbl)) * 0.5f, btn.y + (bh - lh) * 0.5f + asc}, lbl,
								ctx.theme.colors.textOnAccent, bw, false);
				if (hov && ctx.ConsumeMouseClick(0)) {
					::strncpy(result.path, state.confirmDeletePath, 511);
					result.event = state.confirmPermanentDelete ? NkUIFileBrowserEvent::NK_FB_DELETE_PERMANENT
		...
		void NkUIFileBrowser::DrawDndGhost(NkUIDrawList &dl, NkUIFont &font, const NkUIContext &ctx,
										   const NkUIFileBrowserState &state) noexcept {
			if (!state.dndActive || state.dndSourceIdx < 0 || state.dndSourceIdx >= state.numEntries)
				return;
			const NkUIFileEntry &src = state.entries[state.dndSourceIdx];
			...
			font.RenderText(
				dl, {ghost.x + 26.f, ghost.y + (ghost.h - font.metrics.lineHeight) * 0.5f + font.metrics.ascender},
		...
		NkUIFileBrowserResult NkUIFileBrowser::Draw(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id,
													NkRect rect, NkUIFileBrowserConfig &cfg,
													NkUIFileBrowserState &state,
			...
			// Vue active
			{
				...
				if (vr.event != NkUIFileBrowserEvent::NK_FB_NONE)
					result = vr;
			}

			if (ly.hasSave) {
				NkUIFileBrowserResult sr = DrawSaveRow(ctx, dl, font, id, ly, cfg, state);
				if (sr.event != NkUIFileBrowserEvent::NK_FB_NONE)
					result = sr;
			}
			...
			if (cfg.showConfirmButtons) {
				...
				if (cr.event != NkUIFileBrowserEvent::NK_FB_NONE)
					result = cr;
			}
			...
			if (state.dndActive)
				DrawDndGhost(dl, font, ctx, state);
		...
		NkUIAssetType NkUIContentBrowser::GuessAssetType(const char *filename) noexcept {
			...
			if (!::strcasecmp(x, "lua") || !::strcasecmp(x, "py") || !::strcasecmp(x, "cs") || !::strcasecmp(x, "js") ||
				!::strcasecmp(x, "ts") || !::strcasecmp(x, "cpp") || !::strcasecmp(x, "script"))
		...
		NkColor NkUIContentBrowser::AssetTypeColor(NkUIAssetType t) noexcept {
			switch (t) {
				...
				case NkUIAssetType::NK_ASSET_MATERIAL:
					return {100, 230, 140, 255};
				case NkUIAssetType::NK_ASSET_SCENE:
					return {80, 220, 220, 255};
				case NkUIAssetType::NK_ASSET_SCRIPT:
					return {255, 215, 80, 255};
				case NkUIAssetType::NK_ASSET_FONT:
					return {140, 200, 255, 255};
				case NkUIAssetType::NK_ASSET_ANIMATION:
					return {255, 140, 200, 255};
				case NkUIAssetType::NK_ASSET_PREFAB:
		...
		void NkUIContentBrowser::DrawAssetTypeIcon(NkUIDrawList &dl, NkRect r, NkUIAssetType t, bool large) noexcept {
			...
			switch (t) {
				case NkUIAssetType::NK_ASSET_TEXTURE: {
					...
					dl.AddTriangleFilled({cx - hs * 0.5f, cy + hs * 0.3f}, {cx, cy - hs * 0.2f},
										 {cx + hs * 0.5f, cy + hs * 0.3f}, col.WithAlpha(160));
				...
				case NkUIAssetType::NK_ASSET_MATERIAL: {
					...
					// Reflet
					dl.AddArc({cx - hs * 0.18f, cy - hs * 0.25f}, hs * 0.48f, NKUI_PI * 0.95f, NKUI_PI * 1.55f,
				...
				case NkUIAssetType::NK_ASSET_FOLDER:
					...
					break;
				default:
					NkUIFileBrowser::DrawIconFile(dl, r, col);
					break;
			}
		}
		...
		void NkUIContentBrowser::LoadFolder(NkUIContentBrowserState &state, const char *path,
											const NkUIFSProvider &prov) noexcept {
			if (!prov.list || !path || !path[0])
				return;
		...
		void NkUIContentBrowser::BuildTree(NkUIContentBrowserState &state, const char *rootPath,
										   const NkUIFSProvider &prov) noexcept {
			if (!prov.list)
				return;
			...
			for (int32 i = 0; i < n && state.numTreeNodes < NkUIContentBrowserState::MAX_TREE_NODES; ++i) {
				if (entries[i].type != NkUIFileType::NK_FT_DIRECTORY)
					continue;
				...
				for (int32 j = 0; j < sn; ++j)
					if (sub[j].type == NkUIFileType::NK_FT_DIRECTORY) {
		...
		void NkUIContentBrowser::DrawTree(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect rect,
										  NkUIContentBrowserConfig &cfg, NkUIContentBrowserState &state,
										  const NkUIFSProvider &prov, NkUIContentBrowserResult &result) noexcept {
			...
			for (int32 i = 0; i < state.numTreeNodes; ++i) {
				...
				const float32 depth = (node.parent < 0) ? 0.f : (state.treeNodes[node.parent].parent < 0) ? 1.f : 2.f;
				...
				font.RenderText(dl, {rect.x + indent + 28.f, y + (rowH - font.metrics.lineHeight) * 0.5f + asc},
								node.name, active ? ctx.theme.colors.textOnAccent : ctx.theme.colors.textSecondary,
		...
		void NkUIContentBrowser::DrawToolbar(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect rect,
											 NkUIContentBrowserConfig &cfg, NkUIContentBrowserState &state,
											 const NkUIFSProvider &prov) noexcept {
			...
			auto iconBtn = [&](float32 w, bool en, auto drawFn) -> bool {
				const NkRect btn = {cx, cy, w, bh};
				const bool hov = en && NkRectContains(btn, ctx.input.mousePos);
				dl.AddRectFilled(btn, hov ? ctx.theme.colors.buttonHover : ctx.theme.colors.bgSecondary.WithAlpha(70),
								 kBtnR);
				dl.AddRect(btn, ctx.theme.colors.border.WithAlpha(50), 1.f, kBtnR);
			...
			// Refresh
			if (iconBtn(bw, true, [&](NkRect ir, NkColor c, bool) { NkUIFileBrowser::DrawIconRefresh(dl, ir, c); })) {
				if (state.currentPath[0])
					LoadFolder(state, state.currentPath, prov);
			}
			...
			// Label "Taille :"
			{
				...
				font.RenderText(dl, {cx, cy + (bh - font.metrics.lineHeight) * 0.5f + font.metrics.ascender}, lbl,
								ctx.theme.colors.textSecondary, lw, false);
				cx += lw + 4.f;
			}
			...
			// Slider miniature
			{
				...
				const float32 ratio =
					(state.thumbnailSizeCurrent - cfg.minThumbnailSize) / (cfg.maxThumbnailSize - cfg.minThumbnailSize);
				...
				if (ctx.IsActive(slId)) {
					...
					if (rel > 1.f)
						rel = 1.f;
					state.thumbnailSizeCurrent =
						cfg.minThumbnailSize + rel * (cfg.maxThumbnailSize - cfg.minThumbnailSize);
					if (!ctx.input.mouseDown[0])
						ctx.ClearActive();
				}
				cx += sw + 8.f;
			...
			if (sw > 60.f) {
				...
				if (state.searchBuffer[0])
					font.RenderText(dl, {textX, bly}, state.searchBuffer, ctx.theme.colors.inputText, sw - bh - 10.f,
									false);
				else if (!foc)
					font.RenderText(dl, {textX, bly}, "Rechercher assets...", ctx.theme.colors.textDisabled,
									sw - bh - 10.f, false);
				if (NkRectContains(sr, ctx.input.mousePos) && ctx.ConsumeMouseClick(0))
					state.searchFocused = true;
				if (!NkRectContains(sr, ctx.input.mousePos) && ctx.input.IsMouseClicked(0))
		...
		void NkUIContentBrowser::DrawAssetTile(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkRect cell,
											   int32 idx, bool selected, bool hovered, float32 thumbSize,
			...
			// Fond cellule
			if (selected)
				dl.AddRectFilled(cell, ctx.theme.colors.accent.WithAlpha(80), 6.f);
			else if (hovered)
				dl.AddRectFilled(cell, ctx.theme.colors.buttonHover.WithAlpha(65), 6.f);
			...
			if (a.thumbnailTexId > 0) {
				...
				const NkRect ir = {thumbR.x + (thumbR.w - isz) * 0.5f, thumbR.y + (thumbR.h - isz) * 0.5f, isz, isz};
			...
			// Badge type d'asset (coin haut-droite)
			{
				...
				font.RenderText(dl, {badge.x + 3.f, badge.y + font.metrics.ascender}, lbl, {255, 255, 255, 230},
								lw + 4.f, false);
			}

			// Indicateur "non sauvegardé"
			if (a.hasUnsavedChanges)
				dl.AddCircleFilled({thumbR.x + thumbR.w - 6.f, thumbR.y + thumbR.h - 6.f}, 5.f, {255, 200, 50, 240}, 8);
			...
			font.RenderText(dl, {nameX, nameY + (nameH - font.metrics.lineHeight) * 0.5f + asc}, a.name,
							selected ? ctx.theme.colors.textOnAccent : ctx.theme.colors.textPrimary, cell.w - 4.f,
							true);
		...
		NkUIContentBrowserResult NkUIContentBrowser::DrawAssetGrid(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font,
			...
			for (int32 i = 0; i < state.numAssets; ++i) {
				if (state.searchBuffer[0] && !Utf8ContainsI(state.assets[i].name, state.searchBuffer))
					continue;
				++vis;
			}
			const int32 rows = (vis + cols - 1) / cols;
			...
			if (NkRectContains(rect, ctx.input.mousePos) && !ctx.wheelConsumed && ctx.input.mouseWheel != 0.f) {
				state.scrollY -= ctx.input.mouseWheel * cellH;
				ctx.wheelConsumed = true;
			}
			...
			if (totalH > rect.h) {
				...
				if (hSB && ctx.ConsumeMouseClick(0))
					ctx.SetActive(sbId);
				if (aSB) {
					float32 rel = (ctx.input.mousePos.y - rect.y - tH * 0.5f) / (rect.h - tH);
					state.scrollY = (rel < 0.f ? 0.f : rel > 1.f ? 1.f : rel) * maxSc;
					if (!ctx.input.mouseDown[0])
						ctx.ClearActive();
				}
		...
		NkUIContentBrowserResult NkUIContentBrowser::Draw(NkUIContext &ctx, NkUIDrawList &dl, NkUIFont &font, NkUIID id,
														  NkRect rect, NkUIContentBrowserConfig &cfg,
														  NkUIContentBrowserState &state,
														  const NkUIFSProvider &provider) noexcept {
			NkUIContentBrowserResult result{};
			...
			if (state.needsRefresh) {
				...
				if (cfg.showTree)
					BuildTree(state, cfg.rootPath, provider);
			}
			...
			if (cfg.showTree) {
				...
				if (gr.event != NkUIContentBrowserEvent::NK_CB_NONE)
					result = gr;
