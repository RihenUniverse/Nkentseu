...
namespace nkentseu {
	namespace nkui {
		...
		static NKUI_INLINE void UnpackColor(uint32 col, float32 &r, float32 &g, float32 &b, float32 &a) noexcept {
			a = ((col >> 24) & 0xFF) / 255.f;
			r = ((col >> 16) & 0xFF) / 255.f;
			g = ((col >> 8) & 0xFF) / 255.f;
			b = (col & 0xFF) / 255.f;
		...
		void NkUICPURenderer::BlendPixel(int32 x, int32 y, uint8 r, uint8 g, uint8 b, uint8 a) noexcept {
		...
		void NkUICPURenderer::DrawTriangle(const NkUIVertex &v0, const NkUIVertex &v1, const NkUIVertex &v2,
			...
			if (minY < clip.y)
				minY = clip.y;
			if (maxX > clip.x + clip.w)
				maxX = clip.x + clip.w;
			if (maxY > clip.y + clip.h)
				maxY = clip.y + clip.h;
			...
			if (minY < 0)
				minY = 0;
			if (maxX > mW)
				maxX = static_cast<float32>(mW);
			if (maxY > mH)
				maxY = static_cast<float32>(mH);
			...
			for (int32 py = py0; py < py1 && py < mH; ++py) {
				for (int32 px = px0; px < px1 && px < mW; ++px) {
					...
					BlendPixel(px, py, Clamp8f(r * 255), Clamp8f(g * 255), Clamp8f(b * 255), Clamp8f(a * 255));
				}
			}
		}
		...
		NkColor NkUICPURenderer::SampleTexture(uint32 /*texId*/, float32 /*u*/, float32 /*v*/) const noexcept {
