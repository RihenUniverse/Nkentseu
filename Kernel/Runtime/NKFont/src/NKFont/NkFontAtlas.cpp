...
namespace nkentseu {
	...
	namespace {
		...
		static void ApplyMultiplyRect(const nkft_uint8 t[256], nkft_uint8 *px, nkft_int32 x, nkft_int32 y, nkft_int32 w,
									  nkft_int32 h, nkft_int32 stride) {
		...
		static void FlattenQuadratic(const NkVec2f &p0, const NkVec2f &p1, const NkVec2f &p2, float flatness_sq,
									 int depth, NkVector<NkVec2f> &outPoints) {
		...
		static void FlattenCubic(const NkVec2f &p0, const NkVec2f &p1, const NkVec2f &p2, const NkVec2f &p3,
								 float flatness_sq, int depth, NkVector<NkVec2f> &outPoints) {
			NkVec2f d1 = {p1.x - p0.x, p1.y - p0.y};
			NkVec2f d2 = {p2.x - p1.x, p2.y - p1.y};
			NkVec2f d3 = {p3.x - p2.x, p3.y - p2.y};
			NkVec2f d = {p3.x - p0.x, p3.y - p0.y};
			float ll =
				sqrtf(d1.x * d1.x + d1.y * d1.y) + sqrtf(d2.x * d2.x + d2.y * d2.y) + sqrtf(d3.x * d3.x + d3.y * d3.y);
	...
	static const nkft_uint32 sRangesChineseFull[] = {0x0020, 0x00FF, 0x2000, 0x206F, 0x3000, 0x30FF, 0x31F0,
													 0x31FF, 0xFF00, 0xFFEF, 0x4E00, 0x9FAF, 0};

	const nkft_uint32 *NkFontAtlas::GetGlyphRangesDefault() {
		return sRangesDefault;
	}

	const nkft_uint32 *NkFontAtlas::GetGlyphRangesLatinExtA() {
		return sRangesLatinExtA;
	}

	const nkft_uint32 *NkFontAtlas::GetGlyphRangesCyrillic() {
		return sRangesCyrillic;
	}

	const nkft_uint32 *NkFontAtlas::GetGlyphRangesGreek() {
		return sRangesGreek;
	}

	const nkft_uint32 *NkFontAtlas::GetGlyphRangesChineseFull() {
		return sRangesChineseFull;
	}
	...
	void NkFontAtlas::Clear() {
		...
		for (nkft_uint32 i = 0; i < fontCount; ++i) {
			...
			if (!p)
				continue;
			for (nkft_uint32 j = i; j < fontCount; ++j)
				if (fonts[j] == p)
					fonts[j] = nullptr;
	...
	NkFont *NkFontAtlas::AddFontFromFile(const char *path, nkft_float32 sp, const NkFontConfig *cfg) {
		{
			std::ifstream f(path);
			if (!f.good()) {
	...
	NkFont *NkFontAtlas::AddFontFromMemory(const nkft_uint8 *d, nkft_size ds, nkft_float32 sp,
	...
	NkFont *NkFontAtlas::AddFontFromMemoryOwned(nkft_uint8 *d, nkft_size ds, nkft_float32 sp,
		...
		if (cfg.oversampleH < 1)
			cfg.oversampleH = 2;
		if (cfg.oversampleV < 1)
			cfg.oversampleV = 1;
	...
	nkft_int32 NkFontAtlas::AddCustomRect(nkft_uint16 w, nkft_uint16 h) {
		if (customRectCount >= NK_FONT_ATLAS_MAX_CUSTOM_RECTS)
			return -1;
	...
	const NkFontAtlasCustomRect *NkFontAtlas::GetCustomRect(nkft_int32 idx) const {
		if (idx < 0 || (nkft_uint32)idx >= customRectCount)
			return nullptr;
	...
	bool NkFontAtlas::Build() {
		...
		for (nkft_uint32 ci = 0; ci < fontCount; ++ci) {
			...
			for (nkft_uint32 ri = 0; ranges[ri] && ranges[ri + 1]; ri += 2) {
				for (nkft_uint32 cp = ranges[ri]; cp <= ranges[ri + 1] && totalRects < MAX_RECTS; ++cp) {
					...
					if (gid == 0 && cp != cfg.glyphFallback && cp != 0x0020u)
						continue;
					nkft_int32 x0 = 0, y0 = 0, x1 = 0, y1 = 0;
					nkfont::NkGetGlyphBitmapBox(&fi, gid, sc * cfg.oversampleH, sc * cfg.oversampleV, 0, 0, &x0, &y0,
												&x1, &y1);
		...
		nkft_uint8 *sdfTemp = sdfMode ? (nkft_uint8 *)nkentseu::memory::NkAlloc((nk_size)(atlasW * atlasH)) : nullptr;
		if (sdfMode && sdfTemp)
			memset(sdfTemp, 0, (nkft_size)(atlasW * atlasH));
		...
		for (nkft_uint32 ri = 0; ri < crs; ++ri) {
			...
			nkfont::NkGetGlyphBitmapBox(&face, gid, sc * cfg.oversampleH, sc * cfg.oversampleV, 0, 0, &bx0, &by0, &bx1,
										&by1);
			...
			if (!isSpace && bw > 0 && bh > 0) {
				...
				nkfont::NkMakeGlyphBitmap(&face, dst, drawW, drawH, atlasW, sc * cfg.oversampleH, sc * cfg.oversampleV,
										  0, 0, gid);
			...
			if (advX < cfg.glyphMinAdvanceX)
				advX = cfg.glyphMinAdvanceX;
			if (advX > cfg.glyphMaxAdvanceX)
				advX = cfg.glyphMaxAdvanceX;
			if (cfg.pixelSnapH)
				advX = (nkft_float32)(nkft_int32)(advX + 0.5f);
		...
		for (nkft_uint32 i = 0; i < customRectCount; ++i) {
			...
			if (ri >= totalRects || !packRects[ri].packed)
				continue;
			const NkPackRect &r = packRects[ri];
	...
	void NkFontAtlas::GetTexDataAsAlpha8(nkft_uint8 **op, nkft_int32 *ow, nkft_int32 *oh, nkft_int32 *ob) {
	...
	void NkFontAtlas::GetTexDataAsRGBA32(nkft_uint8 **op, nkft_int32 *ow, nkft_int32 *oh, nkft_int32 *ob) {
		if (!texPixels) {
			if (op)
				*op = nullptr;
			return;
	...
	void NkFont::AddGlyph(const NkFontGlyph &g) {
		if (glyphCount >= NK_FONT_MAX_GLYPHS)
			return;
		...
		for (nkft_uint32 i = 0; i < sortedCount; ++i) {
			if (glyphSortedCodepoints[i] == g.codepoint)
				return;
			if (glyphSortedCodepoints[i] > g.codepoint)
				break; // tri croissant -> absent
		}
		glyphs[glyphCount] = g;
		AddGlyphSorted(g);
		++glyphCount;
	...
	void NkFont::AddGlyphSorted(const NkFontGlyph &g) {
		if (sortedCount >= NK_FONT_MAX_GLYPHS)
			return;
	...
	NkFontCodepoint NkFontDecodeUTF8(const char **str, const char *end) {
		...
		if (b0 < 0x80) {
			...
			*str = (const char *)(s + 2);
			...
			cp = ((nkft_uint32)(b0 & 0x0F) << 12) | ((nkft_uint32)(s[1] & 0x3F) << 6) | (s[2] & 0x3F);
			*str = (const char *)(s + 3);
			...
			cp = ((nkft_uint32)(b0 & 0x07) << 18) | ((nkft_uint32)(s[1] & 0x3F) << 12) |
				 ((nkft_uint32)(s[2] & 0x3F) << 6) | (s[3] & 0x3F);
			*str = (const char *)(s + 4);
		} else {
			++*str;
			return NKFONT_CODEPOINT_REPLACEMENT;
	...
	bool NkFont::GetGlyphOutlinePoints(NkFontCodepoint cp, NkVector<NkFontOutlineVertex> &outVertices) const {
		...
		if (!nkfont::NkGetGlyphShape(m_FaceInfo, gid, &vb))
			return false;
