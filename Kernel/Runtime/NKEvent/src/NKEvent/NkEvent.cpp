...
namespace nkentseu {
	...
	NkEvent::NkEvent(uint64 windowId) : mWindowID(windowId), mTimestamp(GetCurrentTimestamp()), mHandled(false) {
	}
	...
	NkEvent::NkEvent() : mWindowID(0), mTimestamp(GetCurrentTimestamp()), mHandled(false) {
	}
	...
	NkEvent::NkEvent(const NkEvent &other)
		: mWindowID(other.mWindowID), mTimestamp(other.mTimestamp), mHandled(other.mHandled) {
	}
