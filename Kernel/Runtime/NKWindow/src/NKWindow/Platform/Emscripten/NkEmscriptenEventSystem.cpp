...
namespace nkentseu {
	...
	template <typename Fn> static void ForEachWASMWindow(Fn &&fn) {
	...
	static CanvasCoords MapCssToCanvas(const char *canvasSelector, int32 cssX, int32 cssY) {
		...
		if (emscripten_get_canvas_element_size(canvasSelector, &canvasWidth, &canvasHeight) !=
		...
		EM_ASM(
			{
				...
				target.__nkDropHandler = function(ev) {
					...
					if (hasCcall && ev.dataTransfer) {
						...
						if (text.length > 0) {
							Module.ccall("NkEmscriptenOnDropText", null, [ "string", "number", "number" ],
										 [ text, ev.clientX, ev.clientY ]);
						}
						...
						if (names.length > 0) {
							Module.ccall("NkEmscriptenOnDropFiles", null, [ "string", "number", "number" ],
	...
	EMSCRIPTEN_KEEPALIVE EM_BOOL OnMouseMove(int, const EmscriptenMouseEvent *me, void *) {
		...
		const CanvasCoords mapped =
			MapCssToCanvas(CurrentCanvasSelector(), static_cast<int32>(me->targetX), static_cast<int32>(me->targetY));
		...
		NkMouseMoveEvent event(mapped.x, mapped.y, static_cast<int32>(me->screenX), static_cast<int32>(me->screenY), dx,
							   dy);
		gWASMEventSystem->Enqueue_Public(event, targetWindowId);
	...
	EMSCRIPTEN_KEEPALIVE EM_BOOL OnMouseDown(int, const EmscriptenMouseEvent *me, void *) {
		...
		const NkMouseButton button = (me->button == 2)	 ? NkMouseButton::NK_MB_RIGHT
									 : (me->button == 1) ? NkMouseButton::NK_MB_MIDDLE
														 : NkMouseButton::NK_MB_LEFT;

		const CanvasCoords mapped =
			MapCssToCanvas(CurrentCanvasSelector(), static_cast<int32>(me->targetX), static_cast<int32>(me->targetY));
		NkMouseButtonPressEvent event(button, mapped.x, mapped.y, static_cast<int32>(me->screenX),
	...
	EMSCRIPTEN_KEEPALIVE EM_BOOL OnMouseUp(int, const EmscriptenMouseEvent *me, void *) {
		...
		const CanvasCoords mapped =
			MapCssToCanvas(CurrentCanvasSelector(), static_cast<int32>(me->targetX), static_cast<int32>(me->targetY));
		NkMouseButtonReleaseEvent event(button, mapped.x, mapped.y, static_cast<int32>(me->screenX),
										static_cast<int32>(me->screenY));
		gWASMEventSystem->Enqueue_Public(event, targetWindowId);
	...
	EMSCRIPTEN_KEEPALIVE void PushTouchEvents(const EmscriptenTouchEvent *te, NkTouchPhase phase) {
		...
		for (int i = 0; i < te->numTouches && count < NK_MAX_TOUCH_POINTS; ++i) {
			...
			const CanvasCoords mapped =
				MapCssToCanvas(CurrentCanvasSelector(), static_cast<int32>(tp.targetX), static_cast<int32>(tp.targetY));
			NkTouchPoint point;
	...
	EMSCRIPTEN_KEEPALIVE EM_BOOL OnCanvasResize(int, const EmscriptenUiEvent *, void *) {
		...
		ForEachWASMWindow([&](NkWindow &window, NkWindowId id) {
			...
			NkWindowResizeEvent event(window.mData.mWidth, window.mData.mHeight, previousWidth, previousHeight);
...
EMSCRIPTEN_KEEPALIVE void NkEmscriptenOnDragEnter(int x, int y, int numItems, int hasText, int hasImage) {
	...
	nkentseu::NkEmscriptenDropTarget::DispatchDragEnter(target, static_cast<float>(x), static_cast<float>(y),
														static_cast<nkentseu::uint32>(numItems < 0 ? 0 : numItems),
														hasText != 0, hasImage != 0);
}
...
EMSCRIPTEN_KEEPALIVE void NkEmscriptenOnDropText(const char *text, int x, int y) {
	...
	nkentseu::NkEmscriptenDropTarget::DispatchDropText(target, static_cast<float>(x), static_cast<float>(y),
...
EMSCRIPTEN_KEEPALIVE void NkEmscriptenOnDropFiles(const char *names, int x, int y) {
	...
	nkentseu::NkEmscriptenDropTarget::DispatchDropFiles(target, static_cast<float>(x), static_cast<float>(y),
