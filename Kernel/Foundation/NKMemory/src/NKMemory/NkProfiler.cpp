...
namespace nkentseu {
	namespace memory {
...

			{
				...
				if (sStats.totalAllocations > 0) {
					sStats.avgAllocationSize =
						static_cast<float32>(::gTotalAllocatedBytes) / static_cast<float32>(sStats.totalAllocations);
				}
			}
			// Lock relâché ici
		...
		void NkMemoryProfiler::DumpProfilerStats() noexcept {
			...
			NK_FOUNDATION_LOG_INFO("Memory: live=%s, peak=%s, avg=%.2f bytes", FormatBytes(stats.liveBytes),
								   FormatBytes(stats.peakBytes), static_cast<double>(stats.avgAllocationSize));
