...
namespace {
	...
	struct GlobalConfig {
			...
			constexpr GlobalConfig() noexcept : alertsEnabled(true), warningThreshold(0.9f) {
			}
	};
	...
	const char *TagName(nkentseu::memory::NkMemoryTag tag) {
		...
		switch (tag) {
			case NkMemoryTag::NK_MEMORY_ENGINE:
				return "ENGINE";
			case NkMemoryTag::NK_MEMORY_CONTAINER:
				return "CONTAINER";
			case NkMemoryTag::NK_MEMORY_ALLOCATOR:
				return "ALLOCATOR";
			case NkMemoryTag::NK_MEMORY_GAME:
				return "GAME";
			case NkMemoryTag::NK_MEMORY_ENTITY:
				return "ENTITY";
			case NkMemoryTag::NK_MEMORY_SCRIPT:
				return "SCRIPT";
			case NkMemoryTag::NK_MEMORY_RENDER:
				return "RENDER";
			case NkMemoryTag::NK_MEMORY_TEXTURE:
				return "TEXTURE";
			case NkMemoryTag::NK_MEMORY_MESH:
				return "MESH";
			case NkMemoryTag::NK_MEMORY_SHADER:
				return "SHADER";
			case NkMemoryTag::NK_MEMORY_PHYSICS:
				return "PHYSICS";
			case NkMemoryTag::NK_MEMORY_COLLISION:
				return "COLLISION";
			case NkMemoryTag::NK_MEMORY_RIGID:
				return "RIGID";
			case NkMemoryTag::NK_MEMORY_AUDIO:
				return "AUDIO";
			case NkMemoryTag::NK_MEMORY_SOUND:
				return "SOUND";
			case NkMemoryTag::NK_MEMORY_STREAMING:
				return "STREAMING";
			case NkMemoryTag::NK_MEMORY_NETWORK:
				return "NETWORK";
			case NkMemoryTag::NK_MEMORY_DEBUG:
				return "DEBUG";
			default:
				return "UNKNOWN";
		}
...
// Vérification de budget et alertes (seulement si configuré)
			...
			if (gConfig.alertsEnabled && state.budget > 0) {
				...
				if (usage >= gConfig.warningThreshold && usage < 1.0f) {
					NK_FOUNDATION_LOG_WARN("[NKMemory] Tag %s at %.1f%% of budget (%llu/%llu bytes)", TagName(tag),
										   usage * 100.0f, static_cast<unsigned long long>(current),
										   static_cast<unsigned long long>(state.budget));
				}

				// Alerte critique si dépassement
				if (usage >= 1.0f) {
					NK_FOUNDATION_LOG_ERROR("[NKMemory] Tag %s OVER BUDGET: %.1f%% (%llu/%llu bytes)", TagName(tag),
											usage * 100.0f, static_cast<unsigned long long>(current),
											static_cast<unsigned long long>(state.budget));
		...
		NkMemoryTagStats NkMemoryBudget::GetStats(NkMemoryTag tag) noexcept {
			...
			if (stats.allocationCount > 0) {
				stats.averageSize =
					static_cast<float32>(stats.totalAllocated) / static_cast<float32>(stats.allocationCount);
			}
		...
		void NkMemoryBudget::DumpStats() noexcept {
			...
			NK_FOUNDATION_LOG_INFO("%-14s | %-10s | %-10s | %-6s | %-10s", "Tag", "Used", "Budget", "%", "Peak");
			...
			constexpr NkMemoryTag displayedTags[] = {
				NkMemoryTag::NK_MEMORY_ENGINE,	  NkMemoryTag::NK_MEMORY_CONTAINER, NkMemoryTag::NK_MEMORY_ALLOCATOR,
				NkMemoryTag::NK_MEMORY_GAME,	  NkMemoryTag::NK_MEMORY_ENTITY,	NkMemoryTag::NK_MEMORY_SCRIPT,
				NkMemoryTag::NK_MEMORY_RENDER,	  NkMemoryTag::NK_MEMORY_TEXTURE,	NkMemoryTag::NK_MEMORY_MESH,
				NkMemoryTag::NK_MEMORY_SHADER,	  NkMemoryTag::NK_MEMORY_PHYSICS,	NkMemoryTag::NK_MEMORY_COLLISION,
				NkMemoryTag::NK_MEMORY_RIGID,	  NkMemoryTag::NK_MEMORY_AUDIO,		NkMemoryTag::NK_MEMORY_SOUND,
				NkMemoryTag::NK_MEMORY_STREAMING, NkMemoryTag::NK_MEMORY_NETWORK,	NkMemoryTag::NK_MEMORY_DEBUG,
			};
			...
			for (NkMemoryTag tag : displayedTags) {
				...
				NK_FOUNDATION_LOG_INFO("%-14s | %-10s | %-10s | %-6s | %-10s", TagName(tag),
									   FormatSize(used, usedBuf, sizeof(usedBuf)),
									   (budget > 0) ? FormatSize(budget, budgetBuf, sizeof(budgetBuf)) : "unlimited",
									   percentStr, FormatSize(peak, peakBuf, sizeof(peakBuf)));
			}
		...
		void NkMemoryBudget::SetBudgetWarningThreshold(float32 threshold) noexcept {
			...
			gConfig.warningThreshold = (threshold < 0.0f) ? 0.0f : (threshold > 1.0f) ? 1.0f : threshold;
		}
		...
		void *TaggedAlloc(nk_size bytes, NkMemoryTag tag, NkAllocator *allocator, nk_size alignment) {
			if (bytes == 0) {
				return nullptr;
			}
		...
		void *TaggedCalloc(nk_size count, nk_size elementSize, NkMemoryTag tag, NkAllocator *allocator,
						   nk_size alignment) {
...
namespace nkentseu {
	namespace memory {
		...
		nk_uint64 NkMemoryBudget::GetBudget(NkMemoryTag) noexcept {
			return 0;
		}
		...
		nk_int64 NkMemoryBudget::GetBudgetRemaining(NkMemoryTag) noexcept {
			return INT64_MAX;
		}

		nk_bool NkMemoryBudget::IsOverBudget(NkMemoryTag) noexcept {
			return false;
		}

		NkMemoryTagStats NkMemoryBudget::GetStats(NkMemoryTag) noexcept {
		...
		void *TaggedCalloc(nk_size count, nk_size elementSize, NkMemoryTag, NkAllocator *allocator, nk_size alignment) {
