...
#if defined(NKENTSEU_MEMORY_TRACKING_VERBOSE) && defined(NKENTSEU_DEBUG)
			NK_FOUNDATION_LOG_DEBUG("[NKMemory] Registered: %p size=%zu tag=%u at %s:%d", info.userPtr, info.size,
									static_cast<unsigned>(info.tag), info.file ? info.file : "<unknown>", info.line);
#endif
		}
...
// Comportement : no-op silencieux (pas d'erreur)
		...
		void NkMemoryTracker::DumpLeaks() const noexcept {
			...
			NK_FOUNDATION_LOG_INFO("%-12s | %-10s | %-6s | %-30s | %s", "Address", "Size", "Tag", "Location", "Name");
			...
			for (nk_size bucket = 0; bucket < NK_TRACKER_BUCKET_COUNT; ++bucket) {
				...
				while (node) {
					...
					NK_FOUNDATION_LOG_INFO("%-12p | %-10s | %-6u | %-30s | %s", info.userPtr, sizeBuf,
										   static_cast<unsigned>(info.tag), locationBuf, info.name ? info.name : "-");
			...
			NK_FOUNDATION_LOG_INFO(
				"-------------|------------|--------|--------------------------------|----------------");
			NK_FOUNDATION_LOG_INFO("TOTAL        | %-10zu |        | %-30s | %zu leaks", totalLeaked, "",
								   mLiveAllocations);
		...
		NkMemoryTracker::Stats NkMemoryTracker::GetStats() const noexcept {
			NkSpinLockGuard guard(mLock);
			return {mTotalLiveBytes, mPeakBytes, mLiveAllocations, mTotalAllocations};
		}
...
namespace nkentseu {
	namespace memory {
		...
		const NkAllocationInfo *NkMemoryTracker::Find(void *) const noexcept {
			return nullptr;
		}
		...
		nk_size NkMemoryTracker::HashPtr(const void *) noexcept {
			return 0;
		}

		NkMemoryTracker::Entry *NkMemoryTracker::FindEntry(void *) noexcept {
			return nullptr;
		}

		const NkMemoryTracker::Entry *NkMemoryTracker::FindEntryConst(const void *) const noexcept {
			return nullptr;
		}
