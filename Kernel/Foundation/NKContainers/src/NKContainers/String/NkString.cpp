...
namespace nkentseu {
	 ...
	 */
	NkString::NkString() : mAllocator(&memory::NkGetDefaultAllocator()), mLength(0), mCapacity(SSO_SIZE) {
		mSSOData[0] = '\0';
		SetSSO(true);
	}
	 ...
	 */
	NkString::NkString(memory::NkIAllocator &allocator) : mAllocator(&allocator), mLength(0), mCapacity(SSO_SIZE) {
		mSSOData[0] = '\0';
		SetSSO(true);
	}
	...
	NkString::NkString(const char *str)
		: mAllocator(&memory::NkGetDefaultAllocator()), mLength(0), mCapacity(SSO_SIZE) {
		mSSOData[0] = '\0';
		SetSSO(true);
		if (str) {
	 ...
	 */
	NkString::NkString(const char *str, memory::NkIAllocator &allocator) : NkString(allocator) {
		if (str) {
			Append(str);
		}
	}
	 ...
	 */
	NkString::NkString(const char *str, SizeType length, memory::NkIAllocator &allocator) : NkString(allocator) {
		if (str && length > 0) {
			Append(str, length);
		}
	}
	 ...
	 */
	NkString::NkString(NkStringView view) : NkString(view.Data(), view.Length()) {
	}
	 ...
	 */
	NkString::NkString(SizeType count, char ch, memory::NkIAllocator &allocator) : NkString(allocator) {
		Append(count, ch);
	}
	...
	NkString::NkString(const NkString &other, memory::NkIAllocator &allocator)
		: mAllocator(&allocator), mLength(0), mCapacity(SSO_SIZE) {
		mSSOData[0] = '\0';
		SetSSO(true);
		Append(other);
	}
	...
	void NkString::Reserve(SizeType newCapacity) {
		...
		if (IsSSO()) {
			...
			char *newData = static_cast<char *>(mAllocator->Allocate((newCapacity + 1) * sizeof(char)));
	...
	void NkString::ShrinkToFit() {
		...
		if (mLength <= SSO_SIZE) {
			...
			char *newData = static_cast<char *>(mAllocator->Allocate((mLength + 1) * sizeof(char)));
			memory::NkMemCopy(newData, mHeapData, mLength + 1);
			mAllocator->Deallocate(mHeapData);
			mHeapData = newData;
			mCapacity = mLength;
	...
	NkString &NkString::RemoveChars(NkStringView charsToRemove) {
		...
		for (SizeType r = 0; r < mLength; ++r) {
			...
			for (SizeType j = 0; j < charsToRemove.Length(); ++j) {
				if (data[r] == charsToRemove[j]) {
					remove = true;
					break;
				}
			}
			if (!remove)
				data[writePos++] = data[r];
		}
		mLength = writePos;
		data[mLength] = '\0';
		return *this;
	...
	bool NkString::IsDigits() const noexcept {
		return string::NkIsAllDigits(View());
	}

	bool NkString::IsAlpha() const noexcept {
		return string::NkIsAllAlpha(View());
	}

	bool NkString::IsAlphaNumeric() const noexcept {
		return string::NkIsAllAlphaNumeric(View());
	}

	bool NkString::IsWhitespace() const noexcept {
		return string::NkIsAllWhitespace(View());
	}

	bool NkString::IsNumeric() const noexcept {
		return string::NkIsNumeric(View());
	}

	bool NkString::IsInteger() const noexcept {
