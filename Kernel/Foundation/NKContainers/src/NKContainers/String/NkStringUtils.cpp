...
namespace nkentseu {
	...
	namespace string {
		...
		bool NkEndsWith(NkStringView str, NkStringView suffix) {
			...
			return memory::NkMemCompare(str.Data() + str.Length() - suffix.Length(), suffix.Data(), suffix.Length()) ==
				   0;
		}
		...
		bool NkEndsWithIgnoreCase(NkStringView str, NkStringView suffix) {
			...
			return NkCompareIgnoreCase(str.SubStr(str.Length() - suffix.Length()), suffix) == 0;
		}
				...
				if (str[i] == '\\' && i + 1 < str.Length()) {
					...
					switch (str[i]) {
						...
						case '7': {
							...
							result.Append(static_cast<char>(std::strtoul(oct, nullptr, 8)));
							break;
						}
						case 'x':
							if (i + 2 < str.Length()) {
								...
								i += 2;
								result.Append(static_cast<char>(std::strtoul(hex, nullptr, 16)));
							}
							break;
						default:
							result.Append(str[i]);
		...
		NkStringView NkGetFileName(NkStringView path) {
			...
			usize lastSeparator = (lastSlash == NkStringView::npos)		  ? lastBackslash
								  ...
								  : (lastSlash > lastBackslash)			  ? lastSlash
																		  : lastBackslash;
		...
		NkString NkRemoveComments(NkStringView str, NkStringView commentMarkers) {
			...
			for (usize i = 0; i < str.Length(); ++i) {
				...
				for (usize j = 0; j < commentMarkers.Length(); ++j) {
					if (str[i] == commentMarkers[j] && i + 1 < str.Length() && str[i] == str[i + 1]) {
		...
		bool NkIsURL(NkStringView str) {
			...
			return NkStartsWithIgnoreCase(trimmed, "http://") || NkStartsWithIgnoreCase(trimmed, "https://") ||
				   NkStartsWithIgnoreCase(trimmed, "ftp://") || NkStartsWithIgnoreCase(trimmed, "file://");
		}
