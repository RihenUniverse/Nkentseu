...
namespace nkentseu {
	...
	NkStringBuilder::SizeType NkStringBuilder::UIntToString(uint64 value, char *buffer, SizeType bufferSize) {
		int len = std::snprintf(buffer, bufferSize, "%llu", static_cast<unsigned long long>(value));
		return len >= 0 ? static_cast<SizeType>(len) : 0;
	}
	 ...
	 */
	NkStringBuilder::SizeType NkStringBuilder::FloatToString(float64 value, char *buffer, SizeType bufferSize,
															 int precision) {
		char format[16];
	 ...
	 */
	NkStringBuilder::SizeType NkStringBuilder::HexToString(uint64 value, char *buffer, SizeType bufferSize,
														   bool prefix) {
		if (prefix) {
			int len = std::snprintf(buffer, bufferSize, "0x%llX", static_cast<unsigned long long>(value));
			return len >= 0 ? static_cast<SizeType>(len) : 0;
		} else {
			int len = std::snprintf(buffer, bufferSize, "%llX", static_cast<unsigned long long>(value));
			return len >= 0 ? static_cast<SizeType>(len) : 0;
		}
	}
	 ...
	 */
	NkStringBuilder::SizeType NkStringBuilder::BinaryToString(uint64 value, char *buffer, SizeType bufferSize,
															  SizeType bits) {
		if (bits > 64) {
			bits = 64;
		}
	 ...
	 */
	NkStringBuilder::SizeType NkStringBuilder::OctalToString(uint64 value, char *buffer, SizeType bufferSize,
															 bool prefix) {
		if (prefix) {
			int len = std::snprintf(buffer, bufferSize, "0%llo", static_cast<unsigned long long>(value));
			return len >= 0 ? static_cast<SizeType>(len) : 0;
		} else {
			int len = std::snprintf(buffer, bufferSize, "%llo", static_cast<unsigned long long>(value));
			return len >= 0 ? static_cast<SizeType>(len) : 0;
		}
	}
	...
	void NkStringBuilder::Reallocate(SizeType newCapacity) {
		...
		char *newBuffer = static_cast<char *>(mAllocator->Allocate((newCapacity + 1) * sizeof(char)));
		if (mBuffer) {
			if (mLength > 0) {
	...
	NkStringBuilder::NkStringBuilder()
		: mAllocator(&memory::NkGetDefaultAllocator()), mBuffer(nullptr), mLength(0), mCapacity(0) {
	...
	NkStringBuilder::NkStringBuilder(memory::NkIAllocator &allocator)
		: mAllocator(&allocator), mBuffer(nullptr), mLength(0), mCapacity(0) {
		Reserve(DEFAULT_CAPACITY);
	}
	...
	NkStringBuilder &NkStringBuilder::RemoveAll(NkStringView str) {
		...
		while (readPos < mLength) {
			if (readPos + strLen <= mLength && memory::NkMemCompare(mBuffer + readPos, str.Data(), strLen) == 0) {
				readPos += strLen;
			} else {
				mBuffer[writePos++] = mBuffer[readPos++];
			}
	...
	void NkStringBuilder::ShrinkToFit() {
		...
		char *newBuffer = static_cast<char *>(mAllocator->Allocate((mLength + 1) * sizeof(char)));
		if (mBuffer) {
			if (mLength > 0) {
				memory::NkMemCopy(newBuffer, mBuffer, mLength);
			}
	...
	NkStringView NkStringBuilder::SubStr(SizeType pos, SizeType count) const {
		NKENTSEU_ASSERT(pos <= mLength);
		SizeType rcount = (count == NkStringView::npos || pos + count > mLength) ? mLength - pos : count;
	 ...
	 */
	NkStringBuilder::SizeType NkStringBuilder::Find(char ch, SizeType pos) const noexcept {
		if (pos >= mLength) {
	 ...
	 */
	NkStringBuilder::SizeType NkStringBuilder::Find(NkStringView str, SizeType pos) const noexcept {
		if (str.Empty()) {
			return pos;
		}
		if (pos > mLength) {
	 ...
	 */
	NkStringBuilder::SizeType NkStringBuilder::FindLast(char ch, SizeType pos) const noexcept {
		if (mLength == 0) {
			return NkStringView::npos;
		}
	 ...
	 * @return Index de la dernière occurrence, ou npos si non trouvé
	 */
	NkStringBuilder::SizeType NkStringBuilder::FindLast(NkStringView str, SizeType pos) const noexcept {
		if (str.Empty()) {
		...
		SizeType searchPos =
			(pos == NkStringView::npos || pos >= mLength - str.Length()) ? mLength - str.Length() : pos;
		for (SizeType i = searchPos + 1; i > 0; --i) {
			SizeType idx = i - 1;
