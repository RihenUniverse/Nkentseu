...
namespace nkentseu {
	namespace string {
		...
		// -----------------------------------------------------------------
		template <typename first, typename second> struct NkStringPair {
				first First;
				second Second;
		...
		uint32 NkHashXX32(const char *str, usize length, uint32 seed) NKENTSEU_NOEXCEPT {
			...
			if (length >= 16) {
				...
				h32 = NkMurmurRotl32(v1, 1) + NkMurmurRotl32(v2, 7) + NkMurmurRotl32(v3, 12) + NkMurmurRotl32(v4, 18);
			} else {
				h32 = seed + PRIME32_5;
			}
		...
		uint64 NkHashXX64(const char *str, usize length, uint64 seed) NKENTSEU_NOEXCEPT {
			...
			if (length >= 32) {
				...
				h64 = NkMurmurRotl64(v1, 1) + NkMurmurRotl64(v2, 7) + NkMurmurRotl64(v3, 12) + NkMurmurRotl64(v4, 18);
		...
		void NkHashSpooky128(const char *str, usize length, uint64 out[2], uint64 seed1,
							 uint64 seed2) NKENTSEU_NOEXCEPT {
			if (!str || length == 0) {
				out[0] = seed1;
				out[1] = seed2;
				return;
