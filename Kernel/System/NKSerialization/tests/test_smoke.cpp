...
static void TestArchiveFlat() {
	...
	NkString name;
	EXPECT_TRUE(a.GetString("name", name));
	EXPECT_STR(name.CStr(), "Nkentseu");
	nk_uint64 build;
	EXPECT_TRUE(a.GetUInt64("build", build));
	EXPECT_EQ(build, 20260413ull);
	nk_bool debug;
	EXPECT_TRUE(a.GetBool("debug", debug));
	EXPECT_TRUE(debug);
	nk_float64 sc;
	EXPECT_TRUE(a.GetFloat64("scale", sc));
	EXPECT_TRUE(sc > 1.2 && sc < 1.3);
	nk_int64 delta;
	EXPECT_TRUE(a.GetInt64("delta", delta));
	EXPECT_EQ(delta, -99LL);
...
static void TestArchiveHierarchical() {
	...
	NkString x;
	EXPECT_TRUE(out.GetString("x", x));
	EXPECT_STR(x.CStr(), "hello");
	nk_int64 y;
	EXPECT_TRUE(out.GetInt64("y", y));
	EXPECT_EQ(y, 42LL);
	...
	nk_int64 v1 = 0;
	EXPECT_TRUE(NkString(got[0].text).ToInt64(v1));
	EXPECT_EQ(v1, 1LL);
}
...
static void TestJSONRoundTrip() {
	...
	NkString name;
	dst.GetString("name", name);
	EXPECT_STR(name.CStr(), "Nkentseu");
	nk_uint64 b = 0;
	dst.GetUInt64("build", b);
	EXPECT_EQ(b, 20260413ull);
	nk_bool ok = false;
	dst.GetBool("ok", ok);
	EXPECT_TRUE(ok);
	nk_float64 pi = 0;
	dst.GetFloat64("pi", pi);
	EXPECT_TRUE(pi > 3.1415 && pi < 3.1416);
}
...
static void TestJSONNested() {
	...
	NkString top;
	dst.GetString("top", top);
	EXPECT_STR(top.CStr(), "level");
	NkArchive dstInner;
	EXPECT_TRUE(dst.GetObject("inner", dstInner));
	NkString key;
	dstInner.GetString("key", key);
	EXPECT_STR(key.CStr(), "value");
	nk_int64 num = 0;
	dstInner.GetInt64("num", num);
	EXPECT_EQ(num, 777LL);
}
...
static void TestXMLRoundTrip() {
	...
	NkString title;
	dst.GetString("title", title);
	EXPECT_STR(title.CStr(), "Test XML");
	nk_int64 v = 0;
	dst.GetInt64("value", v);
	EXPECT_EQ(v, -123LL);
}
...
static void TestYAMLRoundTrip() {
	...
	NkString name;
	dst.GetString("name", name);
	EXPECT_STR(name.CStr(), "engine");
	nk_uint64 v = 0;
	dst.GetUInt64("version", v);
	EXPECT_EQ(v, 2026ull);
	nk_bool rel = true;
	dst.GetBool("release", rel);
	EXPECT_TRUE(!rel);
}
...
static void TestBinaryRoundTrip() {
	...
	NkString b;
	dst.GetString("b", b);
	EXPECT_STR(b.CStr(), "hello binary");
}
...
static void TestNativeRoundTrip() {
	...
	nk_bool flag = false;
	dst.GetBool("flag", flag);
	EXPECT_TRUE(flag);
	nk_int64 cnt = 0;
	dst.GetInt64("count", cnt);
	EXPECT_EQ(cnt, 1234567890LL);
	nk_float64 ratio = 0;
	dst.GetFloat64("ratio", ratio);
	EXPECT_TRUE(ratio > 0.12 && ratio < 0.13);
	NkString lbl;
	dst.GetString("label", lbl);
	EXPECT_STR(lbl.CStr(), "native test");
	nk_uint64 big = 0;
	dst.GetUInt64("big", big);
	EXPECT_EQ(big, 0xDEADBEEFCAFEBABEull);
}
...
static void TestNativeNested() {
	...
	NkString rk;
	out.GetString("root_key", rk);
	EXPECT_STR(rk.CStr(), "root_val");
	NkArchive childOut;
	EXPECT_TRUE(out.GetObject("child", childOut));
	NkString cn;
	childOut.GetString("name", cn);
	EXPECT_STR(cn.CStr(), "child_node");
	nk_uint64 cid = 0;
	childOut.GetUInt64("id", cid);
	EXPECT_EQ(cid, 42ull);
}
...
static void TestArchiveMerge() {
	...
	a.Merge(b, false);
	NkString shared;
	a.GetString("shared", shared);
	EXPECT_STR(shared.CStr(), "from_a");
	nk_int64 ob = 0;
	a.GetInt64("only_b", ob);
	EXPECT_EQ(ob, 2LL);

	// Merge avec overwrite=true : shared devient "from_b"
	a.Merge(b, true);
	a.GetString("shared", shared);
	EXPECT_STR(shared.CStr(), "from_b");
}
