...
namespace nkentseu {
	...
	struct NKENTSEU_SERIALIZATION_API NkArchiveValue {
			...
			[[nodiscard]] bool IsNull() const noexcept {
				return type == NkArchiveValueType::NK_VALUE_NULL;
			}

			/// @brief Vérifie si la valeur est booléenne
			/// @return true si type == NK_VALUE_BOOL
			[[nodiscard]] bool IsBool() const noexcept {
				return type == NkArchiveValueType::NK_VALUE_BOOL;
			}
	...
	struct NKENTSEU_SERIALIZATION_API NkArchiveNode {
			 ...
			 */
			explicit NkArchiveNode(const NkArchiveValue &v) noexcept : kind(NkNodeKind::NK_NODE_SCALAR), value(v) {
			}
...
#undef GetObject
			...
			[[nodiscard]] const NkVector<NkArchiveEntry> &Entries() const noexcept {
				return mEntries;
			}
			...
			[[nodiscard]] NkVector<NkArchiveEntry> &Entries() noexcept {
				return mEntries;
			}
