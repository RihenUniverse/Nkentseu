...
namespace nkentseu {
	...
	template <typename T> [[nodiscard]] constexpr NkTypeId NkTypeOf() noexcept {
		...
		return static_cast<NkTypeId>(reinterpret_cast<nk_size>(&s_tag));
	}
	...
	struct NkSchemaVersion {
			...
			[[nodiscard]] constexpr nk_uint64 Pack() const noexcept {
				return (static_cast<nk_uint64>(major) << 32u) | (static_cast<nk_uint64>(minor) << 16u) |
			...
			[[nodiscard]] static constexpr NkSchemaVersion Unpack(nk_uint64 v) noexcept {
				return NkSchemaVersion(static_cast<nk_uint16>((v >> 32u) & 0xFFFFu),
			...
			[[nodiscard]] NkString ToString() const noexcept {
				return NkString::Fmtf("%u.%u.%u", static_cast<unsigned>(major), static_cast<unsigned>(minor),
	 ...
	 */
	template <typename ReturnType> struct NkFieldMigrationAdapter {
			// -----------------------------------------------------------------
			 ...
			 */
			static nk_bool Migrate(NkArchive &archive, NkSchemaVersion /*from*/, NkSchemaVersion /*to*/) noexcept {
	...
	template <typename ReturnType> NkFunction<ReturnType()> NkFieldMigrationAdapter<ReturnType>::s_getDefault;

	template <typename ReturnType> char NkFieldMigrationAdapter<ReturnType>::s_fieldName[128] = {};
	...
	class NKENTSEU_SERIALIZATION_CLASS_EXPORT NkSchemaRegistry {
		...
		public:
			 ...
			 * @endcode
			 */
			static void RegisterMigration(NkTypeId typeId, NkSchemaVersion from, NkSchemaVersion to,
			 ...
			 * @endcode
			 */
			[[nodiscard]] static nk_bool MigrateArchive(NkTypeId typeId, NkArchive &archive, NkSchemaVersion stored,
				...
				for (int guard = 0; guard < 256 && cursor != current; ++guard) {
					...
					for (nk_size i = 0; i < reg.mMigrations.Size(); ++i) {
						...
						if (me.typeId == typeId && me.migration.from == cursor) {
							...
							if (!me.migration.Apply(archive)) {
								if (err) {
									*err = NkString::Fmtf("Migration %s → %s failed", cursor.ToString().CStr(),
					...
					if (!applied) {
						if (err) {
							*err = NkString::Fmtf("No migration path from %s to %s", cursor.ToString().CStr(),
			...
			static void AddFieldMigration(NkTypeId typeId, NkSchemaVersion from, NkSchemaVersion to,
										  const char *fieldName, DefaultFn getDefault) noexcept {
...
#define NK_SCHEMA_CURRENT_VERSION(Type, Major, Minor, Patch)                                                           \
	namespace {                                                                                                        \
		struct _NkSchVer_##Type {                                                                                      \
				_NkSchVer_##Type() noexcept {                                                                          \
					::nkentseu::NkSchemaRegistry::SetCurrentVersion(::nkentseu::NkTypeOf<Type>(),                      \
																	{Major, Minor, Patch});                            \
				}                                                                                                      \
		} _sNkSchVer_##Type;                                                                                           \
	}
 ...
 * @endcode
 */
#define NK_REGISTER_MIGRATION(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FnPtr)                             \
	namespace {                                                                                                        \
		struct _NkMig_##Type##_##FromMaj##_##ToMaj {                                                                   \
				_NkMig_##Type##_##FromMaj##_##ToMaj() noexcept {                                                       \
					::nkentseu::NkSchemaRegistry::RegisterMigration(                                                   \
						::nkentseu::NkTypeOf<Type>(), {FromMaj, FromMin, FromPat}, {ToMaj, ToMin, ToPat}, FnPtr);      \
				}                                                                                                      \
 ...
 * @endcode
 */
#define NK_ADD_FIELD_MIGRATION_BOOL(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName, DefaultValue)     \
	NK_SCHEMA_ADD_FIELD_MIGRATION_IMPL(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName,                \
 ...
 * @ingroup SchemaMacros
 */
#define NK_ADD_FIELD_MIGRATION_INT32(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName, DefaultValue)    \
	NK_SCHEMA_ADD_FIELD_MIGRATION_IMPL(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName,                \
 ...
 * @ingroup SchemaMacros
 */
#define NK_ADD_FIELD_MIGRATION_FLOAT64(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName, DefaultValue)  \
	NK_SCHEMA_ADD_FIELD_MIGRATION_IMPL(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName,                \
 ...
 * @ingroup SchemaMacros
 */
#define NK_ADD_FIELD_MIGRATION_STRING(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName, DefaultValue)   \
	NK_SCHEMA_ADD_FIELD_MIGRATION_IMPL(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName,                \
 ...
 */
#define NK_SCHEMA_ADD_FIELD_MIGRATION_IMPL(Type, FromMaj, FromMin, FromPat, ToMaj, ToMin, ToPat, FieldName,            \
										   GetterLambda)                                                               \
	namespace {                                                                                                        \
		struct _NkFieldMig_##Type##_##FromMaj##_##ToMaj##_##__LINE__ {                                                 \
				_NkFieldMig_##Type##_##FromMaj##_##ToMaj##_##__LINE__() noexcept {                                     \
					::nkentseu::NkSchemaRegistry::AddFieldMigration(::nkentseu::NkTypeOf<Type>(),                      \
