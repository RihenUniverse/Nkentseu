...
namespace {
	...
	const char *MethodToString(NkHTTPMethod method) noexcept {
		switch (method) {
			case NkHTTPMethod::NK_HTTP_GET:
				return "GET";
			case NkHTTPMethod::NK_HTTP_POST:
				return "POST";
			case NkHTTPMethod::NK_HTTP_PUT:
				return "PUT";
			case NkHTTPMethod::NK_HTTP_PATCH:
				return "PATCH";
			case NkHTTPMethod::NK_HTTP_DELETE:
				return "DELETE";
			case NkHTTPMethod::NK_HTTP_HEAD:
				return "HEAD";
			default:
				return "GET";
		}
...
// =====================================================================
	...
	static void NkCloseSocket(NkNativeSocket s) {
		closesocket(s);
	}
#elif defined(NKENTSEU_PLATFORM_POSIX)
...
// Mode non-bloquant
			...
			if (select(static_cast<int>(sock + 1), nullptr, &writeSet, nullptr, &tv) > 0) {
				...
				if (getsockopt(sock, SOL_SOCKET, SO_ERROR, reinterpret_cast<char *>(&soError), &len) == 0 &&
					soError == 0) {
...
namespace nkentseu {
	...
	namespace net {
		...
		void NkHTTPRequest::SetBasicAuth(const char *user, const char *pass) noexcept {
			...
			NkString b64 = NkHTTPClient::Base64Encode(reinterpret_cast<const uint8 *>(credentials.CStr()),
		...
		uint32 NkHTTPClient::SendAsync(const NkHTTPRequest &req, Callback cb) noexcept {
			...
			threading::NkThread worker([this, asyncReq](void *) { AsyncWorker(asyncReq); });
		...
		uint32
		NkHTTPClient::DownloadFile(const char *url, const char *destPath,
			...
			Callback wrappedCb = [this, destPath, onProgress, onComplete](const NkHTTPResponse &resp) {
				if (!resp.HasError() && resp.IsOK()) {
					...
					if (f.Open(tmpPath.CStr(), NkFileMode::NK_WRITE_BINARY)) {
						const usize written = f.Write(resp.body.CStr(), static_cast<usize>(resp.body.Length()));
						f.Close();
		...
		NkString NkHTTPClient::URLDecode(const char *s) noexcept {
			...
			for (const char *p = s; *p != '\0'; ++p) {
				if (*p == '%' && encoding::ascii::NkIsHexDigit(*(p + 1)) && encoding::ascii::NkIsHexDigit(*(p + 2))) {
					...
					uint8 decoded = static_cast<uint8>((hexToVal(*(p + 1)) << 4) | hexToVal(*(p + 2)));
		...
		bool NkHTTPClient::ParseURL(const NkString &url, NkString &scheme, NkString &host, uint16 &port,
									NkString &path) const noexcept {
			if (url.Empty()) {
				return false;
			}
			...
			if (p < end && *p == ':') {
				...
				while (p < end && encoding::ascii::NkIsDigit(*p)) {
					++p;
				}
				if (portStart < p) {
		...
		NkHTTPResponse NkHTTPClient::SendOverTCP(const NkHTTPRequest &req) noexcept {
			...
			if (!SendWithTimeout(sock, requestStr.CStr(), static_cast<uint32>(requestStr.Length()), req.timeoutMs)) {
		...
		void NkLeaderboard::Configure(const char *baseUrl, const char *apiKey) noexcept {
			if (baseUrl == nullptr || apiKey == nullptr) {
				return;
			}
			mBaseUrl = baseUrl;
			mApiKey = apiKey;
		...
		void NkLeaderboard::SubmitScore(const char *playerName, uint64 score,
										NkFunction<void(bool success)> cb) noexcept {
			if (playerName == nullptr || mBaseUrl.Empty()) {
				return;
			}
		...
		void NkLeaderboard::FetchTop(uint32 count, NkFunction<void(NkVector<NkLeaderboardEntry> &)> cb) noexcept {
			if (mBaseUrl.Empty() || cb == nullptr) {
				return;
			}
		...
		void NkLeaderboard::FetchPlayerRank(const char *playerName,
											NkFunction<void(uint32 rank, uint64 score)> cb) noexcept {
			if (playerName == nullptr || mBaseUrl.Empty() || cb == nullptr) {
				return;
			}
		...
		static void ParseLeaderboardJSON(const NkString &json, NkVector<NkLeaderboardEntry> &out) noexcept {
			...
			while ((pos = json.Find("{", pos)) != NkString::npos) {
				...
				if (scorePos != NkString::npos) {
					...
					while (valStart < entry.Length() && !encoding::ascii::NkIsDigit(entry[valStart])) {
						++valStart;
					}
		...
		static void ParsePlayerRankJSON(const NkString &json, uint32 &outRank, uint64 &outScore) noexcept {
			...
			if (rankPos != NkString::npos) {
				size_t valStart = rankPos + 7;
				while (valStart < json.Length() && !encoding::ascii::NkIsDigit(json[valStart])) {
					++valStart;
				}
		...
		static void SafeStrCopy(char *dst, uint32 dstSize, const char *src, uint32 maxLen) noexcept {
			...
			uint32 len = 0;
			while (src[len] && len < maxLen && len < dstSize - 1) {
				++len;
			}
