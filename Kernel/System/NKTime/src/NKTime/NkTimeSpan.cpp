...
namespace nkentseu {
	...
	NkTimeSpan::NkTimeSpan() noexcept : mTotalNanoseconds(0) {
		// Initialisation explicite à zéro pour clarté
		// La liste d'initialisation du membre suffit, corps vide
	}

	NkTimeSpan::NkTimeSpan(int64 days, int64 hours, int64 minutes, int64 seconds, int64 milliseconds,
						   int64 nanoseconds) noexcept {
		...
		mTotalNanoseconds = days * NS_PER_DAY + hours * NS_PER_HOUR + minutes * NS_PER_MINUTE +
							seconds * NS_PER_SECOND + milliseconds * NS_PER_MILLISECOND + nanoseconds;
	}
	...
	NkTimeSpan NkTimeSpan::operator*(double f) const noexcept {
		...
		return NkTimeSpan(static_cast<int64>(static_cast<double>(mTotalNanoseconds) * f));
	}
	...
	NkTimeSpan &NkTimeSpan::Multiply(double f) noexcept {
		...
		mTotalNanoseconds = static_cast<int64>(static_cast<double>(mTotalNanoseconds) * f);
		return *this;
	}
	...
	NkTimeSpan &NkTimeSpan::Divide(double d) noexcept {
		...
		if (d != 0.0) {
			mTotalNanoseconds = static_cast<int64>(static_cast<double>(mTotalNanoseconds) / d);
		}
		return *this;
	}
