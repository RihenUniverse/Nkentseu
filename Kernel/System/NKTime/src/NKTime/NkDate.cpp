...
namespace nkentseu {
	...
	NkDate::NkDate(int32 y, int32 m, int32 d) : year(y), month(m), day(d) {
	...
	void NkDate::Validate() const {
		...
		NKENTSEU_ASSERT_MSG(year >= 1601 && year <= 30827, "Year must be in range [1601, 30827]");

		// Vérification de la plage mensuelle [1, 12]
		NKENTSEU_ASSERT_MSG(month >= 1 && month <= 12, "Month must be in range [1, 12]");
	...
	void NkDate::SetYear(int32 y) {
		...
		NKENTSEU_ASSERT_MSG(y >= 1601 && y <= 30827, "Year must be in range [1601, 30827]");
		year = y;
	...
	void NkDate::SetMonth(int32 m) {
		...
		NKENTSEU_ASSERT_MSG(m >= 1 && m <= 12, "Month must be in range [1, 12]");
		month = m;
...
// -------------------------------------------------------------------------
	...
	NkString NkDate::GetMonthName() const {
		...
		static const char *months[12] = {"Janvier", "Février", "Mars",		"Avril",   "Mai",	   "Juin",
