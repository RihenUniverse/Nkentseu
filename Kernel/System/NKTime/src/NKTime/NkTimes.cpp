...
namespace nkentseu {
	...
	NkTime::NkTime() noexcept : mHour(0), mMinute(0), mSecond(0), mMillisecond(0), mNanosecond(0) {
	...
	NkTime::NkTime(int32 hour, int32 minute, int32 second, int32 millisecond, int32 nanosecond)
		: mHour(hour), mMinute(minute), mSecond(second), mMillisecond(millisecond), mNanosecond(nanosecond) {
	...
	void NkTime::SetHour(int32 hour) {
		NKENTSEU_ASSERT_MSG(hour >= 0 && hour < HOURS_PER_DAY, "Hour must be in range [0, 23]");
		mHour = hour;
	}

	void NkTime::SetMinute(int32 minute) {
		NKENTSEU_ASSERT_MSG(minute >= 0 && minute < MINUTES_PER_HOUR, "Minute must be in range [0, 59]");
		mMinute = minute;
	}

	void NkTime::SetSecond(int32 second) {
		NKENTSEU_ASSERT_MSG(second >= 0 && second < SECONDS_PER_MINUTE_I32, "Second must be in range [0, 59]");
		mSecond = second;
	}

	void NkTime::SetMillisecond(int32 millis) {
		NKENTSEU_ASSERT_MSG(millis >= 0 && millis < MILLISECONDS_PER_SECOND, "Millisecond must be in range [0, 999]");
		mMillisecond = millis;
	}

	void NkTime::SetNanosecond(int32 nano) {
		NKENTSEU_ASSERT_MSG(nano >= 0 && nano < NANOSECONDS_PER_MILLISECOND, "Nanosecond must be in range [0, 999999]");
		mNanosecond = nano;
	}
	...
	NkTime::operator int64() const noexcept {
		...
		const int64 totalSec =
			...
			static_cast<int64>(SECONDS_PER_MINUTE_I32) *
				(static_cast<int64>(mMinute) + static_cast<int64>(MINUTES_PER_HOUR) * static_cast<int64>(mHour));
...
// -------------------------------------------------------------------------
		...
		// -------------------------------------------------------------------------
		return NkTime(local.tm_hour, local.tm_min, local.tm_sec,
	...
	void NkTime::Validate() const {
		...
		NKENTSEU_ASSERT_MSG(mHour >= 0 && mHour < HOURS_PER_DAY, "Invalid hour value");
		NKENTSEU_ASSERT_MSG(mMinute >= 0 && mMinute < MINUTES_PER_HOUR, "Invalid minute value");
		NKENTSEU_ASSERT_MSG(mSecond >= 0 && mSecond < SECONDS_PER_MINUTE_I32, "Invalid second value");
		NKENTSEU_ASSERT_MSG(mMillisecond >= 0 && mMillisecond < MILLISECONDS_PER_SECOND, "Invalid millisecond value");
		NKENTSEU_ASSERT_MSG(mNanosecond >= 0 && mNanosecond < NANOSECONDS_PER_MILLISECOND, "Invalid nanosecond value");
	}
