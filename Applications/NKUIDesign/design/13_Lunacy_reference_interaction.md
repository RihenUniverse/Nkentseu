...
| **Dégradés** linéaire / radial / angulaire, avec étapes de couleur déplaçables | 🟡 **partiel** (04/09) | le **linéaire est peint** : il suit le contour arrondi et honore son angle (sonde 49, commit `957c909c`) ; le modèle porte type libre + angle + arrêts. ⚠️ **Manquent, et attendent la capture Lunacy demandée à Rodolf (Q82)** : l'ordre des types dans FILLS, la barre d'arrêts (où, ajout, déplacement, opacité par arrêt), les **poignées de dégradé sur la toile**, le geste d'entrée. Radial / angulaire / **losange** : **nommés, pas peints** (le peintre retombe sur la couleur du premier arrêt, et l'interface le dit). La **rampe** est livrée : barre d'arrêts (clic = ajouter, glisser = déplacer, inverser, +) **et** liste (position · pastille · hex · opacité · poubelle), opacité **par arrêt**, calcul prémultiplié en sRGB nommé (sondes 49, 53) |
| Remplissage **image** et ses cadrages (menu `Fill ˅`) | ❌ **absent** | ⚠️ **Tranché par la capture du 04/09 : l'image est un TYPE DE REMPLISSAGE, pas un groupe** — la sixième vignette (damier) du popover. Le type est au format et au menu depuis le 04/09 ; **le peintre ne le rend pas** (il suppose une source d'image dans le document). La capture montre aussi une **rotation** du cadrage et un bouton **« Remove background »** (fonction d'IA) — nommés, hors périmètre |
