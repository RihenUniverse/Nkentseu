...
| **Un vrai sélecteur de fichiers** (deux volets, vignettes) | Blender / l'Explorateur Windows : rail à gauche, vignettes à droite | ✅ **livré le 05/09** | sonde 97, `NKEditorKit/NkFilePickerNav.h`. Rail (disques, Accueil, Bureau, Documents, Téléchargements, Images, favoris, chaîne du dossier courant), **vignettes**, fil d'Ariane, recherche, tri, filtre d'extension, champ de nom. Il **n'écrit aucun navigateur** : le volet droit **est** `NkDrawContentBrowser`. ⚠️ **La mesure a contredit la prémisse** — NKCode n'avait pas de sélecteur à lui, il héritait de celui du kit ; ce qui manquait était le second volet. **L'ancien reste** (« Choisir une image… »). Trois dettes payées au passage : la **vignette déclarée n'était peinte par personne**, le fil d'Ariane ne portait qu'un libellé ambigu, et la bande « Contenu » + les boutons Créer / Importer se taisent désormais sur demande. **Absents, nommés** : tri par date et par taille, favoris persistants, récents |
