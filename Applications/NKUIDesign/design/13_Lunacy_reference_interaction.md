...
|---|---|---|---|
| **Voir la forme PENDANT qu'on la crée** | glisser avec un outil de forme | ✅ **livré le 05/09** | sonde 94. La **forme réelle** se peint pendant le glisser — ellipse, triangle, polygone, étoile, ligne, flèche, cadre, rectangle arrondi — avec le costume qu'elle aura ; la boîte en pointillé et la pastille de taille restent **par-dessus**. ⚠️ **Une seule table de genre** (`NkFormeDeLOutil`) est lue par l'aperçu ET par le relâchement : deux tables auraient permis à la forme créée de différer de celle montrée, et le défaut se serait manifesté loin de sa cause |
...
| **Exporter une page en PNG**, à l'échelle ×1 / ×2 / ×3 | notre menu **Fichier › Exporter…**, **Ctrl+E** ou le **clic droit** — les trois ouvrent LE MÊME dialogue (format, échelle, étendue, sortie, nom), puis le sélecteur de fichier à deux volets du kit (⚠️ la page Export de Lunacy n'a pas été relue pour cette ligne : son geste n'est pas cité) | ✅ **livré le 05/09** | **Sonde 81 en PIXELS** (`e6310f87`) : le même peintre que la toile (`NkDesignPaint` → `NkGuiDrawList`) rendu **sans GPU** par `NkGuiDrawListRaster` (NKGui, en-tête seul), écrit par le codec PNG maison, relu par lui ; rect uni, dégradé, texte, image 2×2, carré tourné — quatre points par élément, à 1× et à 2× (400 × 240), 3× (600 × 360) ; le texte à 2× est **rastérisé à 28 px** (police embarquée à la taille exacte, pas un atlas étiré : 0,317 de bords flous contre 0,605 pour l'atlas de 1× étiré — la mutation). ⚠️ **Le clic dans le sélecteur du kit attend l'œil de Rodolf** (la sonde prouve l'export, pas le geste). Absents, nommés : les **tranches d'export** (*export slices*), JPEG / WebP (un octet de codec, NKImage écrit les deux), les polices de repli (CJK, emoji) |
| **Exporter la sélection en PNG** | le même dialogue, rangée **Étendue › Sélection (N)** | ✅ **livré le 05/09** | sonde 81d : la boîte des nœuds sélectionnés **transformés** plus la marge de leurs ombres et bordures extérieures (60 × 40 + 8 → 76 × 56), le reste **transparent** (alpha 0 mesuré) ; un descendant d'un sélectionné n'est pas redessiné ; sans sélection : **refusé et dit**, aucun fichier |
| **Exporter en SVG** (page, sélection, images embarquées) | le même dialogue, rangée **Format › SVG**, « images embarquées » | 🟡 **partiel** (05/09) | **Un lecteur de plus du document** (`64e65f71`, jamais les commandes du peintre) : `<g transform=matrix>` par nœud, `<rect rx>` / `<path>` par coin ou par tracé édité, un élément par remplissage, `<linearGradient>` / `<radialGradient>` (angulaire et losange **nommés**, rendus en linéaire, dits en commentaire), bordures en `stroke` (intérieure / extérieure par retrait d'une demi-épaisseur) ou quatre `<line>` par côté, `<feDropShadow>`, `<text>` en Inter avec la ligne de base du peintre, `<image href>` relatif au dossier de sortie ou `data:` base64, `fill-opacity`, `mix-blend-mode`. **Témoin (sonde 82)** : le SVG **re-rastérisé par le parseur SVG maison** (`NkSVGCodec`, NKImage — pas du code mort : Mou, NK3DModeler, NKCode l'appellent) aux mêmes points que le PNG (formes, dégradé à 24 de gris près, transformée, fond de page alpha compris) ; **texte et image en STRUCTURE seulement** (ce parseur ne sait ni `<text>` ni `<image>` ni `rx`, son en-tête le dit). **Non traduits, dits dans le fichier** : ombre interne, étendue d'ombre, Tile / Crop / rotation d'une image, les composants (leur boîte et leur libellé) |
| **Le dialogue d'export demande le format** | Lunacy : un panneau Export avec presets ; chez nous **une seule porte** — menu, `Ctrl+E`, clic droit | ✅ **livré le 05/09** | sonde 96. Rangées Format (PNG / SVG ; **PDF et Code visibles et grisés avec leur raison**), Échelle, Étendue, Sortie, Nom, et **une ligne qui dit ce que ça va faire** avant que rien ne soit écrit. ⚠️ **`Ctrl+E` n'est déclaré qu'une fois** (la table de commandes de la coquille) — la leçon de `Ctrl+D` du matin : deux déclarations = deux ouvertures |
| **Le nom du fichier est celui de l'objet** | Lunacy nomme d'après le calque | ✅ **livré le 05/09** | sonde 96 : page « Accueil.png », objet « Carré rouge.png », échelle « Carré rouge@2x.png », sélection multiple « 2 objets.png ». **Assaini** pour le système de fichiers (`\ / : * ? " < > |` → `_`, ni espace ni point final, vide → `sans_nom`) et **les accents restent** — NTFS et ext4 les acceptent, et le nom lu dans l'arbre doit rester reconnaissable dans le dossier |
| **Tout s'exporte, pas seulement les pages** | Lunacy exporte calques et groupes | ✅ **livré le 05/09** | sonde 96 : un **groupe emporte ses enfants**, mesuré DANS LES PIXELS (export 80 × 60, le point de l'enfant est bleu — exporter un groupe n'est pas exporter sa boîte) ; plusieurs objets → **une image** (boîte englobante) ou **un fichier par objet** (un descendant d'un choisi part avec son ancêtre ; un doublon devient ` (2)`, **jamais un écrasement silencieux** ; la sélection est rendue telle quelle) |
| **Un vrai sélecteur de fichiers** (deux volets, vignettes) | Blender / l'Explorateur Windows : rail à gauche, vignettes à droite | ✅ **livré le 05/09**, corrigé le soir sur ses six retours | sondes 97 à 103, `NKEditorKit/NkFilePickerNav.h`. Le volet droit **est** `NkDrawContentBrowser` — aucun quatrième navigateur écrit. ① **plein dès la première image** (`pickerPath` est le seul endroit où vit le chemin : rien à « armer », donc rien à oublier) ; ② **rail à la Windows** — « Récents », « Accès rapide », « Ce PC », « Dossier courant », sections titrées et repliables, dossiers usuels **lus du système** et volumes **montés** avec leur étiquette ; ③ **récents de la session ET du document** (ces derniers écrits dans le `.nkuidoc`, l'aller-retour est mesuré) ; ④ **il ne montre que ce que son mode exige** ; ⑤ **le résultat exporté se voit** — bandeau qui reste, vignette du fichier **relu du disque**, « Ouvrir le dossier » / « Ouvrir le fichier » ; ⑥ **outil par défaut du kit**, `NkDrawSelecteur` en **une ligne**, **quatre modes** (ouvrir un fichier, ouvrir un dossier, **créer un dossier**, enregistrer sous). ⚠️ **La mesure a contredit la prémisse** — NKCode n'avait pas de sélecteur à lui. **L'ancien reste appelable** ; **aucune autre application ne bascule toute seule** (deux lignes pour NK3DModeler ; NKCode demande d'abord de trancher le sort de son panneau supplémentaire). **Absents, nommés** : tri par date et par taille, favoris persistants |
...
| Proportions / depuis le centre / les deux | `Maj` / `Alt` / les deux | ✅ **livré le 02/09**, badge le 05/09 | **Lunacy le dit ainsi** : « Preserve Ratio: Shift + resize », « Resize from center: Alt + resize » (`lunacy.docs.icons8.com/shortcuts`, lu le 05/09) — Rodolf demandait « Maj ou Ctrl ? » : c'est **Maj**, comme chez eux ; la touche est lue **à chaque image pendant le glisser** (pas seulement à l'appui), un badge « proportionnel » / « depuis le centre » suit le pointeur, et la même touche vaut pour la poignée d'un **groupe** (échelle X = échelle Y) — sonde 77. `NkRedimModifie`, cas 49 — le **chemin frère** du tracé (§2) est traité, comme il avait été identifié. ⚠️ Le ratio vient de la boîte **de départ** : le recalculer à chaque image le laisserait glisser d'arrondi en arrondi jusqu'à ce qu'un carré finisse rectangle. ⚠️ Et **le plus grand mouvement commande** — prendre `dx` d'office rendrait le geste vertical inerte. `Alt` se mesure sur **le centre**, pas sur la taille : « depuis le centre » veut dire que le centre ne bouge pas |
...
| Aligner (six alignements) | boutons de la section **ALIGNER LA SÉLECTION** | ✅ **livré le 05/09** | 🔴 **ce ✅ était FAUX jusqu'au 05/09, et la ligne disait pourquoi sans le voir** : « la rangée d'alignement existe » désignait la section ALIGNEMENT, qui écrit `layout.mainAlign` / `crossAlign` — **comment un cadre range SES ENFANTS**. Elle ne déplace aucun objet, et sur une feuille elle affiche « ce nœud n'agence pas d'enfants ». C'est la question de Rodolf (« est-ce que c'est déjà en place ? ») qui l'a mise à nu. *Deux idées qui partagent un mot finissent par se prendre l'une pour l'autre.* Le geste réel vit désormais dans `Alignement.h` (six alignements sur la boîte englobante de la sélection), **sonde 93** ; les positions se calculent sur les rectangles de disposition puis s'ajoutent à `posX/posY`, et un nœud dont le parent agence ses enfants est **laissé, compté et dit** |
| Aligner sur l'**objet clé** (le dernier sélectionné ne bouge pas) | bascule « sur le dernier sélectionné » | ✅ **livré le 05/09** | sonde 93 : centrer verticalement trois objets sur le dernier — les deux autres prennent son milieu, lui ne bouge pas |
| S'aligner sur les bords du **cadre** quand un seul objet est sélectionné | les mêmes boutons, avec un seul élément | ✅ **livré le 05/09** | la référence devient **sa page**, et l'en-tête de la section le dit avant le clic (« 1 élément — référence : sa PAGE »). Sonde 93 : un objet seul se centre sur la page et le pied nomme la page |
| Distribuer H / V | boutons **Répartir H** / **Répartir V** (les raccourcis `Ctrl+Maj+H` / `Ctrl+Maj+V` ne sont **pas** branchés, dit) | ✅ **livré le 05/09** | sonde 93 : **trois éléments exigés** (avec deux il n'y a rien à répartir, et c'est refusé en le disant), les extrêmes tiennent, les autres se posent à intervalle égal **entre les CENTRES** — répartir les *espaces* (Figma) donnerait d'autres nombres sur des tailles différentes, et c'est écrit dans `Alignement.h` |
...
| **Une ligne = pastille (aperçu réel) · nom · œil · poubelle ; le détail dans le popover** | ligne de FILLS / BORDERS | 🟡 **partiel** (04/09) | **remplissages et bordures** : livré (popover complet : types, sélecteur sur l'arrêt courant, hexa, rampe, liste ; épaisseur, position, côtés, jointure, extrémités). **Effets** : la ligne garde ses champs (bornés). Sections : `CALQUE` (opacité + fusion, nommées) avant REMPLISSAGES, EFFETS après BORDURES. **Rotation + arrondi dans le bloc géométrie** (Q86 tranchée, 04/09 : le mandat Lunacy prime sur la décision écrite ; la rétractation est écrite là où l'ancienne décision l'était) — deux rangées, Lunacy n'en a qu'une : la fusion attend son œil. Témoin : l'inspecteur sans fenêtre à 260 et 170 px, aucun sommet hors du cadre (sonde 60) |
...
| **Dégradés** linéaire / radial / angulaire, avec étapes de couleur déplaçables, **poignées sur la toile** | 🟡 **partiel** | **Linéaire, radial, angulaire, losange PEINTS** (04/09) — la même méthode : des bandes qui suivent le contour arrondi (disques / losanges concentriques de l'extérieur vers l'intérieur, secteurs depuis le centre pour l'angulaire, origine = l'angle), nombre de bandes proportionnel à la taille (sonde 64 : deux couleurs franches, la couleur en chaque point est celle de la formule du genre). **Un geste pour les quatre genres (04/09 soir, ses huit captures radial)** : le genre ne dit que où sont les pastilles (l'axe : centre → bord pour le linéaire, origine → rayon dans la direction de l'angle pour radial / losange / angulaire) ; **dans le disque** d'une pastille on glisse le long de l'axe (arrêt : position ; extrémité d'un genre à origine : rayon), **dans l'anneau** autour on tourne l'axe (aimant 0 / 45 / 90, ligne guide rouge, badge d'angle), le centre déplace l'origine (anneau de survol), les carrés règlent les rayons X / Y, le segment ajoute un arrêt ; le curseur annonce le geste avant le clic (sonde 60k, boucle sur les quatre genres). **Le soir** : les anneaux n'existent que popover ouvert ; quand deux zones se chevauchent **la plus proche du pointeur gagne** (arcs de rotation, huit poignées de forme, pastilles — sonde 60s) ; **tolérance nommée 12 px** pour les arcs et les poignées de forme (bornée au tiers du nœud — sonde 60t) ; **info-bulles** après 0,4 s (« Tourner l'axe », « Glisser l'arrêt », « Tourner », « Redimensionner »… — sonde 60v) ; **le sélecteur suit la sélection** (vide, autre nœud, suppression → il se ferme — sonde 60u) ; **l'angulaire tel que Lunacy** : les arrêts SUR le cercle de portée (fraction de tour), l'extrémité glissée le long du cercle tourne l'axe, deux carrés sur le cercle suivent l'axe et règlent le rayon, halo au survol (sonde 60w). Origine et rayons dans le fichier (`o=`, `r=`, additifs, sonde 65a) et le popover (Rayon X / Y, Origine X / Y). **Dit** : l'ellipse de portée reste alignée sur la boîte quand l'axe tourne (Lunacy tourne l'ellipse avec l'axe). **Absent, nommé** : l'opacité par arrêt est approximative pour radial / losange (disques recouvrants). Le linéaire : poignées, orientation par les extrémités, angle dans le popover (sonde 60k) |
| Remplissage **image** et ses cinq cadrages (`Fill` · `Fit` · `Stretch` · `Tile` · `Crop`) | 🟡 **partiel** (05/09) | ⚠️ **Tranché par la capture du 04/09 : l'image est un TYPE DE REMPLISSAGE, pas un groupe** — la sixième vignette du popover. **La chaîne est faite (05/09, sondes 70a-e)** : primitive de kit `NkComponentPaint::ImagePolygone` (virtuelle à vide, additive — les neuf consommateurs recompilent verts) sur `NkGuiDrawList::AddImagePolygon` (polygone convexe texturé, uv par sommet) ; `image=` **relatif au document** (référencé, pas copié), cache `NkCacheImages` (NKImage, 12 codecs, téléversé par `NkEditorShell::UploadRGBA`), **absence dite** (damier + « INTROUVABLE » en rouge, chemin noté) ; **les cinq cadrages PEINTS**, la **rotation**, le **contour arrondi qui rogne** (cellule tournée rognée par `NkGDecouperParConvexe`, uv par l'inverse de la pose — `PushClip` du kit rogne en rectangle, mesuré) ; Tile plafonné à 4096 cellules (dit) ; `crop=x,y,w,h` additif ; popover : aperçu, source, « Choisir une image… » (le sélecteur du kit), « Recharger », fenêtre de crop. **Le crop SUR LA TOILE (05/09, sonde 80, `7100d28a`)** : le popover image ouvert sur un remplissage en cadrage `Crop`, le cadre de l'IMAGE ENTIÈRE se dessine autour du nœud (le nœud est la fenêtre) avec ses huit poignées ; en tirer une change `crop=` — le nœud ne bouge pas, l'image suit — 12 px, décision ordonnée avec les poignées de forme et de dégradé, bulle « Recadrer » ; l'axe qui ne bouge pas garde ses valeurs exactes. Témoin en **texels** sur un PNG 2×2 écrit par la sonde. **Absents, nommés** : la fenêtre de crop **sur la toile** (Lunacy la règle à la souris), « Retirer le fond (IA) », l'export qui embarque le fichier, le témoin en pixels sur le dorsal réel (`--capture`, une fenêtre) |
| **Pipette** pour prélever une couleur (capture 04/09 : en bas à gauche du popover) | ❌ **absent** | ⚠️ **et chez nous elle devrait prélever un RÔLE**, pas une valeur — sinon elle contourne le thème. Un cas où copier Lunacy tel quel serait une régression |
| **L'opacité sur sa propre ligne** (barre damier → couleur pleine + curseur) | ✅ **livré le 05/09** | sonde 95 : une barre sur damier de la couleur **transparente** à la couleur **pleine**, un curseur, et le champ numérique qui reste. **Deux commandes, un seul modèle** — le curseur n'a aucune mémoire propre ; glisser dans la barre écrit l'opacité du remplissage |
| Saisie HEX / RGBA / HSB / HSL | 🟡 **partiel** | **La rangée de Lunacy, sur UNE ligne** (ses neuf captures du 04/09, `design/captures/2026-09-04_lunacy_modele_*_rangee.png`) : `[Modèle ˅] v1 v2 v3 [opacité %]`. **Les neuf modèles opérants (05/09)** : Hex, RGB, HSB, et **HSL, HWB, LCH, LAB, OKLCH, OKLAB** derrière **une seule frontière** (`NkRgbVersModele` / `NkModeleVersRgb`, `couleurdetail` en float64 — CSS Color 4 : courbe sRGB, matrices lin-sRGB↔XYZ **D65** exactes, Lab avec ε = 216/24389 et κ = 24389/27 **relatif à D65** (pas le D50 de `lab()`, dit), OKLab d'Ottosson ; plages et décimales de ses captures, virgule affichée, signes). **Aller-retour Hex → modèle → Hex identique sur les 16,7 M de couleurs pour les huit** (sondes 60g, 71) — là où Lunacy dérive d'une unité (1976D2 → 1A76D1 après OKLAB) : l'affichage n'écrit jamais l'hexa. Champs mesurés pour « -54,00 » (sonde 60f). **La saisie directe est faite (04/09, ②)** : l'hexa se tape dans les popovers ET sur les lignes, les champs de valeurs acceptent la frappe — une seule porte (`NkPorteHex` / `NkPorteNombre`), l'écriture repasse par la frontière (sonde 60j). **Le bouton « Créer une variable de couleur » sous la rangée est FAIT (05/09, sonde 68a)**. **Absent, nommé** : **le plan du sélecteur qui suit le modèle** (LCH/LAB/OKLAB montrent une autre nappe que saturation/valeur) |
...
| **Mode de fusion PAR REMPLISSAGE** (18 modes : Normal, Darken, Multiply, Plus Darker, Color Burn, Lighten, Screen, Plus Lighter, Color Dodge, Overlay, Soft Light, Hard Light, Difference, Exclusion, Hue, Saturation, Color, Luminosity) | 🟡 **partiel** | **Les 18 modes se choisissent et s'enregistrent** (04/09, ②-1) : `NkRemplissage.fusion` = la clé CSS `mix-blend-mode`, `fusion=<clé>` additif au fichier, la ligne dit « Linéaire · Multiply » ; **cinq se peignent, exacts au GPU** (②-2, primitive de kit : un mode de mélange par commande dans `NkGuiDrawCmd`, `PushBlend/PopBlend` sur la liste de dessin et le peintre, honoré par NKCanvas sur GL / DX11 / DX12 / Vulkan) : Multiply, Screen, Darken, Lighten, Plus Lighter (sondes 60q, 67). **Treize restent « enregistré, pas encore peint »** (Overlay, Soft/Hard Light, Burn/Dodge, Difference, Exclusion, Hue, Saturation, Color, Luminosity : ils lisent la destination → rendu en texture, nommé) ; le témoin en pixels sur le dorsal réel passe par `--capture` (fenêtre) |
| **Styles de couleur nommés** (la ligne de remplissage porte « Dark Primary ») et **`Styles` / `Variables`** au rail de gauche | 🟡 **partiel** (04/09) | capture 04/09. Périmètre tranché (§15.14, validé par délégation) : **la VARIABLE est livrée** — `variable = <clé> <valeur> nom="…" @mode=valeur`, référence « @clé » sur remplissage / bordure / ombre / arrêt / texte, absente **dite** (magenta + nom en rouge), **modes** (valeur par mode, mode courant du document — la place de Dark Pro / Light Pro), propagation Q51 par la référence (sonde 63) ; la ligne FILLS montre le **nom**. **Dans l'interface (05/09, sondes 68a-f)** : **« Créer une variable de couleur »** sous la rangée modèle du sélecteur (la place de Lunacy) — un clic crée « Couleur N » et la couleur (remplissage ou arrêt) la référence ; quand la couleur référence une variable, le sélecteur montre pastille · **nom** · **« Détacher »** et **édite la VARIABLE** (carré SV, hexa, RGB / HSB par une seule porte) — tout ce qui la référence suit, la référence tient, aucun écart d'instance posé ; le modèle compte / refuse / détache / supprime par **un seul visiteur** de couleurs (huit champs) ; **deux modes** éprouvés à l'aller-retour fichier (rend différemment selon le mode courant) ; **le rail `Variables`** (gauche, onglet à côté de la Hiérarchie) : pastille du mode courant, nom renommable sur place, clé, une ligne par mode, usages ×N, poubelle **gardée** (« utilisée par N remplissages »). **Les STYLES (05/09 nuit, sondes 69a-g, §15.15)** : style de **calque** (remplissages + bordures + effets) et de **texte** (taille, graisse, couleur — la police-famille n'est pas dans le modèle du nœud, donc pas dans le style, dit) ; `style = <clé> genre=… nom="…"` puis les listes **par les mêmes écrivains et lecteurs que le nœud** (`apparence` = un nœud sans géométrie), `style_calque` / `style_texte` sur les nœuds, inconnu préservé, aller-retour octet pour octet ; **propagation par le même mécanisme que les instances** (copie à la modification sous les bits d'écart ; l'écart détecté à l'édition humaine dans `MarkHumanEdit`) — modifier le style propage, une surcharge locale tient, « Réinit. » rend la main, « Appliquer » fait du calque le style ; un style absent est **dit**, le calque garde ses valeurs ; une variable dans un style compte comme usage et se compose ; **le rail `Styles`** (aperçu, nom, ×N, poubelle gardée « utilisé par N calques ») ; **la rangée « Style : … » en tête de REMPLISSAGES et de TYPOGRAPHIE** (Créer, Lier ˅ déplié en place, nom du style lié, Détacher, surcharge → Réinit. / Appliquer). Mutation payée : la propagation ignorée → six cas rouges. **Lier une variable existante** depuis le sélecteur : fait (05/09, sonde 68g) — « Lier ˅ » à droite de « Créer une variable », la liste dépliée dans le popover, un clic fait de la couleur une référence. **Absents, nommés** : la capture Lunacy du rail Styles (la place du bouton vient de l'icône de FILLS), la **bascule de mode** dans l'interface (le rail DIT le mode), le nom d'une variable dans le popover de **bordure** |
| **Opacité + mode de fusion du CALQUE** (section `LAYER`) | 🟡 **partiel** | l'opacité existe ; le mode de fusion du calque, non |
...
| Poser une instance | glisser depuis le panneau | ✅ **livré le 02/09 — porte ET geste** | `InstancierComposant` (instance **matérialisée**, mêmes raisons mesurées qu'à l'extraction) + **double-clic** dans la section COMPOSANTS de la Hiérarchie — qui liste désormais les composants **du document** (le registre du kit y vivait **en double** de la palette du rail ; séparation §15.1 appliquée aux panneaux). Cible = la sélection, repli sur la **première page** (jamais la racine), refus **qui parle** sans page. L'état vide se dit. Mutations « lien sans chair » et « racine au lieu de la page » payées. ⚠️ Le **glisser-déposer** de Lunacy n'est pas porté — le geste ici est le double-clic (celui du panneau Assets) |
...
| Le composant se propage à ses instances **sauf** là où une surcharge existe | `Ctrl+Alt+M` | ✅ **livré le 02/09 — porte ET geste** | 📌 **Q51 RÉVISÉE ET VALIDÉE (02/09)** : propagation **automatique** pour MES composants (comme Lunacy), surcharges tenues — « l'arrondi change les deux, le bleu reste bleu » ; modifier une INSTANCE = surcharge locale + dialogue à trois branches (variation locale / appliquer à l'original / nouveau composant), seulement si `NkPeutModifierDeclaration` dit oui — sinon **copie, seule voie, sans dialogue** (kit ET tiers ; le fork m'appartient ensuite). « En retard » + mise à jour volontaire pour le TIERS seulement (R4). Cf. §15.6, révision datée avec les deux citations |
| États d'un composant (Défaut / Survol / Pressé) | `Ctrl+Alt+P` | 🟡 **partiel — le mécanisme EXISTE déjà, la question est son étendue** | 📌 **Réconciliation faite le 02/09, et elle a supprimé le chantier au lieu de le cadrer** : le format `.nkgui` porte déjà une **liste FERMÉE de six états** (`Normal`, `Hover`, `Pressed`, `Focus`, `FocusVisible`, `Disabled`), tranchée par **Rodolf le 27/08**, sous forme de blocs `appearance(État)` qui portent des **surcharges par propriété** — la même forme que `NkUINode::ecarts` — et dont **l'ordre de la table EST la priorité**, tenu par un contrôle. Les trois états de Lunacy en sont un **sous-ensemble** : aucune divergence à assumer. ✅ **Q53 tranchée (02/09) : LES SIX**, et « si plus tard on en ajoutait, ça devrait montrer le nombre exact » — contrainte de structure : le panneau ITÈRE la table fermée (aucune liste en dur), banc = rangées du panneau vs entrées de la table (jumeau de `NkNbRaccourcisCtx`). Reste à CODER le panneau |
