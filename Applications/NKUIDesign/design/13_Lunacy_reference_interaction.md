...
| Saisie HEX / RGBA / HSB / HSL | 🟡 **partiel** | HEX seulement — **et le sélecteur de couleur est branché depuis le 04/09** (carré SV + barre de teinte de `NKGui`, en popup, sur remplissage / bordure / effet / état / **chaque arrêt de dégradé**). ⚠️ La capture montre **neuf modèles** dans le menu `Hex ˅` : Hex, RGB, HSB, HSL, HWB, LCH, LAB, OKLCH, OKLAB — **sept absents**, nommés ici |
...
| **Styles de couleur nommés** (la ligne de remplissage porte « Dark Primary ») et **`Styles` / `Variables`** au rail de gauche | ❌ **absent** | capture 04/09. C'est le *« styles comme Lunacy »* de Rodolf. ⚠️ **Question de périmètre posée (Q84)** : un style porte-t-il seulement la couleur, ou aussi bordure / ombre / texte ? Le mécanisme de partage par filiation existe déjà pour les composants — les deux se rejoindront |
