...
| **Dégradés** linéaire / radial / angulaire, avec étapes de couleur déplaçables, **poignées sur la toile** | 🟡 **partiel** (04/09) | le **linéaire est peint** : il suit le contour arrondi et honore son angle (sonde 49, commit `957c909c`) ; le modèle porte type libre + angle + arrêts. ⚠️ **Manquent, et attendent la capture Lunacy demandée à Rodolf (Q82)** : l'ordre des types dans FILLS, la barre d'arrêts (où, ajout, déplacement, opacité par arrêt), les **poignées de dégradé sur la toile**, le geste d'entrée. Radial / angulaire / **losange** : **nommés, pas peints** (le peintre retombe sur la couleur du premier arrêt, et l'interface le dit). La **rampe** est livrée : barre d'arrêts (clic = ajouter, glisser = déplacer, inverser, +) **et** liste (position · pastille · hex · opacité · poubelle), opacité **par arrêt**, calcul prémultiplié en sRGB nommé (sondes 49, 53). **Poignées sur la toile (linéaire) : livrées** — une par arrêt sur le segment, glisser = déplacer, clic sur le segment = ajouter par la même fonction que la barre, poignée réclamée AVANT le segment (sonde 61) ; le geste attend l'œil de Rodolf. Radial / angulaire / losange : pas de poignées tant qu'ils ne sont pas peints |
