...
| **Dégradés** linéaire / radial / angulaire, avec étapes de couleur déplaçables, **poignées sur la toile** | 🟡 **partiel** | **Linéaire, radial, angulaire, losange PEINTS** (04/09) — la même méthode : des bandes qui suivent le contour arrondi (disques / losanges concentriques de l'extérieur vers l'intérieur, secteurs depuis le centre pour l'angulaire, origine = l'angle), nombre de bandes proportionnel à la taille (sonde 64 : deux couleurs franches, la couleur en chaque point est celle de la formule du genre). **Les poignées des trois genres sont sur la toile (04/09, ses captures)** : l'ellipse de portée, le centre (origine), le carré du côté (rayon X), le carré du haut (rayon Y), une pastille par arrêt sur le segment origine → bas, la pastille de contour de l'angulaire ; origine et rayons entrent dans le fichier (`o=`, `r=`, additifs) et dans le popover (Rayon X / Y, Origine X / Y) ; l'icône de rotation sur la barre pour l'angulaire (sonde 60o, 65a). **Absent, nommé** : l'opacité par arrêt est approximative pour radial / losange (disques recouvrants). Le linéaire : poignées, orientation par les extrémités, angle dans le popover (sonde 60k) |
