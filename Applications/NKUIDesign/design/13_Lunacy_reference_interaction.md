...
| Remplissage **image** et ses cinq cadrages (`Fill` · `Fit` · `Stretch` · `Tile` · `Crop`) | 🟡 **partiel** (04/09) | ⚠️ **Tranché par la capture du 04/09 : l'image est un TYPE DE REMPLISSAGE, pas un groupe** — la sixième vignette (damier) du popover. Modèle, format (`genre=image cadrage=… image=… rotation_image=…`, additifs, inconnu préservé), popover (damier, cadrage à cinq mots, rotation) et peintre **livrés** — le peintre montre le **damier** d'une image absente et le dit : **la source n'est pas chargée** (aucune primitive d'image au peintre du kit, aucune source dans le document). Sonde 62. La capture montre aussi une **rotation** du cadrage et un bouton **« Remove background »** (fonction d'IA) — nommés, hors périmètre |
...
| Saisie HEX / RGBA / HSB / HSL | 🟡 **partiel** | **La rangée de Lunacy, sur UNE ligne** (ses neuf captures du 04/09, `design/captures/2026-09-04_lunacy_modele_*_rangee.png`) : `[Modèle ˅] v1 v2 v3 [opacité %]`. **Hex, RGB, HSB opérants** ; **HSL, HWB, LCH, LAB, OKLCH, OKLAB nommés dans le menu, grisés** (« conversion non écrite ») — derrière **une seule frontière** (`NkRgbVersModele` / `NkModeleVersRgb`, plages et formats par modèle : entiers, deux ou trois décimales, **virgule** affichée, **signes** LAB/OKLAB). **Aller-retour Hex → modèle → Hex identique sur les 16,7 M de couleurs** (sonde 60g) — là où Lunacy dérive d'une unité (1976D2 → 1A76D1 après OKLAB) : l'affichage n'écrit jamais l'hexa. Champs mesurés pour « -54,00 » (sonde 60f). **La saisie directe est faite (04/09, ②)** : l'hexa se tape dans les popovers ET sur les lignes (remplissage, bordure), les champs de valeurs (RGB / HSB, opacités, positions, épaisseurs) acceptent la frappe au clic net — une seule porte (`NkPorteHex` / `NkPorteNombre`), l'écriture RGB / HSB repasse par la frontière de conversion (sonde 60j). **Le bouton « Créer une variable de couleur » sous la rangée est FAIT (05/09, sonde 68a)** — Lunacy crée la variable DANS le sélecteur, nous aussi. **Absent, nommé** : **le plan du sélecteur qui suit le modèle** (LCH/LAB/OKLAB montrent une autre nappe que saturation/valeur) |
