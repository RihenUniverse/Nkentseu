...
// SW CPU shaders
// =============================================================================
static NkSWVertexShader MakeBgVertSW() {
    return [](const void* vdata, uint32 idx, const void*) -> NkSWVertex {
        const VtxBg* v = static_cast<const VtxBg*>(vdata) + idx;
        NkSWVertex out; out.position={v->x,v->y,0,1}; out.color={v->r,v->g,v->b,1};
        return out;
    };
}
static NkSWPixelShader MakeBgFragSW() {
    return [](const NkSWVertex& v, const void*, const void*) -> NkSWColor {
        return v.color;
    };
}

static NkSWVertexShader MakeTextVertSW() {
