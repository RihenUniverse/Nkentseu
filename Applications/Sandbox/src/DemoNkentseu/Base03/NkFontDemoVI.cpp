...
public:
    ...
    void GenerateText3DMesh(NkFont* fnt, const char* text,
        ...
        NkVec4f color = {r, g, b, a};

        // Appel à l'API native NKFont pour générer le mesh 3D complet
        NkFontGlyphMesh3D mesh = fnt->GenerateTextMesh3D(text, scale, extrusionDepth, worldMatrix, color);
        
        // Conversion et envoi des vertices via le callback
        if (mesh.vertices.Size() > 0) {
            for (const auto& v : mesh.vertices) {
                // Conversion NkFontGlyph3DVertex → FontVtx3D_NoTex (sans UV pour shader NoTex)
                FontVtx3D_NoTex vtx = {
                    v.position.x, v.position.y, v.position.z,  // position
                    v.normal.x, v.normal.y, v.normal.z,        // normale
                    v.color.r, v.color.g, v.color.b, v.color.a // couleur
                };
                callback(&vtx, 1, userData);  // Appel callback par vertex (simple et robuste)
            }
        }
    }
