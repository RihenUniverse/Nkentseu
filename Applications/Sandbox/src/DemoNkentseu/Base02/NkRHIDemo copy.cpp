...
static void DemoRHIRendering(NkIDevice* dev) {
    ...
    auto texDesc = NkTextureDesc::Tex2D(2,2,NkPixelFormat::RGBA8_Srgb,1);
    auto tex = dev->CreateTexture(texDesc);
    dev->WriteTexture(tex, pixels, 2*4);
    printf("[RHI] Texture 2×2: %s\n", tex.IsValid() ? "OK" : "FAIL");

    // ── Sampler ───────────────────────────────────────────────────────────────
    auto samp = dev->CreateSampler(NkSamplerDesc::NK_NEAREST());
...
void main(){ frag = texture(uTex, vUV); })";
    ...
    NkShaderDesc sd;
    sd.AddGLSL(NkShaderStage::NK_VERTEX,   vsGLSL)
      .AddGLSL(NkShaderStage::NK_FRAGMENT, fsGLSL);
    ...
    layoutDesc.Add(0, NkDescriptorType::NK_UNIFORM_BUFFER, NkShaderStage::NK_VERTEX)
              .Add(1, NkDescriptorType::NK_COMBINED_IMAGE_SAMPLER, NkShaderStage::NK_FRAGMENT);
    ...
    cmd->BindIndexBuffer(ib, NkIndexFormat::NK_UINT16);
    cmd->DrawIndexed(3);
