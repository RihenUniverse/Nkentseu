/**
 * @File    NkUIDemoNKEngine.cpp  — v5 (intégration police GPU via NkUIRHIRenderer)
 * @Brief   Version complète avec NKWindow + NKRHI + NkShaderConvert.
 *
 * MODIFICATIONS v5 :
 *   [H] Intégration police GPU : NkUIFontLoader utilise désormais un callback
 *       qui appelle NkUIRHIRenderer::UploadFontTexture().
 *       NkUI ne connaît pas NKRHI — seul NkUIRHIRenderer gère les textures.
 *
 *   [I] NkUIRHIRenderer gère maintenant un map de textures de polices
 *       (fontTextures) pour pouvoir binder la bonne texture par draw call.
 *
 *   [J] Les draw commands NkUI peuvent maintenant spécifier une texture custom
 *       (texId) qui sera bindée avant le draw.
 */
...
struct NkUIRHIRenderer {
    ...
    // Map des textures de polices (ID NkUI -> handle GPU)
    NkHashMap<uint32, NkTextureHandle> fontTextures;
    uint32 currentTexId = 0;  // Texture actuellement bindée
    ...
    // Uploader une texture de police depuis les données CPU
bool UploadFontTexture(uint32 texId, const uint8* data, int32 w, int32 h) {
    if (!device) {
        logger.Errorf("[NkUIRenderer] No device!\n");
        return false;
    }
    
    logger.Infof("[NkUIRenderer] Creating font texture %u (%dx%d)\n", texId, w, h);
    
    // Validation des paramètres
    if (w <= 0 || h <= 0 || !data) {
        logger.Errorf("[NkUIRenderer] Invalid parameters\n");
        return false;
    }
    
    // Déterminer si les données sont en R8 ou RGBA8
    // On regarde la taille : si w*h*4 == données, c'est RGBA8, sinon R8
    // Pour simplifier, on suppose que le callback reçoit toujours RGBA8
    // car nous avons configuré uploadRGBA8 = true
    
    // Créer la texture
    NkTextureDesc texDesc = NkTextureDesc::Tex2D(w, h, NkGPUFormat::NK_RGBA8_UNORM, 1);
    texDesc.bindFlags = NkBindFlags::NK_SHADER_RESOURCE;
    texDesc.debugName = "NkUI_FontAtlas";
    
    logger.Infof("[NkUIRenderer] Creating RGBA8 texture...\n");
    NkTextureHandle texture;// = device->CreateTexture(texDesc);
    
    if (!texture.IsValid()) {
        logger.Errorf("[NkUIRenderer] CreateTexture failed\n");
        return false;
    }
    
    logger.Infof("[NkUIRenderer] Writing texture data...\n");
    uint32 rowPitch = w * 4;  // 4 octets par pixel pour RGBA8
    
    bool writeSuccess = device->WriteTexture(texture, data, rowPitch);
    
    if (!writeSuccess) {
        logger.Errorf("[NkUIRenderer] WriteTexture failed\n");
        device->DestroyTexture(texture);
        return false;
    }
    
    // Stocker dans la map
    fontTextures[texId] = texture;
    
    logger.Infof("[NkUIRenderer] Font texture %u created successfully\n", texId);
    return true;
}
    
    // Obtenir une texture de police par son ID
    NkTextureHandle GetFontTexture(uint32 texId) {
        auto it = fontTextures.Find(texId);
        if (it != nullptr) {
            return *it;
        }
    ...
    // Binder une texture spécifique dans le descriptor set
    void BindTexture(NkICommandBuffer* cmd, uint32 texId) {
        if (!cmd || !hDescSet.IsValid()) return;
        
        if (texId == 0) {
            // Utiliser la texture blanche par défaut
            if (currentTexId != 0) {
                NkDescriptorWrite write{};
                write.set = hDescSet;
                write.binding = 0;
                write.type = NkDescriptorType::NK_COMBINED_IMAGE_SAMPLER;
                write.texture = hWhiteTex;
                write.sampler = hSampler;
                write.textureLayout = NkResourceState::NK_SHADER_READ;
    ...
    //         if (vsz > kVBOCap || isz > kIBOCap) {
    //             logger.Infof("[NkUIRenderer] Layer %d: buffer overflow, skip\n", l);
    //             continue;
    //         }
    ...
    //                 const float32 x = ::fmaxf(dc.clipRect.x, 0.f);
    //                 const float32 y = ::fmaxf(dc.clipRect.y, 0.f);
    //                 const float32 x2 = ::fminf(x + dc.clipRect.w, (float32)fbW);
    //                 const float32 y2 = ::fminf(y + dc.clipRect.h, (float32)fbH);
    //                 currentScissor = {
    //                     (int32)x,
    //                     (int32)y,
    //                     (int32)::fmaxf(x2 - x, 0.f),
    //                     (int32)::fmaxf(y2 - y, 0.f)
    //                 };
    //                 cmd->SetScissor(currentScissor);
    //                 continue;
    //             }
    ...
    //             // Pour chaque draw, on met à jour le descriptor set avec la bonne texture
    //             NkDescriptorWrite write{};
    //             write.set = hDescSet;
    //             write.binding = 0;
    //             write.type = NkDescriptorType::NK_COMBINED_IMAGE_SAMPLER;
    ...
    //             if (dc.texId == 0) {
    //                 // Utiliser la texture blanche pour l'UI
    //                 write.texture = hWhiteTex;
    //             } else {
    //                 // Utiliser la texture de police
    //                 auto it = fontTextures.Find(dc.texId);
    //                 if (it != nullptr) {
    //                     write.texture = *it;
    //                 } else {
    //                     write.texture = hWhiteTex;
    ...
    void Destroy() {
        ...
        // Détruire toutes les textures de polices
        for (auto& pair : fontTextures) {
            if (pair.Second.IsValid()) {
                device->DestroyTexture(pair.Second);
            }
        }
        fontTextures.Clear();
...
int nkmain(const nkentseu::NkEntryState& state) {
    ...
    // ShaderCache
    {
        ...
        logger.Infof("[NkUIDemoEngine] ShaderCache dir: Build/ShaderCache\n");
    }
    ...
    if (!window.Create(winCfg)) {
        logger.Infof("[NkUIDemoEngine] Window creation failed\n");
        return 1;
    }
    ...
    if (!device || !device->IsValid()) {
        logger.Infof("[NkUIDemoEngine] NkIDevice creation failed (%s)\n",
                    NkGraphicsApiName(api));
        window.Close();
        return 1;
    }
    logger.Infof("[NkUIDemoEngine] Device OK. VRAM: %llu Mo\n", (unsigned long long)(device->GetCaps().vramBytes >> 20));
    ...
    logger.Infof("[NkUIDemoEngine] Allocating NkUIDemo on heap...\n");
    auto demo = std::make_unique<nkui::NkUIDemo>();
    logger.Infof("[NkUIDemoEngine] NkUIDemo allocated.\n");
    
    // NkUI GPU Renderer
    NkUIRHIRenderer uiRenderer;
    if (!uiRenderer.Init(device, device->GetSwapchainRenderPass(), api)) {
        logger.Infof("[NkUIDemoEngine] NkUIRHIRenderer init failed\n");
        demo->Destroy();
        device->WaitIdle();
    ...
    // STOCKER LE RENDERER POUR LE CALLBACK
    static NkUIRHIRenderer* s_uiRenderer = &uiRenderer;

    // Configurer le loader AVANT l'initialisation
    fontLoader.SetGPUDevice(device);
    fontLoader.SetGPUUploadFunc([](void* device, uint32 texId, const uint8* data, int32 w, int32 h) -> bool {
        if (!s_uiRenderer) {
            logger.Errorf("[FontUpload] No renderer set\n");
            return false;
        }
        logger.Infof("[FontUpload] Uploading font texture %u (%dx%d)\n", texId, w, h);
        return s_uiRenderer->UploadFontTexture(texId, data, w, h);
    });

    nkentseu::nkui::NkUIFontConfig fontConfig;
    fontConfig.yAxisUp = false;        // OpenGL: Y vers le bas
    fontConfig.enableAtlas = true;     // Activer l'atlas
    fontConfig.enableBitmapFallback = true;

    // Initialiser le loader avec le device et le callback
    if (!fontLoader.Init(fontConfig, 8*1024*1024, 2*1024*1024, 512, 512, device, fontLoader.GetGPUUploadFunc())) {
        logger.Errorf("[NkUIDemoEngine] Failed to init font loader\n");
        uiRenderer.Destroy();
        device->WaitIdle();
        NkDeviceFactory::Destroy(device);
        window.Close();
    ...
    // Initialiser le démo (le loader est déjà configuré)
    if (!demo->Init(ctx, wm, fontLoader)) {
        logger.Errorf("[NkUIDemoEngine] NkUIDemo init failed\n");
        uiRenderer.Destroy();
        device->WaitIdle();
        NkDeviceFactory::Destroy(device);
        window.Close();
    ...
    logger.Infof("[NkUIDemoEngine] NkUIDemo initialized.\n");

    // Command buffer
    NkICommandBuffer* cmd = device->CreateCommandBuffer(NkCommandBufferType::NK_GRAPHICS);
    if (!cmd || !cmd->IsValid()) {
        logger.Infof("[NkUIDemoEngine] Command buffer creation failed\n");
        demo->Destroy();
        uiRenderer.Destroy();
        device->WaitIdle();
        NkDeviceFactory::Destroy(device);
    ...
    logger.Infof("[NkUIDemoEngine] Loop started. ESC=exit.\n");
    ...
    while (running) {
        ...
        // Upload de l'atlas si nécessaire (créera la texture GPU via callback)
        ...
        if (frameCount % 300 == 0) {
            ...
            logger.Infof("[NkUIStats] frame=%llu vtx=%u idx=%u cmds=%u\n",
                        (unsigned long long)frameCount, totalVtx, totalIdx, totalCmd);
        }
    ...
    logger.Infof("[NkUIDemoEngine] Clean exit.\n");
    return 0;
}