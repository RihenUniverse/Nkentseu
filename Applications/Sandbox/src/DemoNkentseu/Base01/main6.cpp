...
// Multi-fenAtres - deux fenAtres indApendantes, events distinguAs par windowId
//
// Couvre : NkWindow x2, NkEventSystem multi-fenAtre, NkEvent::GetWindowId(),
//          WM_DESTROY conditionnel (PostQuitMessage seulement si 0 fenAtres),
//          rendu plasma avec couleur de teinte par fenAtre,
...
// ---------------------------------------------------------------------------
// Plasma avec dAcalage de teinte par fenAtre
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Traiter les AvAnements d'une fenAtre donnAe
// ---------------------------------------------------------------------------
...
int nkmain(const nkentseu::NkEntryState& /*state*/)
{
    ...
    // FenAtre A - rouge/plasma chaud, en haut A  gauche
    NkWindowConfig cfgA;
    cfgA.title       = "ex05 - FenAtre A (rouge)";
    cfgA.width       = 640;
    cfgA.height      = 480;
    cfgA.x           = 80;
    cfgA.y           = 100;
    ...
    // FenAtre B - cyan/plasma froid, en bas A  droite
    NkWindowConfig cfgB;
    cfgB.title       = "ex05 - FenAtre B (cyan)";
    cfgB.width       = 640;
    cfgB.height      = 480;
    cfgB.x           = 820;
    cfgB.y           = 300;
    ...
    NkContextApplyWindowHints(cfgA);
    NkContextApplyWindowHints(cfgB);
    ...
    NkContext contextA{};
    NkContext contextB{};
    if (!NkContextCreate(windowA, contextA, &contextConfig) ||
        !NkContextCreate(windowB, contextB, &contextConfig)) {
        NkContextDestroy(contextA);
        NkContextDestroy(contextB);
        windowA.Close();
        windowB.Close();
        NkContextShutdown();
        return -3;
    ...
    // 3. Renderer pour chaque fenAtre
    ...
    wins[0].title     = "A";
    if (!wins[0].renderer.Create(windowA, rcfg)) {
        NkContextDestroy(contextA);
        NkContextDestroy(contextB);
        windowA.Close();
        windowB.Close();
    ...
    wins[1].title     = "B";
    if (!wins[1].renderer.Create(windowB, rcfg)) {
        wins[0].renderer.Shutdown();
        NkContextDestroy(contextA);
        NkContextDestroy(contextB);
        windowA.Close();
        windowB.Close();
    ...
    while (wins[0].open || wins[1].open)
    {
        ...
        while ((ev = es.PollEvent()) != nullptr)
        {
            ...
            // Event sans fenAtre associAe (ex: gamepad connect)
            if (!dispatched) {
        ...
        // VArifier si une fenAtre a AtA fermAe par le bouton X
        for (auto& ws : wins) {
        ...
        // -----------------------------------------------------------------------
        // Rendu de chaque fenAtre ouverte
        // -----------------------------------------------------------------------
        for (auto& ws : wins)
        {
