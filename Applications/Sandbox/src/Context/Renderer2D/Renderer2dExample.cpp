...
static NkGraphicsApi ParseBackend(const NkVector<NkString> &args) {
	...
	return NkGraphicsApi::NK_GFX_API_OPENGLES;
#elif defined(NKENTSEU_PLATFORM_EMSCRIPTEN)
	return NkGraphicsApi::NK_GFX_API_WEBGL;
#else
	return NkGraphicsApi::NK_GFX_API_OPENGL;
#endif
}
...
int nkmain(const NkEntryState &state) {
	...
	if (!gfx && requestedApi != NkGraphicsApi::NK_GFX_API_OPENGL) {
	...
	struct R2DGuard {
			...
			~R2DGuard() {
				if (ptr) {
					NkRenderer2DFactory::Destroy(ptr);
				}
			}
	} r2dGuard{r2dRaw};
	...
	NkTexture playerTex;
	{
		...
		for (uint32 y = 0; y < 64; ++y) {
			for (uint32 x = 0; x < 64; ++x) {
				...
				NkColor2D c = checker ? NkColor2D{100, 180, 255, 255} : NkColor2D{255, 255, 255, 200};
				img.SetPixel(x, y, c);
			}
		}
	...
	if (!hasFont)
		logger.Warn("Font not found — text will be skipped");
	...
	while (running && window.IsOpen()) {
		...
		if (fpsAccum >= 0.5f) {
			...
			snprintf(fpsBuf, sizeof(fpsBuf), "%.0f fps | %s | %ux%u", fps, NkGraphicsApiName(gfx->GetApi()), winW,
					 winH);
			if (hasFont)
				fpsText.SetString(fpsBuf);
			fpsAccum = 0.f;
			fpsFrames = 0;
		}
		...
		float ta = time * 0.8f;
		r2d.DrawFilledTriangle({cx - 300.f + cosf(ta) * 10.f, cy + 150.f}, {cx - 260.f, cy + 70.f},
							   {cx - 220.f + cosf(ta + 1.f) * 10.f, cy + 150.f}, {220, 60, 60, 220});
		r2d.DrawTriangle({cx - 300.f + cosf(ta) * 10.f, cy + 150.f}, {cx - 260.f, cy + 70.f},
						 {cx - 220.f + cosf(ta + 1.f) * 10.f, cy + 150.f}, {255, 120, 120, 255}, 2.f,
						 {255, 120, 120, 255});
		...
		// Ligne animée
		r2d.DrawLine({cx + 320.f, cy + 70.f},
					 {cx + 320.f + 60.f * cosf(time * 3.f), cy + 150.f + 50.f * sinf(time * 3.f)}, {255, 210, 60, 255},
					 3.f);
		...
		switch (gfx->GetApi()) {
			case NkGraphicsApi::NK_GFX_API_VULKAN:
				apiColor = {180, 60, 60, 255};
				break;
			case NkGraphicsApi::NK_GFX_API_DX11:
				apiColor = {60, 120, 200, 255};
				break;
			case NkGraphicsApi::NK_GFX_API_DX12:
				apiColor = {40, 80, 240, 255};
				break;
			case NkGraphicsApi::NK_GFX_API_OPENGL:
				apiColor = {60, 180, 100, 255};
				break;
			case NkGraphicsApi::NK_GFX_API_SOFTWARE:
				apiColor = {160, 100, 40, 255};
				break;
			default:
				break;
		}
