/**
* @File TimeZone.cpp
* @Description Implémentation de la classe TimeZone
*/
...
namespace nkentseu {
    ...
    // Time TimeZone::ToLocal(const Time& utcTime) const {
    ...
    //     return Time(localTm.tm_hour, localTm.tm_min, localTm.tm_sec);
    // }

    // TimeSpan TimeZone::GetUtcOffset(const Date& date) const {
    //     std::tm tm = {0};
    ...
    //     std::mktime(&tm);
    //     return TimeSpan(0, static_cast<int64>(tm.tm_gmtoff / 3600), 0, 0);
    // }

    // bool TimeZone::IsDaylightSavingTime(const Date& date) const {
    //     std::tm tm = {0};
    //     tm.tm_year = date.GetYear() - 1900;
    //     tm.tm_mon = date.GetMonth() - 1;
    //     tm.tm_mday = date.GetDay();
