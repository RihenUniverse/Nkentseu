...
namespace nkentseu {
    ...
    class NKENTSEU_API ConsoleStream : public Stream {
    public:
        enum StreamType { Input, Output, Error };

        ConsoleStream(StreamType type) : m_Type(type) {
    #if defined(NKENTSEU_PLATFORM_WINDOWS)
            m_Handle = GetStdHandle(
                type == Input ? STD_INPUT_HANDLE :
                type == Error ? STD_ERROR_HANDLE : STD_OUTPUT_HANDLE
            );
    #else
            m_Handle = type == Input ? 0 : type == Error ? 2 : 1;
    #endif
        }
        ...
        usize WriteRaw(const void* data, usize byteCount) override {
            ...
            const HANDLE hConsole = GetStdHandle(
                m_Type == Error ? STD_ERROR_HANDLE : STD_OUTPUT_HANDLE
            );
            
            if(m_Encoding == Encoding::UTF16_LE) {
