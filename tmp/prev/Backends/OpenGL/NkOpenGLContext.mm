...
namespace nkentseu {
    ...
    bool NkOpenGLContext::InitNSGL(const NkSurfaceDesc& surf, const NkOpenGLDesc& gl) {
        ...
        if (!view) { logger.Error("[NkOpenGL/NSGL] NSView is null"); return false; }
        ...
        if (!pf) { logger.Error("[NkOpenGL/NSGL] NSOpenGLPixelFormat creation failed"); return false; }
        ...
        if (!ctx) { logger.Error("[NkOpenGL/NSGL] NSOpenGLContext creation failed"); return false; }
        ...
        logger.Info("[NkOpenGL/NSGL] NSGL OK (OpenGL {}.{})", gl.majorVersion, gl.minorVersion);
        return true;
    }
