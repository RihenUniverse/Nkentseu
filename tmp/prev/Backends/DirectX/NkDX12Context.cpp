// =============================================================================
// NkDX12Context.cpp
// Implémentation du contexte Direct3D 12.
//
// DX12 est explicite : toute synchronisation CPU-GPU est manuelle.
// Étapes d'init :
//   1. Debug layer optionnelle
//   2. DXGI Factory6 + choix de l'adaptateur
//   3. D3D12 Device5
//   4. Command Queue (DIRECT)
//   5. Swapchain (DXGI)
//   6. Descriptor Heaps (RTV + DSV)
//   7. Render Target Views
//   8. Depth Buffer
//   9. Command Allocators (un par frame)
//  10. Command List
//  11. Fence + Event (synchronisation)
// Patches :
//   - OnResize 0x0 guard
...
#define NK_DX12_CHECK(hr, msg)                                                                                               \
    do {                                                                                                                     \
        HRESULT _hr = (hr);                                                                                                  \
        if (FAILED(_hr)) {                                                                                                   \
            logger.Errorf("[NkDX12] %s hr=0x%08X", msg, static_cast<unsigned>(_hr));                                        \
            return false;                                                                                                    \
        }                                                                                                                    \
    } while (0)
...
namespace nkentseu {
    ...
    bool NkDX12Context::Initialize(const NkWindow& window, const NkContextDesc& desc) {
        if (mIsValid) { logger.Error("[NkDX12] Already initialized"); return false; }
        mDesc = desc;
        const NkSurfaceDesc surf = window.GetSurfaceDesc();
        if (!surf.IsValid()) { logger.Error("[NkDX12] Invalid NkSurfaceDesc"); return false; }
        ...
        mIsValid = true;
        logger.Info("[NkDX12] Ready - {0} | VRAM {1} MB", mData.renderer, mData.vramMB);
        return true;
    }
    ...
    bool NkDX12Context::CreateDevice(const NkDirectX12Desc& d) {
        ...
        if (d.debugDevice) {
            ...
            if (d.gpuValidation) {
#if defined(__ID3D12Debug1_INTERFACE_DEFINED__) || defined(ID3D12Debug1)
                ComPtr<ID3D12Debug1> debug1;
                if (SUCCEEDED(debug.As(&debug1)) && debug1) {
                    debug1->SetEnableGPUBasedValidation(TRUE);
                }
#else
                logger.Info("[NkDX12] GPU validation requested but ID3D12Debug1 is unavailable with this SDK");
#endif
            }
        }
        ...
        if (!mData.adapter) { logger.Error("[NkDX12] No suitable DX12 adapter"); return false; }
        ...
        if (d.debugDevice) {
#if defined(__ID3D12InfoQueue_INTERFACE_DEFINED__) || defined(ID3D12InfoQueue)
            ComPtr<ID3D12InfoQueue> infoQueue;
            if (SUCCEEDED(mData.device.As(&infoQueue)) && infoQueue) {
                infoQueue->SetBreakOnSeverity(D3D12_MESSAGE_SEVERITY_CORRUPTION, TRUE);
                infoQueue->SetBreakOnSeverity(D3D12_MESSAGE_SEVERITY_ERROR, TRUE);
            }
#endif
        }
        logger.Info("[NkDX12] Device OK (VRAM {0} MB)", mData.vramMB);
        return true;
    }
    ...
    bool NkDX12Context::CreateDescriptorHeaps(const NkDirectX12Desc& d) {
        ...
        rtvHeapDesc.NumDescriptors = d.rtvDescriptorHeapSize;
    ...
    bool NkDX12Context::CreateSyncObjects() {
        ...
        if (!mData.fenceEvent) { logger.Error("[NkDX12] CreateEvent failed"); return false; }
    ...
    void NkDX12Context::Shutdown() {
        ...
        mData.factory.Reset();
        mIsValid = false;
        logger.Info("[NkDX12] Shutdown OK");
    }
    ...
    void NkDX12Context::Present() {
        ...
        if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET) {
            logger.Errorf("[NkDX12] Device lost 0x%08X",
                          static_cast<unsigned>(mData.device->GetDeviceRemovedReason()));
