...
namespace nkentseu {
    ...
    NkVulkanCommandBuffer::NkVulkanCommandBuffer(NkVulkanDevice* dev, NkCommandBufferType type)
        ...
        cpci.queueFamilyIndex = 0; // graphics family
    ...
    void NkVulkanCommandBuffer::BeginRenderPass(NkRenderPassHandle rp,
        ...
        rpbi.renderArea  = {{area.x,area.y},{area.width,area.height}};
    ...
    void NkVulkanCommandBuffer::SetScissor(const NkRect2D& r) {
        VkRect2D sc{{r.x,r.y},{r.width,r.height}};
        vkCmdSetScissor(mCmdBuf,0,1,&sc);
    }
    void NkVulkanCommandBuffer::SetScissors(const NkRect2D* r, uint32 n) {
        std::vector<VkRect2D> v(n);
        for (uint32 i=0;i<n;i++) v[i]={{r[i].x,r[i].y},{r[i].width,r[i].height}};
    ...
    void NkVulkanCommandBuffer::PushConstants(NkShaderStage stages, uint32 offset,
        ...
        if ((uint32)stages & (uint32)NkShaderStage::Vertex)   vkStages |= VK_SHADER_STAGE_VERTEX_BIT;
        if ((uint32)stages & (uint32)NkShaderStage::Fragment) vkStages |= VK_SHADER_STAGE_FRAGMENT_BIT;
        if ((uint32)stages & (uint32)NkShaderStage::Compute)  vkStages |= VK_SHADER_STAGE_COMPUTE_BIT;
    ...
    void NkVulkanCommandBuffer::BindVertexBuffers(uint32 first, const NkBufferHandle* bufs,
        ...
        std::vector<VkBuffer>   vb(n);
        std::vector<VkDeviceSize> vo(n);
    ...
    void NkVulkanCommandBuffer::BindIndexBuffer(NkBufferHandle buf, NkIndexFormat fmt, uint64 off) {
        vkCmdBindIndexBuffer(mCmdBuf, mDev->GetVkBuffer(buf.id), off,
            fmt==NkIndexFormat::Uint16 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32);
    }
    ...
    void NkVulkanCommandBuffer::BlitTexture(NkTextureHandle s, NkTextureHandle d,
        ...
        vkCmdBlitImage(mCmdBuf,
            ...
            1, &blit, filter==NkFilter::Nearest?VK_FILTER_NEAREST:VK_FILTER_LINEAR);
    }
    ...
    void NkVulkanCommandBuffer::Barrier(const NkBufferBarrier* bb, uint32 bc,
        ...
        auto toAccess=[](NkResourceState s, bool write) -> VkAccessFlags {
            switch(s){
                case NkResourceState::UniformBuffer:    return VK_ACCESS_UNIFORM_READ_BIT;
                case NkResourceState::StorageBuffer:
                case NkResourceState::UnorderedAccess:  return write?VK_ACCESS_SHADER_WRITE_BIT:VK_ACCESS_SHADER_READ_BIT;
                case NkResourceState::ShaderRead:       return VK_ACCESS_SHADER_READ_BIT;
                case NkResourceState::RenderTarget:     return VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
                case NkResourceState::DepthWrite:       return VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
                case NkResourceState::TransferSrc:      return VK_ACCESS_TRANSFER_READ_BIT;
                case NkResourceState::TransferDst:      return VK_ACCESS_TRANSFER_WRITE_BIT;
                default:                                return VK_ACCESS_MEMORY_READ_BIT|VK_ACCESS_MEMORY_WRITE_BIT;
            }
        };
        auto toStage=[](NkPipelineStage s) -> VkPipelineStageFlags {
            if ((uint32)s & (uint32)NkPipelineStage::ComputeShader) return VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
            if ((uint32)s & (uint32)NkPipelineStage::Transfer)      return VK_PIPELINE_STAGE_TRANSFER_BIT;
            if ((uint32)s & (uint32)NkPipelineStage::FragmentShader)return VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
            if ((uint32)s & (uint32)NkPipelineStage::VertexShader)  return VK_PIPELINE_STAGE_VERTEX_SHADER_BIT;
        ...
        auto toLayout=[](NkResourceState s) -> VkImageLayout {
            switch(s){
                case NkResourceState::RenderTarget:    return VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
                case NkResourceState::DepthWrite:      return VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
                case NkResourceState::DepthRead:       return VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL;
                case NkResourceState::ShaderRead:      return VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                case NkResourceState::UnorderedAccess: return VK_IMAGE_LAYOUT_GENERAL;
                case NkResourceState::TransferSrc:     return VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
                case NkResourceState::TransferDst:     return VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
                case NkResourceState::Present:         return VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
        ...
        for (uint32 i=0;i<tc;i++) {
            ...
            if (tb[i].stateAfter==NkResourceState::DepthWrite||tb[i].stateAfter==NkResourceState::DepthRead)
