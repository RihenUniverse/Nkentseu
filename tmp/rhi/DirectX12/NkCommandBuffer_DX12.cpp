...
NkCommandBuffer_DX12::NkCommandBuffer_DX12(NkDevice_DX12* dev, NkCommandBufferType type)
    : mDev(dev), mType(type) {
    auto listType = (type == NkCommandBufferType::Compute)
...
void NkCommandBuffer_DX12::SetViewports(const NkViewport* vps, uint32 n) {
    ...
    n = std::min(n, (uint32)D3D12_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE);
...
void NkCommandBuffer_DX12::BindVertexBuffers(uint32 first,
    ...
    n = std::min(n, (uint32)D3D12_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT);
    for (uint32 i = 0; i < n; i++) {
...
void NkCommandBuffer_DX12::BindIndexBuffer(NkBufferHandle buf,
    ...
    ibv.Format         = fmt == NkIndexFormat::Uint16 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
