...
NkCommandBuffer_Metal::NkCommandBuffer_Metal(NkDevice_Metal* dev, NkCommandBufferType type)
    : mDev(dev), mType(type) {
    id<MTLCommandBuffer> cmd = [dev->MtlQueue() commandBuffer];
    mCmdBuf = (__bridge_retained void*)cmd;
}

NkCommandBuffer_Metal::~NkCommandBuffer_Metal() {
    EndCurrentEncoder();
    if (mCmdBuf) CFRelease(mCmdBuf);
}

void NkCommandBuffer_Metal::Begin() {
...
void NkCommandBuffer_Metal::End() { EndCurrentEncoder(); }
void NkCommandBuffer_Metal::Reset() {
    EndCurrentEncoder();
    if (mCmdBuf) CFRelease(mCmdBuf);
    id<MTLCommandBuffer> cmd = [mDev->MtlQueue() commandBuffer];
    mCmdBuf = (__bridge_retained void*)cmd;
...
void NkCommandBuffer_Metal::EndCurrentEncoder() {
...
void NkCommandBuffer_Metal::CommitAndWait() {
    EndCurrentEncoder();
    [CMD_BUF commit];
    [CMD_BUF waitUntilCompleted];
}

void NkCommandBuffer_Metal::CommitAndPresent(void* drawable) {
    EndCurrentEncoder();
    if (drawable) {
...
// Render Pass
// =============================================================================
void NkCommandBuffer_Metal::BeginRenderPass(NkRenderPassHandle rpH,
...
void NkCommandBuffer_Metal::EndRenderPass() {
    if (mRenderEncoder) {
...
// Viewport & Scissor
// =============================================================================
void NkCommandBuffer_Metal::SetViewport(const NkViewport& vp) {
    if (!mRenderEncoder) return;
    MTLViewport v{ vp.x, vp.y, vp.width, vp.height, vp.minDepth, vp.maxDepth };
    [RENDER_ENC setViewport:v];
}

void NkCommandBuffer_Metal::SetViewports(const NkViewport* vps, uint32 n) {
    if (!mRenderEncoder) return;
...
void NkCommandBuffer_Metal::SetScissor(const NkRect2D& r) {
    if (!mRenderEncoder) return;
...
void NkCommandBuffer_Metal::SetScissors(const NkRect2D* r, uint32 /*n*/) {
    SetScissor(r[0]);
}
...
// Pipeline
// =============================================================================
void NkCommandBuffer_Metal::BindGraphicsPipeline(NkPipelineHandle p) {
    if (!mRenderEncoder) return;
...
void NkCommandBuffer_Metal::BindComputePipeline(NkPipelineHandle p) {
    EndCurrentEncoder();
    auto* pipe = mDev->GetPipeline(p.id);
...
// Descriptor Set
// =============================================================================
void NkCommandBuffer_Metal::BindDescriptorSet(NkDescSetHandle set,
                                               uint32 /*idx*/,
...
// =============================================================================
void NkCommandBuffer_Metal::PushConstants(NkShaderStage stages, uint32 /*offset*/,
                                           uint32 size, const void* data) {
    UINT slot = 30; // slot réservé pour les push constants en MSL
    if (!mRenderEncoder && !mComputeEncoder) return;
    bool doVert = (uint32)stages & (uint32)NkShaderStage::Vertex;
    bool doFrag = (uint32)stages & (uint32)NkShaderStage::Fragment;
    bool doComp = (uint32)stages & (uint32)NkShaderStage::Compute;
    if (mRenderEncoder) {
...
// Vertex / Index
// =============================================================================
void NkCommandBuffer_Metal::BindVertexBuffer(uint32 binding, NkBufferHandle buf, uint64 off) {
    if (!mRenderEncoder) return;
...
void NkCommandBuffer_Metal::BindVertexBuffers(uint32 first, const NkBufferHandle* bufs,
                                               const uint64* offs, uint32 n) {
    for (uint32 i = 0; i < n; i++) BindVertexBuffer(first + i, bufs[i], offs[i]);
}

void NkCommandBuffer_Metal::BindIndexBuffer(NkBufferHandle buf, NkIndexFormat fmt, uint64 off) {
    mCurrentIndexBuffer = mDev->GetMTLBuffer(buf.id);
    mIndexOffset  = off;
    mIndexUint32  = (fmt == NkIndexFormat::Uint32);
}

// =============================================================================
// Draw
// =============================================================================
void NkCommandBuffer_Metal::Draw(uint32 vtx, uint32 inst, uint32 firstVtx, uint32 firstInst) {
    if (!mRenderEncoder) return;
...
void NkCommandBuffer_Metal::DrawIndexed(uint32 idx, uint32 inst, uint32 firstIdx,
...
void NkCommandBuffer_Metal::DrawIndirect(NkBufferHandle buf, uint64 off, uint32 cnt, uint32 stride) {
    if (!mRenderEncoder) return;
    id<MTLBuffer> b = (__bridge id<MTLBuffer>)mDev->GetMTLBuffer(buf.id);
...
void NkCommandBuffer_Metal::DrawIndexedIndirect(NkBufferHandle buf, uint64 off, uint32 cnt, uint32 stride) {
    if (!mRenderEncoder || !mCurrentIndexBuffer) return;
    id<MTLBuffer> ib = (__bridge id<MTLBuffer>)mCurrentIndexBuffer;
...
// Compute
// =============================================================================
void NkCommandBuffer_Metal::Dispatch(uint32 gx, uint32 gy, uint32 gz) {
...
void NkCommandBuffer_Metal::DispatchIndirect(NkBufferHandle buf, uint64 off) {
    if (!mComputeEncoder) return;
    auto* enc = (__bridge id<MTLComputeCommandEncoder>)mComputeEncoder;
    id<MTLBuffer> b = (__bridge id<MTLBuffer>)mDev->GetMTLBuffer(buf.id);
...
// Copies
// =============================================================================
void NkCommandBuffer_Metal::CopyBuffer(NkBufferHandle src, NkBufferHandle dst,
...
void NkCommandBuffer_Metal::CopyBufferToTexture(NkBufferHandle src, NkTextureHandle dst,
...
void NkCommandBuffer_Metal::CopyTextureToBuffer(NkTextureHandle src, NkBufferHandle dst,
                                                 const NkBufferTextureCopyRegion& r) {
    EndCurrentEncoder();
    id<MTLBlitCommandEncoder> blit = [CMD_BUF blitCommandEncoder];
...
void NkCommandBuffer_Metal::CopyTexture(NkTextureHandle src, NkTextureHandle dst,
...
void NkCommandBuffer_Metal::BlitTexture(NkTextureHandle src, NkTextureHandle dst,
...
// Mip generation
// =============================================================================
void NkCommandBuffer_Metal::GenerateMipmaps(NkTextureHandle tex, NkFilter /*f*/) {
    EndCurrentEncoder();
    id<MTLBlitCommandEncoder> blit = [CMD_BUF blitCommandEncoder];
...
// Debug
// =============================================================================
void NkCommandBuffer_Metal::BeginDebugGroup(const char* name, float r, float g, float b) {
...
void NkCommandBuffer_Metal::EndDebugGroup() {
    if (mRenderEncoder)  [RENDER_ENC popDebugGroup];
    else                 [CMD_BUF popDebugGroup];
}
void NkCommandBuffer_Metal::InsertDebugLabel(const char* name) {
    NSString* s = [NSString stringWithUTF8String:name];
