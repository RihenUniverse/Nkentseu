...
// =============================================================================
class NkDevice_DX11 final : public NkIDevice {
public:
    NkDevice_DX11()  = default;
    ~NkDevice_DX11() override;
...
private:
    ...
    static DXGI_FORMAT      ToDXGIFormat(NkFormat f, bool uav=false);
    ...
    std::unordered_map<uint64, NkDX11Buffer>       mBuffers;
    std::unordered_map<uint64, NkDX11Texture>      mTextures;
    std::unordered_map<uint64, NkDX11Sampler>      mSamplers;
    std::unordered_map<uint64, NkDX11Shader>       mShaders;
    std::unordered_map<uint64, NkDX11Pipeline>     mPipelines;
    std::unordered_map<uint64, NkDX11RenderPass>   mRenderPasses;
    std::unordered_map<uint64, NkDX11Framebuffer>  mFramebuffers;
    std::unordered_map<uint64, NkDX11DescSetLayout>mDescLayouts;
    std::unordered_map<uint64, NkDX11DescSet>      mDescSets;
    std::unordered_map<uint64, NkDX11FenceObj>     mFences;
