...
NkCommandBuffer_DX11::NkCommandBuffer_DX11(NkDevice_DX11* dev, NkCommandBufferType type)
    : mDev(dev), mType(type) {
...
void NkCommandBuffer_DX11::Execute(NkDevice_DX11* dev) {
...
void NkCommandBuffer_DX11::SetViewports(const NkViewport* vps, uint32 n) {
    ...
    n = std::min(n, (uint32)D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE);
...
void NkCommandBuffer_DX11::BindVertexBuffer(uint32 binding,
    ...
    UINT stride = 0; // le stride est dans le pipeline (InputLayout)
...
void NkCommandBuffer_DX11::BindVertexBuffers(uint32 first,
    ...
    n = std::min(n, (uint32)D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT);
    for (uint32 i = 0; i < n; i++) {
...
void NkCommandBuffer_DX11::BindIndexBuffer(NkBufferHandle buf,
    ...
    DXGI_FORMAT dxFmt = fmt == NkIndexFormat::Uint16
...
void NkCommandBuffer_DX11::GenerateMipmaps(NkTextureHandle tex, NkFilter /*f*/) {
    ...
    mDev->GenerateMipmaps(tex, NkFilter::Linear);
}
