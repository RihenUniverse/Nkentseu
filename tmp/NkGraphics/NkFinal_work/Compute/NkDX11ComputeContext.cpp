// =============================================================================
// NkDX11ComputeContext.cpp — Compute DX11 (CS_5_0, Structured Buffers, UAV)
// =============================================================================
...
#define NK_DX11C_LOG(...) printf("[NkDX11Compute] " __VA_ARGS__)
#define NK_DX11C_ERR(...) printf("[NkDX11Compute][ERROR] " __VA_ARGS__)
...
// ── Buffers (Structured Buffer + UAV + Staging readback) ─────────────────────
struct DX11BufSet {
...
NkComputeBuffer NkDX11ComputeContext::CreateBuffer(const NkComputeBufferDesc& desc) {
    ...
    bd.StructureByteStride = 4; // uint/float de base — l'utilisateur gère le layout dans HLSL
...
NkComputeShader NkDX11ComputeContext::CreateShaderFromFile(
    ...
    std::ifstream f(path);
    if (!f.is_open()) { NK_DX11C_ERR("Cannot open: %s\n", path); return {}; }
...
// ── Pipeline = juste le shader en DX11 ───────────────────────────────────────
...
void NkDX11ComputeContext::WaitIdle() {
    // DX11 est synchrone par défaut — Flush suffit
    mContext->Flush();
}

void NkDX11ComputeContext::MemoryBarrier() {
    // DX11 gère les barrières automatiquement entre Dispatch/Map
}
