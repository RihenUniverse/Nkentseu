...
enum class NkCommandBufferType : uint32 {
    Graphics,   // peut aussi faire compute et copy
    Compute,    // compute + copy uniquement (queue dédiée)
    Transfer,   // copy uniquement (DMA engine)
};
...
public:
    ...
    void TextureBarrier(NkTextureHandle tex,
                        ...
                        NkPipelineStage src=NkPipelineStage::AllCommands,
                        NkPipelineStage dst=NkPipelineStage::AllCommands) {
    ...
    void UAVBarrier(NkBufferHandle buf) {
        NkBufferBarrier b{buf,NkResourceState::UnorderedAccess,
                          NkResourceState::UnorderedAccess};
