/**
* @File NkTimeSpan.cpp
* @Description Implémentation de la classe NkTimeSpan
*/
...
namespace nkentseu {
    ...
    NkTimeSpan::NkTimeSpan(int64 days, int64 hours, int64 minutes, int64 seconds,
        ...
        mTotalNanoseconds = days * 86400LL * 1'000'000'000LL +
    ...
    NkTimeSpan NkTimeSpan::FromDays(int64 days) noexcept {
        return NkTimeSpan(days, 0, 0, 0);
    }

    // Implémentation des accesseurs
    int64 NkTimeSpan::GetDays() const noexcept {
        return mTotalNanoseconds / (86400LL * 1'000'000'000LL);
    }

    // Implémentation des opérations mathématiques
    NkTimeSpan& NkTimeSpan::Add(const NkTimeSpan& other) noexcept {
        mTotalNanoseconds += other.mTotalNanoseconds;
        return *this;
    }

    // Implémentation des opérateurs
    NkTimeSpan NkTimeSpan::operator+(const NkTimeSpan& other) const noexcept {
        return NkTimeSpan(mTotalNanoseconds + other.mTotalNanoseconds);
    }

    // Implémentation des comparaisons
    bool NkTimeSpan::operator==(const NkTimeSpan& other) const noexcept {
        return mTotalNanoseconds == other.mTotalNanoseconds;
    }

    std::string NkTimeSpan::ToString() const {
        const int64 nanoseconds = std::abs(mTotalNanoseconds);
    ...
    NkTime NkTimeSpan::GetTime() const noexcept {
        ...
        int64 adjusted_ns = mTotalNanoseconds % NS_PER_DAY;
