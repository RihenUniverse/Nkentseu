#pragma once
// =============================================================================
// NkVertex.h — Vertex formats 2D / 3D + NkVertexLayout descriptor
// =============================================================================
...
enum class NkAttribFmt : uint32 {
    ...
    NK_UBYTE4N,   // RGBA [0-255] → [0-1]  (couleur vertex packed)
...
struct NkVertexAttrib {
    ...
    uint32      offset       = 0;
    const char* semanticName = nullptr;  // DX : "POSITION", "NORMAL" etc.
...
struct NkVertexLayout {
    ...
    NkVertexLayout& Attrib(uint32 loc, uint32 bind, NkAttribFmt fmt, uint32 off,
...
struct alignas(4) NkVertex2D {
    ...
    NkVertex2D(float32 x, float32 y, float32 u, float32 v, NkColorF c = NkColorF::White())
...
static_assert(sizeof(NkVertex2D)==20,"NkVertex2D size");
...
static_assert(sizeof(NkVertex3D)==48,"NkVertex3D size");

// =============================================================================
// NkVertexSkinned — animation squelettique
// Hérite du layout 3D + boneIdx(4×uint8) + boneWeight(4f) = 68 bytes
// =============================================================================
...
static_assert(sizeof(NkVertexSkinned)==68,"NkVertexSkinned size");

// =============================================================================
// NkVertexPosOnly — depth/shadow pass
// 12 bytes
// =============================================================================
struct alignas(4) NkVertexPosOnly {
    ...
    static NkVertexLayout Layout() {
        NkVertexLayout l;
        l.Attrib(0,0,NkAttribFmt::NK_FLOAT3,0,"POSITION",0).Bind(0,12);
        return l;
    }
};
static_assert(sizeof(NkVertexPosOnly)==12,"NkVertexPosOnly size");

// =============================================================================
// NkVertexDebug — lignes/axes/wireframe
// pos(3f)+color(uint) = 16 bytes
// =============================================================================
struct alignas(4) NkVertexDebug {
    float32 px=0,py=0,pz=0;
    uint32  color = 0xFFFFFFFF;
    NkVertexDebug()=default;
    NkVertexDebug(float32 x,float32 y,float32 z,NkColorF c=NkColorF::White())
...
static_assert(sizeof(NkVertexDebug)==16,"NkVertexDebug size");

// =============================================================================
// NkVertexParticle — billboard / particule 2D dans espace 3D
// pos(3f)+uv(2f)+color(uint)+size(f)+rotation(f) = 32 bytes
// =============================================================================
struct alignas(4) NkVertexParticle {
    ...
    static NkVertexLayout Layout() {
        ...
        l.Attrib(0,0,NkAttribFmt::NK_FLOAT3, 0,  "POSITION",0)
         ...
         .Attrib(3,0,NkAttribFmt::NK_FLOAT2, 24, "TEXCOORD",1) // size+rotation
         .Bind(0, sizeof(NkVertexParticle));
        return l;
    }
};
static_assert(sizeof(NkVertexParticle)==32,"NkVertexParticle size");
