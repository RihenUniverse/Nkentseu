...
public:
    ...
    // ── Semaphores (GLsync-based) ─────────────────────────────────────────────
    NkSemaphoreHandle CreateGpuSemaphore()                          override;
    void              DestroySemaphore(NkSemaphoreHandle& h)     override;

    // ── Friend : fonctions internes partagées (NkOpenGLDeviceInternal.cpp) ────
    friend GLuint NkOpenglGetBufferID (NkOpenGLDevice*, uint64);
    friend GLuint NkOpenglGetTextureID(NkOpenGLDevice*, uint64);
    friend GLuint NkOpenglGetFBOID    (NkOpenGLDevice*, uint64);
    friend GLuint NkOpenglGetSamplerID(NkOpenGLDevice*, uint64);
    friend GLuint NkOpenglGetProgramID(NkOpenGLDevice*, uint64);
    friend GLuint NkOpenglGetVAOID    (NkOpenGLDevice*, uint64);
    friend GLenum NkOpenglGetPrimitive(NkOpenGLDevice*, uint64);
    friend bool     NkOpenglIsCompute   (NkOpenGLDevice*, uint64);
    friend void     NkOpenglApplyRenderState(NkOpenGLDevice*, uint64);
    friend void     NkOpenglApplyDescSet(NkOpenGLDevice*, uint64, const NkVector<uint32>&);
    friend GLsizei  NkOpenglGetPipelineBindingStride(NkOpenGLDevice*, uint64, uint32);
...
private:
    ...
    // Semaphore GL — GLsync en mode GPU-GPU
    struct GLSemaphoreObj { GLsync sync=nullptr; };
    ...
    NkUnorderedMap<uint64, GLSemaphoreObj>  mSemaphores;
