...
extern GLsizei  NkOpenglGetPipelineBindingStride(NkOpenGLDevice* dev, uint64 pipeId, uint32 binding);
...
void NkOpenGLCommandBuffer::GL_BindVertexBuffer(uint32 binding,
    ...
    // Récupérer le stride depuis le vertex layout du pipeline courant
    GLsizei stride = NkOpenglGetPipelineBindingStride(mDev, mBoundPipeline.id, binding);
    glVertexArrayVertexBuffer(mCurrentVAO, (GLuint)binding, bufId, (GLintptr)off, stride);
}
