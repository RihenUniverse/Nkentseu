...
void NkVulkanCommandBuffer::BeginRenderPass(NkRenderPassHandle rp,
    ...
    rpbi.renderArea  = {{area.x,area.y},{(uint32_t)area.width,(uint32_t)area.height}};
...
void NkVulkanCommandBuffer::SetViewports(const NkViewport* vps, uint32 n) {
    NkVector<VkViewport> v; v.Resize(n);
...
void NkVulkanCommandBuffer::SetScissor(const NkRect2D& r) {
    VkRect2D sc{{r.x,r.y},{(uint32_t)r.width,(uint32_t)r.height}};
    vkCmdSetScissor(mCmdBuf,0,1,&sc);
}
void NkVulkanCommandBuffer::SetScissors(const NkRect2D* r, uint32 n) {
    NkVector<VkRect2D> v; v.Resize(n);
    for (uint32 i=0;i<n;i++) v[i]={{r[i].x,r[i].y},{(uint32_t)r[i].width,(uint32_t)r[i].height}};
...
void NkVulkanCommandBuffer::BindVertexBuffers(uint32 first, const NkBufferHandle* bufs,
    ...
    NkVector<VkBuffer> vb; vb.Resize(n);
    NkVector<VkDeviceSize> vo; vo.Resize(n);
...
void NkVulkanCommandBuffer::Barrier(const NkBufferBarrier* bb, uint32 bc,
    ...
    vkCmdPipelineBarrier(mCmdBuf, srcStage, dstStage, 0,
        0, nullptr,
        (uint32)bufBarriers.Size(), bufBarriers.IsEmpty()?nullptr:bufBarriers.Data(),
        (uint32)imgBarriers.Size(), imgBarriers.IsEmpty()?nullptr:imgBarriers.Data());
}
