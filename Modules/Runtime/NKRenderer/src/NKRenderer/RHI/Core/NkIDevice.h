...
public:
    ...
    // =========================================================================
    // Créer un swapchain lié à ce device et au contexte graphique fourni.
    // Le device ne possède pas de swapchain interne — l'application en crée
    // autant qu'elle veut (ex: multi-fenêtre).
    virtual NkISwapchain* CreateSwapchain(NkIGraphicsContext* ctx,
                                           const NkSwapchainDesc& desc)      = 0;
    virtual void          DestroySwapchain(NkISwapchain*& sc)                 = 0;
...
#endif
    virtual NkSemaphoreHandle CreateGpuSemaphore()                               = 0;
    virtual void              DestroySemaphore(NkSemaphoreHandle& handle)     = 0;
