// bloom_downsample.frag.gl.glsl — Bloom Downsample (OpenGL 4.6)
#version 460 core
layout(location=0) in vec2 vUV; layout(location=0) out vec4 fragColor;
layout(binding=0) uniform sampler2D tSrc;
layout(std140,binding=1) uniform BloomUBO{float threshold;float bloomStrength;float upsampleRadius;float _p;};
