// colorgrade.frag.gl.glsl — NKRenderer v4.0 — Color Grading (OpenGL 4.6)
#version 460 core
layout(location=0) in vec2 vUV; layout(location=0) out vec4 fragColor;
layout(binding=0) uniform sampler2D tLDR;
layout(binding=1) uniform sampler3D tLUT;    // 3D LUT 32x32x32
layout(std140,binding=2) uniform ColorGradeUBO{
