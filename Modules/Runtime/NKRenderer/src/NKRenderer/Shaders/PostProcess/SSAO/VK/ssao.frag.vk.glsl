// ssao.frag.vk.glsl — NKRenderer v4.0 — SSAO (Vulkan)
#version 460 core
#extension GL_ARB_separate_shader_objects : enable
layout(location=0) in vec2 vUV; layout(location=0) out float fragAO;
layout(set=1,binding=0) uniform sampler2D tDepth;
layout(set=1,binding=1) uniform sampler2D tNoise;    // 4x4 random rotation
