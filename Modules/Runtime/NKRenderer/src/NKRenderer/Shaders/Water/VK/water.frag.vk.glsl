// water.frag.vk.glsl — NKRenderer v4.0 — Water Fragment (Vulkan)
#version 460 core
...
layout(set=0,binding=10,std140) uniform WaterUBO{vec4 waveDir[4];float speed,foamThreshold,refractionStrength,_p;}uWater;
layout(set=1,binding=4) uniform sampler2D tNormal1;      // flowing normal map 1
layout(set=1,binding=5) uniform sampler2D tNormal2;      // flowing normal map 2 (different scale/speed)
layout(set=1,binding=6) uniform sampler2D tFoam;         // foam texture
layout(set=1,binding=7) uniform sampler2D tSceneColor;   // refraction (scene behind water)
layout(set=1,binding=8) uniform samplerCube tEnvPrefilter;// reflection
layout(set=1,binding=9) uniform sampler2D tDepth;        // for depth-based foam
void main(){
