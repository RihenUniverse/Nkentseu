...
void NkVulkanComputeContext::InitFromGraphicsContext(NkIGraphicsContext* gfx) {
    ...
    if (gfx->GetApi() != NkGraphicsApi::NK_API_VULKAN) return;
    auto* vkData = static_cast<NkVulkanContextData*>(gfx->GetNativeContextData());
    if (!vkData || vkData->physicalDevice == VK_NULL_HANDLE) return;
    ...
    if (mData.computeFamily == UINT32_MAX) {
        NK_VKC_ERR("No compute queue family available in Vulkan context data\n");
        return;
    }
    ...
    qci.queueFamilyIndex = mData.computeFamily;
    qci.queueCount       = 1;
    ...
    if (vkCreateDevice(mData.physicalDevice, &dci, nullptr, &mData.device) != VK_SUCCESS) {
        NK_VKC_ERR("vkCreateDevice failed\n");
        return;
    }
    mData.ownsDevice = true;

    vkGetDeviceQueue(mData.device, mData.computeFamily, 0, &mData.computeQueue);
    if (!mData.computeQueue) {
        NK_VKC_ERR("vkGetDeviceQueue failed for compute family\n");
        Shutdown();
        return;
    }
...
void NkVulkanComputeContext::Shutdown() {
    if (!mIsValid &&
        mData.device == VK_NULL_HANDLE &&
        mData.commandPool == VK_NULL_HANDLE &&
    ...
    if (mData.device != VK_NULL_HANDLE) {
    ...
    if (mData.ownsDevice && mData.device) {
