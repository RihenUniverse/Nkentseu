...
void NkSLSemantic::AnalyzeProgram(NkSLProgramNode* prog) {
    ...
    for (auto* child : prog->children) {
        if (!child) continue;
        if (child->kind == NkSLNodeKind::DECL_STRUCT) {
            auto* s = static_cast<NkSLStructDeclNode*>(child);
            NkSLSymbol sym;
            sym.name = s->name;
            sym.kind = NkSLSymbolKind::STRUCT_TYPE;
            sym.structDecl = s;
            mSymbols.Define(sym);
        } else if (child->kind == NkSLNodeKind::DECL_FUNCTION) {
            auto* fn = static_cast<NkSLFunctionDeclNode*>(child);
            NkSLSymbol* existing = mSymbols.Resolve(fn->name);
            if (!existing) {
                NkSLSymbol sym;
                sym.name = fn->name;
                sym.kind = NkSLSymbolKind::FUNCTION;
                mSymbols.Define(sym);
...
void NkSLSemantic::AnalyzeStructDecl(NkSLStructDeclNode* s) {
    ...
    for (auto* m : s->members) {
        ...
        if (m->type && m->type->baseType == NkSLBaseType::STRUCT) {
...
void NkSLSemantic::AnalyzeBlockDecl(NkSLBlockDeclNode* b) {
    ...
    if (!b->instanceName.Empty()) {
        ...
        sym.kind = NkSLSymbolKind::VARIABLE;
        sym.type.base     = NkSLBaseType::STRUCT;
...
void NkSLSemantic::AnalyzeVarDecl(NkSLVarDeclNode* v, bool isGlobal) {
    ...
    // Vérifier que le type existe
    if (v->type && v->type->baseType == NkSLBaseType::STRUCT) {
    ...
    if (isGlobal) {
        switch (v->storage) {
            case NkSLStorageQual::IN:
                mInputVars.PushBack(v);
                break;
            case NkSLStorageQual::OUT:
    ...
    if (isGlobal && !v->isConst &&
        v->storage == NkSLStorageQual::NONE &&
    ...
    if (v->initializer) {
        ...
        if (initType.base != NkSLBaseType::UNKNOWN &&
            declType.base != NkSLBaseType::UNKNOWN) {
...
void NkSLSemantic::AnalyzeFunction(NkSLFunctionDeclNode* fn) {
    ...
    for (auto* p : fn->params) {
        ...
        NkSLSymbol sym;
        sym.name    = p->name;
        sym.kind    = NkSLSymbolKind::PARAMETER;
    ...
    switch (node->kind) {
        ...
        case NkSLNodeKind::STMT_DO_WHILE:
            ...
            if (node->children.Size() > 1) {
                ...
                if (!ct.IsVoid() && ct.base != NkSLBaseType::BOOL && ct.IsNumeric())
...
void NkSLSemantic::AnalyzeIf(NkSLIfNode* n) {
    if (n->condition) {
        ...
        if (!ct.IsVoid() && ct.base != NkSLBaseType::BOOL) {
...
void NkSLSemantic::AnalyzeReturn(NkSLReturnNode* n) {
    ...
    if (n->value) {
        ...
        if (!expected.IsVoid() && actual.base != NkSLBaseType::UNKNOWN) {
...
void NkSLSemantic::AnalyzeSwitch(NkSLNode* n) {
    ...
    if (!n->children.Empty()) {
        ...
        if (ct.base != NkSLBaseType::INT && ct.base != NkSLBaseType::UINT)
...
NkString NkSLSemantic::TypeName(const NkSLResolvedType& t) {
    if (t.base == NkSLBaseType::STRUCT) return t.typeName;
    switch (t.base) {
...
NkSLResolvedType NkSLSemantic::TypeOf(NkSLNode* node) {
    ...
    switch (node->kind) {
        case NkSLNodeKind::EXPR_LITERAL:  return TypeOfLiteral(static_cast<NkSLLiteralNode*>(node));
        case NkSLNodeKind::EXPR_IDENT:    return TypeOfIdent  (static_cast<NkSLIdentNode*>(node));
        case NkSLNodeKind::EXPR_UNARY:    return TypeOfUnary  (static_cast<NkSLUnaryNode*>(node));
        case NkSLNodeKind::EXPR_BINARY:   return TypeOfBinary (static_cast<NkSLBinaryNode*>(node));
        case NkSLNodeKind::EXPR_TERNARY:  return TypeOfTernary(node);
        case NkSLNodeKind::EXPR_CALL:     return TypeOfCall   (static_cast<NkSLCallNode*>(node));
        case NkSLNodeKind::EXPR_MEMBER:   return TypeOfMember (static_cast<NkSLMemberNode*>(node));
        case NkSLNodeKind::EXPR_INDEX:    return TypeOfIndex  (static_cast<NkSLIndexNode*>(node));
        case NkSLNodeKind::EXPR_ASSIGN:   return TypeOfAssign (static_cast<NkSLAssignNode*>(node));
        case NkSLNodeKind::EXPR_CAST:     return TypeOfCast   (static_cast<NkSLCastNode*>(node));
        case NkSLNodeKind::STMT_EXPR:
            return node->children.Empty() ? NkSLResolvedType::Void() : TypeOf(node->children[0]);
        default: return NkSLResolvedType{NkSLBaseType::UNKNOWN};
    }
}
...
NkSLResolvedType NkSLSemantic::TypeOfIdent(NkSLIdentNode* id) {
    ...
    if (!sym) {
        ...
        return {NkSLBaseType::UNKNOWN};
    }
    if (sym->kind == NkSLSymbolKind::FUNCTION || sym->kind == NkSLSymbolKind::BUILTIN_FUNC)
        return NkSLResolvedType{NkSLBaseType::UNKNOWN}; // fonction, pas un type
    return sym->type;
}
...
NkSLResolvedType NkSLSemantic::TypeOfBinary(NkSLBinaryNode* b) {
    ...
    if (lhs.base == NkSLBaseType::UNKNOWN || rhs.base == NkSLBaseType::UNKNOWN)
        return {NkSLBaseType::UNKNOWN};

    // Vérifications spécifiques
    if (b->op == "/" && rhs.base == NkSLBaseType::INT)
        Warning(b->line, "Integer division may truncate");

    if ((b->op == "%" || b->op == "&" || b->op == "|" || b->op == "^" ||
         b->op == "<<" || b->op == ">>") &&
        (lhs.base == NkSLBaseType::FLOAT || lhs.base == NkSLBaseType::VEC2 ||
         lhs.base == NkSLBaseType::VEC3  || lhs.base == NkSLBaseType::VEC4)) {
...
bool NkSLSemantic::AreCompatibleForOp(const NkString& op,
    ...
    if (op == "*") {
        // matrix * vector
        if (lhs.base == NkSLBaseType::MAT4 && rhs.base == NkSLBaseType::VEC4) return true;
        if (lhs.base == NkSLBaseType::MAT3 && rhs.base == NkSLBaseType::VEC3) return true;
        if (lhs.base == NkSLBaseType::MAT2 && rhs.base == NkSLBaseType::VEC2) return true;
...
NkSLResolvedType NkSLSemantic::TypeOfTernary(NkSLNode* n) {
    if (n->children.Size() < 3) return {NkSLBaseType::UNKNOWN};
...
NkSLResolvedType NkSLSemantic::TypeOfCall(NkSLCallNode* call) {
    ...
    if (sym && sym->kind == NkSLSymbolKind::STRUCT_TYPE) {
        ...
        if (s && (uint32)call->args.Size() != (uint32)s->members.Size())
            Warning(call->line, "Struct constructor argument count mismatch for '" +
                    call->callee + "'");
        NkSLResolvedType rt; rt.base = NkSLBaseType::STRUCT; rt.typeName = call->callee;
        return rt;
    }

    // Constructeur de type primitif
    static const struct { const char* name; NkSLBaseType t; } kConstructors[] = {
        {"float",NkSLBaseType::FLOAT},{"int",NkSLBaseType::INT},{"uint",NkSLBaseType::UINT},
        {"bool",NkSLBaseType::BOOL},
        {"vec2",NkSLBaseType::VEC2},{"vec3",NkSLBaseType::VEC3},{"vec4",NkSLBaseType::VEC4},
        {"ivec2",NkSLBaseType::IVEC2},{"ivec3",NkSLBaseType::IVEC3},{"ivec4",NkSLBaseType::IVEC4},
        {"uvec2",NkSLBaseType::UVEC2},{"uvec3",NkSLBaseType::UVEC3},{"uvec4",NkSLBaseType::UVEC4},
        {"mat2",NkSLBaseType::MAT2},{"mat3",NkSLBaseType::MAT3},{"mat4",NkSLBaseType::MAT4},
        {"mat2x2",NkSLBaseType::MAT2},{"mat2x3",NkSLBaseType::MAT2X3},
        {"mat2x4",NkSLBaseType::MAT2X4},{"mat3x2",NkSLBaseType::MAT3X2},
        {"mat3x4",NkSLBaseType::MAT3X4},{"mat4x2",NkSLBaseType::MAT4X2},
        {"mat4x3",NkSLBaseType::MAT4X3},
        {nullptr, NkSLBaseType::UNKNOWN}
    };
...
NkSLResolvedType NkSLSemantic::TypeOfMember(NkSLMemberNode* m) {
    ...
    if (base.base == NkSLBaseType::UNKNOWN) return base;

    // Swizzle sur vecteur
    if (base.IsVector() || base.base == NkSLBaseType::FLOAT) {
        return ValidateSwizzle(base, m->member, m->line);
    }

    // Accès membre sur struct
    if (base.base == NkSLBaseType::STRUCT) {
...
NkSLResolvedType NkSLSemantic::ValidateSwizzle(const NkSLResolvedType& base,
    ...
    if (n == 1) { /* scalar */ }
        ...
        if (base.ScalarBase()==NkSLBaseType::FLOAT)  result.base = NkSLBaseType::VEC2;
        if (base.ScalarBase()==NkSLBaseType::INT)    result.base = NkSLBaseType::IVEC2;
        if (base.ScalarBase()==NkSLBaseType::UINT)   result.base = NkSLBaseType::UVEC2;
    } else if (n == 3) {
        if (base.ScalarBase()==NkSLBaseType::FLOAT)  result.base = NkSLBaseType::VEC3;
        if (base.ScalarBase()==NkSLBaseType::INT)    result.base = NkSLBaseType::IVEC3;
        if (base.ScalarBase()==NkSLBaseType::UINT)   result.base = NkSLBaseType::UVEC3;
    } else if (n == 4) {
        if (base.ScalarBase()==NkSLBaseType::FLOAT)  result.base = NkSLBaseType::VEC4;
        if (base.ScalarBase()==NkSLBaseType::INT)    result.base = NkSLBaseType::IVEC4;
        if (base.ScalarBase()==NkSLBaseType::UINT)   result.base = NkSLBaseType::UVEC4;
    } else {
...
NkSLResolvedType NkSLSemantic::TypeOfIndex(NkSLIndexNode* idx) {
    ...
    if (idxType.base != NkSLBaseType::INT && idxType.base != NkSLBaseType::UINT)
    ...
    if (arrType.IsMatrix()) {
        switch (arrType.base) {
            case NkSLBaseType::MAT2: return NkSLResolvedType::Vec2();
            case NkSLBaseType::MAT3: return NkSLResolvedType::Vec3();
            case NkSLBaseType::MAT4: return NkSLResolvedType::Vec4();
...
NkSLResolvedType NkSLSemantic::TypeOfAssign(NkSLAssignNode* a) {
    ...
    if (a->lhs->kind == NkSLNodeKind::EXPR_LITERAL)
    ...
    if (!NkSLSymbolTable::IsAssignable(rhs, lhs) &&
        lhs.base != NkSLBaseType::UNKNOWN && rhs.base != NkSLBaseType::UNKNOWN)
...
void NkSLSemantic::CheckEntryPoints() {
    ...
    for (auto* ep : mEntryPoints) {
        if (ep->stage != mStage && ep->stage != NkSLStage::VERTEX) {
    ...
    if (mStage == NkSLStage::FRAGMENT && mOutputVars.Empty()) {
