...
// =============================================================================
NkString NkSLCodeGen_CPP::BaseTypeToCPP(NkSLBaseType t) {
    switch (t) {
        case NkSLBaseType::VOID:   return "void";
        case NkSLBaseType::BOOL:   return "bool";
        case NkSLBaseType::INT:    return "int32_t";
        case NkSLBaseType::IVEC2:  return "NkVec2i";
        case NkSLBaseType::IVEC3:  return "NkVec3i";
        case NkSLBaseType::IVEC4:  return "NkVec4i";
        case NkSLBaseType::UINT:   return "uint32_t";
        case NkSLBaseType::UVEC2:  return "NkVec2u";
        case NkSLBaseType::UVEC3:  return "NkVec3u";
        case NkSLBaseType::UVEC4:  return "NkVec4u";
        case NkSLBaseType::FLOAT:  return "float";
        case NkSLBaseType::VEC2:   return "NkVec2f";
        case NkSLBaseType::VEC3:   return "NkVec3f";
        case NkSLBaseType::VEC4:   return "NkVec4f";
        case NkSLBaseType::DOUBLE: return "double";
        case NkSLBaseType::DVEC2:  return "NkVec2d";
        case NkSLBaseType::DVEC3:  return "NkVec3d";
        case NkSLBaseType::DVEC4:  return "NkVec4d";
        case NkSLBaseType::MAT2:   return "NkMat2f";
        case NkSLBaseType::MAT3:   return "NkMat3f";
        case NkSLBaseType::MAT4:   return "NkMat4f";
        // Pour les samplers en CPU: pointeur vers une texture software
        case NkSLBaseType::SAMPLER2D:
        case NkSLBaseType::SAMPLER2D_SHADOW:
        case NkSLBaseType::SAMPLER2D_ARRAY:
            return "const NkSWTexture2D*";
        case NkSLBaseType::SAMPLER_CUBE:
            return "const NkSWTextureCube*";
        default: return "float";
    }
}

NkString NkSLCodeGen_CPP::TypeToCPP(NkSLTypeNode* t) {
    if (!t) return "void";
...
NkString NkSLCodeGen_CPP::IntrinsicToCPP(const NkString& name) {
...
// =============================================================================
NkSLCompileResult NkSLCodeGen_CPP::Generate(NkSLProgramNode* ast,
                                             NkSLStage stage,
    ...
    res.source  = mOutput;
    res.target  = NkSLTarget::CPLUSPLUS;
    res.stage   = stage;
    res.errors  = mErrors;
...
void NkSLCodeGen_CPP::GenProgram(NkSLProgramNode* prog) {
    ...
    EmitLine("#include \"NKRenderer/RHI/Backend/Software/NkSWTypes.h\"");
    EmitNewLine();
    ...
    for (auto* child : prog->children) {
        if (!child) continue;
        if (child->kind == NkSLNodeKind::DECL_UNIFORM_BLOCK ||
            child->kind == NkSLNodeKind::DECL_PUSH_CONSTANT) {
            if (!hasUniforms) {
...
void NkSLCodeGen_CPP::GenFunction(NkSLFunctionDeclNode* fn, bool asEntry) {
    if (asEntry) {
        // Entry point : génère une fonction statique
        NkString stageName = (mStage == NkSLStage::VERTEX)  ? "vertex"   :
                             (mStage == NkSLStage::FRAGMENT) ? "fragment" : "compute";
    ...
    for (uint32 i = 0; i < (uint32)fn->params.Size(); i++) {
        auto* p = fn->params[i];
        if (i > 0) sig += ", ";
        if (p->storage == NkSLStorageQual::OUT || p->storage == NkSLStorageQual::INOUT) {
            sig += TypeToCPP(p->type) + "& " + p->name;
        } else if (NkSLTypeIsMatrix(p->type->baseType) || p->type->baseType == NkSLBaseType::STRUCT) {
...
void NkSLCodeGen_CPP::GenStmt(NkSLNode* node) {
    if (!node) return;
    switch (node->kind) {
...
NkString NkSLCodeGen_CPP::GenExpr(NkSLNode* node) {
    if (!node) return "";
    switch (node->kind) {
...
NkString NkSLCodeGen_CPP::GenCall(NkSLCallNode* call) {
...
// LiteralToStr C++
// =============================================================================
NkString NkSLCodeGen_CPP::LiteralToStr(NkSLLiteralNode* lit) {
    char buf[64];
    switch (lit->baseType) {
        ...
        case NkSLBaseType::FLOAT:  snprintf(buf,sizeof(buf),"%.8gf",lit->floatVal); return buf;
        case NkSLBaseType::DOUBLE: snprintf(buf,sizeof(buf),"%.16g",lit->floatVal); return buf;
