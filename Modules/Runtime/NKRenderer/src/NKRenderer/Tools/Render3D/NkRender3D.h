    ...
    namespace renderer {
        ...
        class NkRender3D {
           public:
                ...
                void DrawLine    (const NkVec3f& a, const NkVec3f& b,
                ...
                void DrawSkeleton(const NkMat4f* boneMatrices, uint32 boneCount,
                                   const int32* parentIndices,
                                   const NkColorF& c = NkColorF::Yellow());

                // Clore et exécuter la scène
                void EndScene(NkICommandBuffer* cmd,
                               const NkRenderPass3DInfo& pass = {});

                // ── Rendu direct (sans RenderGraph, pour cas spéciaux) ────────────────
                void RenderDirect(NkICommandBuffer* cmd,
                                   const NkSceneContext3D& ctx,
                                   const NkVector<NkDrawCall3D>& drawCalls);
