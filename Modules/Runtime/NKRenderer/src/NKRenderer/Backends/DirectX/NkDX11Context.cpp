...
#define NK_DX11_CHECK(hr, msg)                                                                                               \
    do {                                                                                                                     \
        HRESULT _hr = (hr);                                                                                                  \
        if (FAILED(_hr)) {                                                                                                   \
            logger.Errorf("[NkDX11] %s hr=0x%08X", msg, static_cast<unsigned>(_hr));                                        \
            return false;                                                                                                    \
        }                                                                                                                    \
    } while (0)
...
namespace nkentseu {
    ...
    bool NkDX11Context::Initialize(const NkWindow& window, const NkContextDesc& desc) {
        if (mIsValid) { logger.Error("[NkDX11] Already initialized"); return false; }
        mDesc = desc;
        const NkSurfaceDesc surf = window.GetSurfaceDesc();
        if (!surf.IsValid()) { logger.Error("[NkDX11] Invalid NkSurfaceDesc"); return false; }
        ...
        mIsValid = true;
        logger.Info("[NkDX11] Ready - {0} | VRAM {1} MB", mData.renderer, mData.vramMB);
        return true;
    }
    ...
    bool NkDX11Context::CreateDeviceAndSwapchain(const NkContextDesc& d, HWND hwnd) {
        ...
        if (FAILED(hr)) {
            ...
            logger.Info("[NkDX11] Hardware device failed, trying WARP");
        ...
        logger.Info("[NkDX11] Device+Swapchain OK (feature level {0}, VRAM {1} MB)",
                    static_cast<unsigned>(chosenLevel), mData.vramMB);
        return true;
    }
    ...
    void NkDX11Context::HandleDeviceLost() {
        logger.Error("[NkDX11] Device lost - attempting recovery");
    ...
    void NkDX11Context::Shutdown() {
        ...
        mData.device.Reset();
        mIsValid = false;
        logger.Info("[NkDX11] Shutdown OK");
    }
    ...
    void NkDX11Context::Present() {
        ...
        if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET)
            ...
            logger.Errorf("[NkDX11] Present failed 0x%08X", static_cast<unsigned>(hr));
    }
