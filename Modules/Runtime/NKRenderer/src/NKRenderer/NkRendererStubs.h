...
namespace nkentseu {
    ...
    #define NK_STUB_RENDERER_IMPL(ClassName, ApiEnum, ApiStr, HwAccel)                                 \
        class ClassName : public NkIRendererImpl {                                                     \
            public:                                                                                        \
                ClassName() = default;                                                                     \
                ~ClassName() override = default;                                                           \
                bool Init(const NkRendererConfig& c, const NkSurfaceDesc& s) override {                   \
                    mConfig = c; mSurface = s; mReady = true; return true;                                \
                }                                                                                          \
                void Shutdown() override { mReady = false; }                                               \
                bool IsValid()               const override { return mReady; }                            \
                const char* GetName()        const override { return ApiStr; }                            \
                bool IsHardwareAccelerated() const override { return HwAccel; }                           \
