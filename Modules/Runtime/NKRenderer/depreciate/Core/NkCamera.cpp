...
// Helper indépendant des #includes internes math
namespace {
    ...
    static NkMat4 PerspVK(float32 fovY, float32 aspect,
                           float32 n, float32 f) noexcept {
        float32 tanHalf = math::Tan(math::DegToRad(fovY) * 0.5f);
        NkMat4 m;
    ...
    static NkMat4 PerspGL(float32 fovY, float32 aspect,
                           float32 n, float32 f) noexcept {
        float32 tanHalf = math::Tan(math::DegToRad(fovY) * 0.5f);
        NkMat4 m;
        m.col[0] = {1.f/(aspect*tanHalf), 0,           0,              0};
    ...
    static NkMat4 LookAtRH(NkVec3 eye, NkVec3 center, NkVec3 up) noexcept {
        NkVec3 f = (center - eye).Normalize();
        NkVec3 s = f.Cross(up).Normalize();
        NkVec3 u = s.Cross(f);
...
namespace nkentseu {
    ...
    void NkCamera2D::RebuildIfDirty() const noexcept {
        ...
        float32 rad = math::DegToRad(mRotation);
        float32 cosR = math::Cos(rad);
        float32 sinR = math::Sin(rad);
        NkMat4 rot;
    ...
    NkVec3 NkCamera3D::GetForward() const noexcept {
        return (mTarget - mPosition).Normalize();
    }

    NkVec3 NkCamera3D::GetRight() const noexcept {
        return GetForward().Cross(mUp).Normalize();
    }
