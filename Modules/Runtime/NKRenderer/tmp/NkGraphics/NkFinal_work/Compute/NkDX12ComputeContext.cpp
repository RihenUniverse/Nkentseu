// =============================================================================
// NkDX12ComputeContext.cpp — DX12 Compute
// =============================================================================
...
#define NK_DX12C_LOG(...) printf("[NkDX12Compute] " __VA_ARGS__)
#define NK_DX12C_ERR(...) printf("[NkDX12Compute][ERROR] " __VA_ARGS__)
...
// Nombre max de UAV bindables (root signature simplifiée)
...
void NkDX12ComputeContext::InitFromGraphicsContext(NkIGraphicsContext* gfx) {
    ...
    // Command objects dédiés compute
    ...
    // Root signature simplifiée : kMaxUAV UAV (unbounded table)
...
bool NkDX12ComputeContext::ReadBuffer(const NkComputeBuffer& buf,
    ...
    // Copy GPU → readback via command list
    mAllocator->Reset();
...
NkComputeShader NkDX12ComputeContext::CreateShaderFromFile(
    ...
    std::ifstream f(path);
    if (!f.is_open()) { NK_DX12C_ERR("Cannot open: %s\n", path); return {}; }
    std::string src((std::istreambuf_iterator<char>(f)),
                     std::istreambuf_iterator<char>());
    return CreateShaderFromSource(src.c_str(), entry);
}
...
void NkDX12ComputeContext::BindPipeline(const NkComputePipeline& p) {
    if (!p.valid) return;
    // Stocké pour Dispatch
    (void)p;
}

void NkDX12ComputeContext::BindBuffer(uint32 /*slot*/, NkComputeBuffer& /*buf*/) {
    // En DX12, les UAV sont bindés via descriptor heap
    // Implémentation complète nécessite un SRV/UAV heap compute
}
