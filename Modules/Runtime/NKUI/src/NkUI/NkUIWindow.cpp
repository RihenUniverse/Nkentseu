...
namespace nkentseu
{
    ...
    {
        // ============================================================
        // Variables statiques de NkUIWindow
        // ============================================================
        ...
        // ============================================================
        // Helper : vérifie si une fenêtre peut capturer le pointeur
        // ============================================================

        static bool WindowCanCapturePointer(const NkUIContext& ctx,
                                            NkUIID            windowId) noexcept
        {
        ...
        // ============================================================
        // Helper : calcule la ligne de base Y pour le texte dans un rectangle
        // ============================================================
        ...
        static float32 BaselineY(const NkRect& r, const NkUIFont& font) noexcept
        {
            const float32 asc = (font.metrics.ascender > 0.f)
                                    ...
                                    : (font.size * 0.8f);
            const float32 descRaw = (font.metrics.descender >= 0.f)
                                        ? font.metrics.descender
                                        : -font.metrics.descender;
            const float32 desc = (descRaw > 0.f)
                                    ? descRaw
                                    : (font.size * 0.2f);
            const float32 lineH = (font.metrics.lineHeight > 0.f)
        ...
        // ============================================================
        // Helper : calcule quel bord de la fenêtre est sous le curseur
        // ============================================================
        ...
        {
            if (!canInteract) return 0;

            // Utiliser une marge intérieure de 8 pixels pour faciliter la prise
            const float32 margin = (borderSize < 8.f) ? 8.f : borderSize;

            const bool inY = (mp.y >= r.y - margin) && (mp.y <= r.y + r.h + margin);
            const bool inX = (mp.x >= r.x - margin) && (mp.x <= r.x + r.w + margin);
            uint8 edge = 0;

            if (inY && mp.x >= r.x - margin && mp.x <= r.x + margin) edge |= 4;      // left
            if (inY && mp.x >= r.x + r.w - margin && mp.x <= r.x + r.w + margin) edge |= 8; // right
            if (inX && mp.y >= r.y - margin && mp.y <= r.y + margin) edge |= 1;      // top
            if (inX && mp.y >= r.y + r.h - margin && mp.y <= r.y + r.h + margin) edge |= 2; // bottom

            return edge;
        }

        // ============================================================
        // Helper : retourne le curseur approprié pour un bord de redimensionnement
        // ============================================================
        ...
        void NkUIWindowManager::Destroy() noexcept
        {
            numWindows = 0;
        }
        ...
        void NkUIWindowManager::BringToFront(NkUIID id) noexcept
        {
            ...
            for (int32 i = 0; i < numWindows; ++i)
            {
                if (windows[i].zOrder > maxZ) maxZ = windows[i].zOrder;
            }
            ws->zOrder = maxZ + 1;
        }
        ...
        {
            // Tri à bulles sur zOrder[] (ordre croissant de zOrder)
        ...
        NkUIWindowState* NkUIWindowManager::HitTest(NkVec2   pos,
                                                    float32  titleBarHeight) noexcept
        {
            SortZOrder();
            // Test dans l'ordre inverse (du plus haut au plus bas)
            for (int32 i = numZOrder - 1; i >= 0; --i)
            {
                ...
                const float32 hitH = ws->isCollapsed
                                        ? (HasFlag(ws->flags, NkUIWindowFlags::NK_NO_TITLE_BAR)
                                                ? ws->size.y
                                                : titleBarHeight)
                                        : ws->size.y;
                const NkRect r = { ws->pos.x, ws->pos.y, ws->size.x, hitH };
                if (NkRectContains(r, pos))
                {
                    return ws;
                }
            }
        ...
        void NkUIWindowManager::BeginFrame(NkUIContext& ctx) noexcept
        {
            ...
            // Gestion du déplacement en cours
            ...
            // Gestion du redimensionnement en cours
            if (resizingId != NKUI_ID_NONE)
            {
                ...
                if (ws && !ws->isDocked && !HasFlag(ws->flags, NkUIWindowFlags::NK_NO_RESIZE))
                {
                    ...
                    const float32 mx = ctx.input.mousePos.x;
                    const float32 my = ctx.input.mousePos.y;
                    ...
                    if (resizeEdge & 4u) // left
                    {
                        newX = resizeStartPos.x + dx;
                        newW = resizeStartSize.x - dx;
                    }
                    if (resizeEdge & 8u) // right
                    {
                        newW = resizeStartSize.x + dx;
                    }
                    if (resizeEdge & 1u) // top
                    {
                        newH = resizeStartSize.y - dy;
                        if (newH < minH) newH = minH;
                        if (newH > maxH) newH = maxH;
                        const float32 originalBottom = resizeStartPos.y + resizeStartSize.y;
                        newY = originalBottom - newH;
                    }
                    if (resizeEdge & 2u) // bottom
                    {
                        newH = resizeStartSize.y + dy;
                    }
            ...
            // Fenêtre actuellement survolée (celle du dessus sous le curseur)
        ...
        void NkUIWindowManager::EndFrame(NkUIContext& /*ctx*/) noexcept
        {
                                    ...
                                    bool                   focused) noexcept
        {
            ...
            for (int32 i = 2; i >= 0; --i)
            {
                ...
                dl.AddRectFilled({ r.x + 1.5f - expand,
                                   r.y + 1.5f - expand,
                                   r.w + expand * 2,
                                   r.h + expand * 2 },
                                 sc, 10.f);
                                      ...
                                      NkUIWindowState& ws) noexcept
        {
            ...
            {
                // Arrondi haut uniquement (bits 0+1 = TL+TR)
                dl.AddRectFilledCorners(titleR, titleBg, rx, 0x3);
            }
            ...
            if (ws.name[0])
            {
                ...
                font.RenderText(dl, { titleR.x + m.titleBarPadX, ty },
                                ws.name, c.titleBarText, textMaxW, true);
            }
            ...
            if (!HasFlag(ws.flags, NkUIWindowFlags::NK_NO_CLOSE))
            {
                ...
                // X
                const float32 cx = closeR.x + btnSz * 0.5f;
                const float32 cy = closeR.y + btnSz * 0.5f;
                const float32 d  = btnSz * 0.25f;
            ...
            // Bouton collapse (réduire/agrandir)
            if (!HasFlag(ws.flags, NkUIWindowFlags::NK_NO_COLLAPSE))
            {
                ...
                dl.AddArrow({ collapseR.x + btnSz * 0.5f, collapseR.y + btnSz * 0.5f },
                            7.f, ws.isCollapsed ? 0 : 1, c.titleBarText);
                if (hov && ctx.ConsumeMouseClick(0))
                {
            ...
            // Zone de déplacement (toute la barre sauf les boutons)
            NkRect dragR = titleR;
            float32 dragRight = titleR.x + titleR.w;
            if (hasClose)      dragRight = closeR.x;
            if (hasCollapse && collapseR.x < dragRight) dragRight = collapseR.x;
            if (dragRight > dragR.x) dragR.w = dragRight - dragR.x;
            else                     dragR.w = 0.f;

            // Double-clic pour collapse
            if (canInteract && NkRectContains(dragR, ctx.input.mousePos)
                && ctx.input.mouseDblClick[0]
                ...
                && !HasFlag(ws.flags, NkUIWindowFlags::NK_NO_MOVE))
            {
                ...
                if (edge == 0) // aucun bord n'est survolé
                {
                    if (ctx.ConsumeMouseClick(0))
                    {
                        ctx.wm->movingId = ws.id;
                        ctx.wm->moveOffset = { ctx.input.mousePos.x - ws.pos.x,
                                               ctx.input.mousePos.y - ws.pos.y };
        ...
        // ============================================================
        // Gestion du redimensionnement (déclenchement)
        // ============================================================
                                      ...
                                      NkUIWindowState& ws) noexcept
        {
            ...
            if (ctx.wm)
            {
                uint8 cursorEdge = (ctx.wm->resizingId == ws.id) ? ctx.wm->resizeEdge : edge;
                if (cursorEdge != 0u)
                {
        ...
        // ============================================================
        // Dessin de la poignée de redimensionnement (coin bas-droit)
        // ============================================================
                                           ...
                                           const NkUIWindowState& ws) noexcept
        {
            if (HasFlag(ws.flags, NkUIWindowFlags::NK_NO_RESIZE) || ws.isCollapsed) return;
            const NkRect r = { ws.pos.x, ws.pos.y, ws.size.x, ws.size.y };
            const float32 sz = 5.f; // taille de la poignée de coin
            dl.AddResizeGrip({ r.x + r.w - sz - 2, r.y + r.h - sz - 2 },
                             sz, ctx.theme.colors.separator);
        }
                               ...
                               NkUIWindowFlags     flags) noexcept
        {
            ...
            // Appliquer SetNextWindowPos même sur fenêtres existantes
            if (hadNextPos)  ws->pos  = defPos;
            if (hadNextSize) ws->size = defSz;

            // Synchroniser l'état ouvert
            ...
            const float32 titleH = HasFlag(ws->flags, NkUIWindowFlags::NK_NO_TITLE_BAR)
                                        ? 0.f
            ...
            if (!ws->isCollapsed)
            {
                ...
                // Bordure
                dl.AddRect(winR,
                           focused ? ctx.theme.colors.border : ctx.theme.colors.separator,
                           1.f, rx);
            }
            ...
            if (!HasFlag(ws->flags, NkUIWindowFlags::NK_NO_SCROLLBAR))
            {
                ...
                // Recalcul car une barre peut forcer l'autre
            ...
            const float32 padY = ctx.theme.metrics.windowPadY;
            const float32 contentW = (clientR.w - padX * 2.f) > 1.f ? (clientR.w - padX * 2.f) : 1.f;
            const NkVec2 contentStart = { clientR.x + padX, ws->pos.y + titleH + padY };
                             ...
                             NkUILayoutStack&    ls) noexcept
        {
            ...
            const float32 cursorContentH = ctx.cursor.y - cur.startCursor.y
                                           + ctx.theme.metrics.windowPadY
                                           + ws->scrollY;
            ws->contentSize.y = cursorContentH;
            ws->contentSize.x = ctx.cursor.x - cur.startCursor.x
                                + ctx.theme.metrics.windowPadX
                                + ws->scrollX;

            // Nettoyage de la pile de layout
            ...
            const float32 titleH = ws->isDocked
                                        ? 0.f
                                        : (HasFlag(ws->flags, NkUIWindowFlags::NK_NO_TITLE_BAR)
                                                ? 0.f
                                                : ctx.theme.metrics.titleBarHeight);
            ...
            const float32 viewW = (fullViewW - (showV ? sbW : 0.f)) > 1.f
                                    ? (fullViewW - (showV ? sbW : 0.f))
                                    : 1.f;
            const float32 viewH = (fullViewH - (showH ? sbW : 0.f)) > 1.f
                                    ? (fullViewH - (showH ? sbW : 0.f))
                                    : 1.f;
            ...
            if (showV)
            {
                const NkRect track = { ws->pos.x + fullViewW - sbW,
                                       ...
                                       sbW,
                                       viewH > 0.f ? viewH : 1.f };
                NkUILayout::DrawScrollbar(ctx, dl, true, track,
                                          ws->contentSize.y, viewH, ws->scrollY,
                                          NkHashInt(ws->id, 99));
            }
            ...
            if (showH)
            {
                const NkRect track = { ws->pos.x,
                                       ...
                                       viewW > 0.f ? viewW : 1.f,
                                       sbW };
                NkUILayout::DrawScrollbar(ctx, dl, false, track,
                                          ws->contentSize.x, viewW, ws->scrollX,
                                          NkHashInt(ws->id, 100));
            }
        ...
        // ============================================================
        // Helpers statiques (position, taille, focus, etc.)
        // ============================================================
        ...
        NkVec2 NkUIWindow::GetWindowPos(NkUIWindowManager& wm, const char* name) noexcept
        {
            ...
            return ws ? ws->pos : NkVec2{};
        }

        NkVec2 NkUIWindow::GetWindowSize(NkUIWindowManager& wm, const char* name) noexcept
        {
            NkUIWindowState* ws = wm.Find(name);
            return ws ? ws->size : NkVec2{};
        }

        bool NkUIWindow::IsWindowFocused(NkUIWindowManager& wm, const char* name) noexcept
        {
            NkUIWindowState* ws = wm.Find(name);
            return ws && ws->id == wm.activeId;
        }

        bool NkUIWindow::IsWindowHovered(NkUIContext&       ctx,
                                         NkUIWindowManager& wm,
                                         const char*        name) noexcept
        {
            NkUIWindowState* ws = wm.Find(name);
            if (!ws) return false;
            return NkRectContains({ ws->pos.x, ws->pos.y, ws->size.x, ws->size.y },
                                  ctx.input.mousePos);
        }

        void NkUIWindow::SetWindowPos(NkUIWindowManager& wm,
                                      const char*        name,
                                      NkVec2             pos) noexcept
        {
            NkUIWindowState* ws = wm.Find(name);
        ...
        void NkUIWindow::SetWindowSize(NkUIWindowManager& wm,
                                       const char*        name,
        ...
        void NkUIWindow::SetWindowCollapsed(NkUIWindowManager& wm,
                                            const char*        name,
                                            bool               collapsed) noexcept
        {
            NkUIWindowState* ws = wm.Find(name);
            if (ws) ws->isCollapsed = collapsed;
        }
                                   ...
                                   NkUIFont&           font) noexcept
        {
            ...
            for (int32 i = 0; i < wm.numZOrder; ++i)
            {
                ...
                {
                    dl.AddRectFilled({ 0, 0,
                                       static_cast<float32>(ctx.viewW),
