...
// =============================================================================
NkSwapchainCallbackHandle NkDX12Context::AddCleanUpCallback(NkSwapchainCleanFn fn) {
...
NkSwapchainCallbackHandle NkDX12Context::AddRecreateCallback(NkSwapchainRecreateFn fn) {
    if (!fn) return NK_INVALID_CALLBACK_HANDLE;
    uint32 h = mNextCbHandle++;
    NkDX12CbEntry e; e.handle = h; e.fn = fn;
...
void NkDX12Context::RemoveCleanUpCallback(NkSwapchainCallbackHandle h) {
    for (usize i = 0; i < mCleanCallbacks.Size(); ++i) {
...
void NkDX12Context::RemoveRecreateCallback(NkSwapchainCallbackHandle h) {
    for (usize i = 0; i < mRecreateCallbacks.Size(); ++i) {
