...
#define NK_ASSERT_TRUE(cond)  if (!(cond)) { printf("  FAIL: %s : " #cond "\n", _testName); _ok = false; }
#define NK_ASSERT_NEAR(a, b, eps) if (fabsf((a)-(b)) > (eps)) { printf("  FAIL: %s : " #a " ~= " #b " (got %f vs %f)\n", _testName, (float)(a), (float)(b)); _ok = false; }
...
static void TestAudioGenerator() {
    ...
    NK_TEST("GenerateTone - Sine 440Hz")
        ...
        NK_ASSERT_TRUE(sine.IsValid());
        NK_ASSERT_TRUE(sine.frameCount == (usize)(48000));
        NK_ASSERT_TRUE(sine.channels == 1);
        NK_ASSERT_NEAR(sine.GetDuration(), 1.0f, 0.01f);
    ...
    NK_TEST("ApplyEnvelope ADSR")
        ...
        NK_ASSERT_NEAR(tone.data[0], 0.0f, 0.01f);
...
static void TestAudioLoader() {
    ...
    NK_TEST("DetectFormat - WAV magic bytes")
        ...
        NK_ASSERT_TRUE(fmt == AudioFormat::WAV);
    NK_END_TEST()

    NK_TEST("DetectFormat - MP3 ID3")
        uint8 mp3[3] = { 'I','D','3' };
        AudioFormat fmt = AudioLoader::DetectFormat(mp3, 3);
        NK_ASSERT_TRUE(fmt == AudioFormat::MP3);
    NK_END_TEST()

    NK_TEST("DetectFormat - OGG")
        uint8 ogg[4] = { 'O','g','g','S' };
        AudioFormat fmt = AudioLoader::DetectFormat(ogg, 4);
        NK_ASSERT_TRUE(fmt == AudioFormat::OGG);
    NK_END_TEST()

    NK_TEST("DetectFormatFromPath - .wav")
        NK_ASSERT_TRUE(AudioLoader::DetectFormatFromPath("test.wav")  == AudioFormat::WAV);
        NK_ASSERT_TRUE(AudioLoader::DetectFormatFromPath("test.mp3")  == AudioFormat::MP3);
        NK_ASSERT_TRUE(AudioLoader::DetectFormatFromPath("test.ogg")  == AudioFormat::OGG);
        NK_ASSERT_TRUE(AudioLoader::DetectFormatFromPath("test.flac") == AudioFormat::FLAC);
    NK_END_TEST()

    NK_TEST("SaveWAV + LoadWAV round-trip")
        // Générer un sample
        AudioSample original = AudioGenerator::GenerateTone(440.0f, 0.1f);
        NK_ASSERT_TRUE(original.IsValid());

        // Sauvegarder
        bool saved = AudioLoader::SaveWAV("/tmp/nk_test_audio.wav", original);
        NK_ASSERT_TRUE(saved);

        // Recharger
        AudioSample loaded = AudioLoader::Load("/tmp/nk_test_audio.wav");
        NK_ASSERT_TRUE(loaded.IsValid());
        NK_ASSERT_TRUE(loaded.channels   == 1);
        NK_ASSERT_NEAR((float32)loaded.sampleRate, (float32)original.sampleRate, 1.0f);
        // La durée doit être identique (tolérance ±1 frame)
        NK_ASSERT_NEAR(loaded.GetDuration(), original.GetDuration(), 0.01f);
    ...
    NK_TEST("ConvertChannels mono->stereo")
        ...
        NK_ASSERT_TRUE(mono.channels == 1);
        AudioLoader::ConvertChannels(mono, 2);
        NK_ASSERT_TRUE(mono.channels == 2);
        NK_ASSERT_TRUE(mono.IsValid());
    ...
    NK_TEST("Resample 48000->44100")
        ...
        NK_ASSERT_TRUE(s.sampleRate == 48000);
        AudioLoader::Resample(s, 44100, false);
        NK_ASSERT_TRUE(s.sampleRate == 44100);
        NK_ASSERT_NEAR(s.GetDuration(), 1.0f, 0.05f);
        AudioLoader::Free(s);
    NK_END_TEST()
}
...
static void TestAudioAnalyzer() {
    ...
    NK_TEST("CalculateRms - silence")
        ...
        NK_ASSERT_NEAR(AudioAnalyzer::CalculateRms(silent), 0.0f, 0.001f);
    ...
    NK_TEST("CalculateRms - sine wave")
        ...
        NK_ASSERT_NEAR(rms, 0.707f, 0.02f);
        AudioLoader::Free(sine);
    NK_END_TEST()
    ...
    NK_TEST("CalculatePeak - sine amplitude 0.8")
        ...
        NK_ASSERT_NEAR(peak, 0.8f, 0.02f);
        AudioLoader::Free(sine);
    NK_END_TEST()
    ...
    NK_TEST("ComputeFft - taille correcte")
        ...
        NK_ASSERT_TRUE(fft.fftSize == 2048);
        NK_ASSERT_TRUE((int32)fft.magnitudes.Size() == 1025); // fftSize/2 + 1
        NK_ASSERT_TRUE((int32)fft.phases.Size()     == 1025);
        AudioLoader::Free(sine);
    NK_END_TEST()
    ...
    NK_TEST("ComputeFft - pic à 440Hz")
        ...
        NK_ASSERT_NEAR(detectedFreq, 440.0f, 22.0f);
        AudioLoader::Free(sine);
    NK_END_TEST()
...
static void TestAudioMixer() {
    ...
    NK_TEST("Normalize")
        ...
        NK_ASSERT_NEAR(peak, 0.9f, 0.02f);
        AudioLoader::Free(s);
    NK_END_TEST()
    ...
    NK_TEST("FadeIn/FadeOut")
        ...
        NK_ASSERT_NEAR(s.data[0], 0.0f, 0.01f);
        mixer.FadeOut(s, 0.1f);
        // Dernier sample proche de 0
        NK_ASSERT_NEAR(s.data[s.frameCount - 1], 0.0f, 0.01f);
        (void)firstSample;
        AudioLoader::Free(s);
    NK_END_TEST()
    ...
    NK_TEST("ApplyGain x2")
        ...
        NK_ASSERT_NEAR(peakAfter, peakBefore * 2.0f, 0.02f);
        AudioLoader::Free(s);
    NK_END_TEST()
    ...
    NK_TEST("Reverse")
        ...
        mixer.Reverse(s);
        NK_ASSERT_NEAR(s.data[0],               last,  0.001f);
        NK_ASSERT_NEAR(s.data[s.frameCount - 1], first, 0.001f);
        AudioLoader::Free(s);
    NK_END_TEST()
    ...
    NK_TEST("Mix 2 sources")
        ...
        NK_ASSERT_TRUE(a.IsValid());
        NK_ASSERT_TRUE(b.IsValid());
        AudioLoader::Free(a);
        AudioLoader::Free(b);
    NK_END_TEST()
    ...
    NK_TEST("Concatenate 2 samples")
        ...
        NK_ASSERT_NEAR(durA + durB, 1.0f, 0.05f);
        AudioLoader::Free(a);
        AudioLoader::Free(b);
    NK_END_TEST()
}
...
static void TestEffects() {
    ...
    NK_TEST("BiquadFilter LowPass 1kHz")
        ...
        NK_ASSERT_TRUE(rmsAfter < 0.5f); // Atténuation visible
    NK_END_TEST()
    ...
    NK_TEST("Effect enabled/disabled")
        ...
        NK_ASSERT_NEAR(buf[64], expected, 0.001f);
    NK_END_TEST()
}
...
static void TestAudioEngine() {
    ...
    NK_TEST("Initialize avec NullBackend")
        ...
        NK_ASSERT_TRUE(ok);
        NK_ASSERT_TRUE(AudioEngine::Instance().IsInitialized());
    NK_END_TEST()

    NK_TEST("Play + Stop")
        AudioSample sine = AudioGenerator::GenerateTone(440.0f, 5.0f);
        AudioHandle h = AudioEngine::Instance().Play(sine);
        NK_ASSERT_TRUE(h.IsValid());
        NK_ASSERT_TRUE(AudioEngine::Instance().IsPlaying(h));
    ...
    NK_TEST("Pause/Resume")
        ...
        NK_ASSERT_TRUE(AudioEngine::Instance().IsPaused(h));
    ...
    NK_TEST("SetVolume / GetVolume")
        ...
        NK_ASSERT_NEAR(AudioEngine::Instance().GetVolume(h), 0.5f, 0.001f);
        AudioEngine::Instance().Stop(h);
        AudioLoader::Free(sine);
    NK_END_TEST()
    ...
    NK_TEST("SetPitch / GetPitch")
        ...
        NK_ASSERT_NEAR(AudioEngine::Instance().GetPitch(h), 1.5f, 0.001f);
        AudioEngine::Instance().Stop(h);
        AudioLoader::Free(sine);
    NK_END_TEST()
    ...
    NK_TEST("MasterVolume")
        ...
        NK_ASSERT_NEAR(AudioEngine::Instance().GetMasterVolume(), 0.8f, 0.001f);
