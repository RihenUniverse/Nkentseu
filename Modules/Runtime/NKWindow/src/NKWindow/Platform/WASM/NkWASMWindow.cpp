// =============================================================================
// NkWASMWindow.cpp - NkWindow implementation for WASM/Emscripten
// =============================================================================
...
namespace nkentseu {
    ...
    static std::vector<NkWindow*> sWasmWindows;
    static std::unordered_map<NkWindowId, NkWindow*> sWasmWindowById;
    static NkWindow* sWasmLastWindow = nullptr;
    static NkWindowId sWasmActiveWindowId = NK_INVALID_WINDOW_ID;

    static const char* NormalizeCanvasSelector(const std::string& canvasId) {
        return canvasId.empty() ? "#canvas" : canvasId.c_str();
    }
    ...
    static void SetDocumentTitle(const std::string& title) {
    ...
    NkWindow* NkWASMFindWindowById(NkWindowId id) {
        ...
        auto it = sWasmWindowById.find(id);
        return (it != sWasmWindowById.end()) ? it->second : nullptr;
    }
    ...
    void NkWASMRegisterWindow(NkWindow* window) {
        ...
        if (std::find(sWasmWindows.begin(), sWasmWindows.end(), window) == sWasmWindows.end()) {
            sWasmWindows.push_back(window);
        }

        sWasmWindowById[id] = window;
    ...
    void NkWASMUnregisterWindow(NkWindow* window) {
        ...
        sWasmWindows.erase(
            std::remove(sWasmWindows.begin(), sWasmWindows.end(), window),
            sWasmWindows.end());

        NkWindowId staleId = NK_INVALID_WINDOW_ID;
        for (const auto& kv : sWasmWindowById) {
            if (kv.second == window) {
                staleId = kv.first;
                break;
            }
        ...
        if (sWasmLastWindow == window) {
            sWasmLastWindow = sWasmWindows.empty() ? nullptr : sWasmWindows.back();
        }
    ...
    void NkWASMSetActiveWindowId(NkWindowId id) {
        std::lock_guard<std::mutex> lock(sWasmWindowsMutex);
        if (id != NK_INVALID_WINDOW_ID && sWasmWindowById.find(id) == sWasmWindowById.end()) {
            return;
        }
        sWasmActiveWindowId = id;
    }
    ...
    bool NkWindow::Create(const NkWindowConfig& config) {
        ...
        NkWASMRegisterWindow(this);
        NkWASMSetActiveWindowId(mId);

        mData.mDropTarget = new NkWASMDropTarget(mId, mData.mCanvasId);
    ...
    NkSurfaceDesc NkWindow::GetSurfaceDesc() const {
        ...
        desc.canvasId = mData.mCanvasId.c_str();
        return desc;
    }
