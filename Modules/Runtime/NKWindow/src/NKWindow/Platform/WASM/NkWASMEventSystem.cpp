...
#include "NKWindow/Platform/WASM/NkWASMDropTarget.h"
...
namespace nkentseu {
    ...
    static std::string gCanvasSelector = "#canvas";

    static const char* CurrentCanvasSelector() {
        return gCanvasSelector.empty() ? "#canvas" : gCanvasSelector.c_str();
    }

    template <typename Fn>
    static void ForEachWASMWindow(Fn&& fn) {
        const std::vector<NkWindow*> windows = NkWASMGetWindowsSnapshot();
    ...
    static NkWindowId ResolveActiveWindowId() {
        NkWindowId active = NkWASMGetActiveWindowId();
        if (active != NK_INVALID_WINDOW_ID && NkWASMFindWindowById(active)) {
            return active;
        }

        if (gFocusedWindowId != NK_INVALID_WINDOW_ID && NkWASMFindWindowById(gFocusedWindowId)) {
            return gFocusedWindowId;
        }

        if (NkWindow* last = NkWASMGetLastWindow()) {
    ...
    static void SyncActiveWindowContext() {
        ...
        NkWASMSetActiveWindowId(active);

        NkWindow* window = NkWASMFindWindowById(active);
        if (!window) {
            gWebInputOpts = {};
    ...
    static void InstallDropBridge(const char* canvasSelector) {
        ...
        EM_ASM({
            ...
            target.__nkDragEnterHandler = function(ev) {
                ...
                if (typeof Module !== 'undefined' && Module['_NkWASMOnDragEnter']) {
                    Module._NkWASMOnDragEnter(ev.clientX, ev.clientY, count, hasText, hasImage);
                }
            };
            ...
            target.__nkDragOverHandler = function(ev) {
                ev.preventDefault();
                ev.stopPropagation();
                if (typeof Module !== 'undefined' && Module['_NkWASMOnDragOver']) {
                    Module._NkWASMOnDragOver(ev.clientX, ev.clientY);
                }
            };
            ...
            target.__nkDragLeaveHandler = function(ev) {
                ev.preventDefault();
                ev.stopPropagation();
                if (typeof Module !== 'undefined' && Module['_NkWASMOnDragLeave']) {
                    Module._NkWASMOnDragLeave();
                }
            };
            ...
            target.__nkDropHandler = function(ev) {
                ...
                if (hasCcall && ev.dataTransfer) {
                    ...
                    if (text.length > 0) {
                        Module.ccall("NkWASMOnDropText", null, ["string", "number", "number"], [text, ev.clientX, ev.clientY]);
                    }
                    ...
                    if (names.length > 0) {
                        Module.ccall("NkWASMOnDropFiles", null, ["string", "number", "number"], [names.join("\n"), ev.clientX, ev.clientY]);
                    }
                }
            };
    ...
    static EM_BOOL OnKeyDown(int, const EmscriptenKeyboardEvent* ke, void*) {
    ...
    static EM_BOOL OnKeyUp(int, const EmscriptenKeyboardEvent* ke, void*) {
        if (!gWASMEventSystem || !ke) {
            return EM_FALSE;
        }
    ...
    static EM_BOOL OnMouseMove(int, const EmscriptenMouseEvent* me, void*) {
    ...
    static EM_BOOL OnMouseDown(int, const EmscriptenMouseEvent* me, void*) {
        if (!gWASMEventSystem || !me) {
            return EM_TRUE;
        }
    ...
    static EM_BOOL OnMouseUp(int, const EmscriptenMouseEvent* me, void*) {
        if (!gWASMEventSystem || !me) {
            return EM_TRUE;
        }
    ...
    static EM_BOOL OnWheel(int, const EmscriptenWheelEvent* we, void*) {
    ...
    static void PushTouchEvents(const EmscriptenTouchEvent* te, NkTouchPhase phase) {
    ...
    static EM_BOOL OnTouchStart(int, const EmscriptenTouchEvent* te, void*) {
        PushTouchEvents(te, NkTouchPhase::NK_TOUCH_PHASE_BEGAN);
        return gWebInputOpts.captureTouch ? EM_TRUE : EM_FALSE;
    }

    static EM_BOOL OnTouchMove(int, const EmscriptenTouchEvent* te, void*) {
        PushTouchEvents(te, NkTouchPhase::NK_TOUCH_PHASE_MOVED);
        return gWebInputOpts.captureTouch ? EM_TRUE : EM_FALSE;
    }

    static EM_BOOL OnTouchEnd(int, const EmscriptenTouchEvent* te, void*) {
        PushTouchEvents(te, NkTouchPhase::NK_TOUCH_PHASE_ENDED);
        return gWebInputOpts.captureTouch ? EM_TRUE : EM_FALSE;
    }

    static EM_BOOL OnTouchCancel(int, const EmscriptenTouchEvent* te, void*) {
    ...
    static EM_BOOL OnCanvasResize(int, const EmscriptenUiEvent*, void*) {
        ...
        ForEachWASMWindow([&](NkWindow& window, NkWindowId id) {
            const char* selector = window.mData.mCanvasId.empty() ? "#canvas" : window.mData.mCanvasId.c_str();
    ...
    static std::vector<std::string> ParseDroppedNames(const char* newlineSeparated) {
        std::vector<std::string> names;
        if (!newlineSeparated || !*newlineSeparated) {
            return names;
        }

        std::stringstream stream(newlineSeparated);
        std::string item;
...
EMSCRIPTEN_KEEPALIVE void NkWASMOnDragEnter(int x, int y, int numItems, int hasText, int hasImage) {
    ...
    nkentseu::NkWASMDropTarget::DispatchDragEnter(
        target,
        static_cast<float>(x),
        static_cast<float>(y),
...
EMSCRIPTEN_KEEPALIVE void NkWASMOnDragOver(int x, int y) {
    if (!nkentseu::gWASMEventSystem) {
        return;
    }
    ...
    nkentseu::NkWASMDropTarget::DispatchDragOver(target, static_cast<float>(x), static_cast<float>(y));
}
...
EMSCRIPTEN_KEEPALIVE void NkWASMOnDragLeave() {
    ...
    nkentseu::NkWASMDropTarget::DispatchDragLeave(target);
}

EMSCRIPTEN_KEEPALIVE void NkWASMOnDropText(const char* text, int x, int y) {
    if (!nkentseu::gWASMEventSystem) {
        return;
    }
    ...
    nkentseu::NkWASMDropTarget::DispatchDropText(
        target,
        static_cast<float>(x),
        static_cast<float>(y),
...
EMSCRIPTEN_KEEPALIVE void NkWASMOnDropFiles(const char* names, int x, int y) {
    if (!nkentseu::gWASMEventSystem) {
        return;
    }
    ...
    nkentseu::NkWASMDropTarget::DispatchDropFiles(
        target,
        static_cast<float>(x),
        static_cast<float>(y),
