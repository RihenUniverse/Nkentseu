...
namespace nkentseu {
    ...
    public:
        ...
        float64           GetOffsetX()       const noexcept { return mDeltaX;       }
        float64           GetOffsetY()       const noexcept { return mDeltaY;       }
    ...
    // =========================================================================
    // NkMouseScrollEvent — scroll unifié pour trackpad / molette smooth-scroll
    // Porte à la fois le delta X et Y avec position du curseur.
    // Différent de NkMouseWheelVertical/Horizontal : peut avoir les deux axes.
    // =========================================================================

    class NkMouseScrollEvent final : public NkMouseEvent {
    public:
        NK_EVENT_TYPE_FLAGS(NK_MOUSE_SCROLL)

        /// @param deltaX      Delta horizontal (positif = droite)
        /// @param deltaY      Delta vertical   (positif = haut)
        /// @param x / y       Position du curseur
        /// @param pixelDeltaX Delta en pixels X (trackpad haute précision)
        /// @param pixelDeltaY Delta en pixels Y (trackpad haute précision)
        NkMouseScrollEvent(float64 deltaX, float64 deltaY,
                           int32 x = 0, int32 y = 0,
            ...
            : NkMouseEvent(windowId)
            , mDeltaX(deltaX), mDeltaY(deltaY)
            , mPixelDeltaX(pixelDeltaX), mPixelDeltaY(pixelDeltaY)
            , mX(x), mY(y)
            , mHighPrecision(highPrecision), mModifiers(mods)
        {}

        NkEvent*    Clone()    const override { return new NkMouseScrollEvent(*this); }
        NkString ToString() const override {
