...
static uint8* ReadSubBlocks(NkImageStream& s, usize& outLen) noexcept {
    ...
    uint8* buf = static_cast<uint8*>(::malloc(total));
                         ...
                         NkImage* canvas, const NkImage* prevCanvas) noexcept
{
    ...
    uint8* indexBuf = static_cast<uint8*>(::malloc(fw*fh));
    if(!indexBuf){ ::free(lzwBuf); return false; }
    ::memset(indexBuf, 0, fw*fh);
...
NkImage* NkGIFCodec::Decode(const uint8* data, usize size) noexcept {
    ...
    if(::memcmp(hdr,"GIF87a",6)!=0 && ::memcmp(hdr,"GIF89a",6)!=0) return nullptr;
    ...
    // Canvas RGBA32
    NkImage* canvas = NkImage::Alloc(lsWidth, lsHeight, NkPixelFormat::RGBA32);
    ...
    if(hasGCT && bgColor < gctSize) {
        ...
        ::memset(canvas->Pixels(),0,canvas->TotalBytes());
    }
...
bool NkGIFCodec::Encode(const NkImage& img, uint8*& out, usize& outSize) noexcept {
    ...
    const bool hasAlpha = (img.Format()==NkPixelFormat::RGBA32||img.Format()==NkPixelFormat::GrayA16);
    ...
    uint32* pixels = static_cast<uint32*>(::malloc(sizeof(uint32)*total));
    ...
    uint8* indexBuf = static_cast<uint8*>(::malloc(total));
