...
namespace nkentseu {
    ...
    static void UnfilterRow(uint8* row, const uint8* prev, int32 rowBytes, int32 bpp, uint8 filter) noexcept {
        switch(filter){
            case 0: break; // None
            case 1: // Sub
    ...
    static uint8 BestFilter(const uint8* row, const uint8* prev, int32 rowBytes, int32 bpp) noexcept {
    ...
    // Expanse un bit depth vers 8 bits (pour les canaux scalaires : grayscale, alpha)
    // Les valeurs sont normalisées vers [0-255].
    static void ExpandBits(const uint8* src, uint8* dst, int32 width, int32 bitDepth) noexcept {
        switch(bitDepth){
            case 1:
    ...
    // Décompacte les indices de palette (colorType==3) — sans normalisation.
    // Un index 4-bit 3 → dst[i]=3, pas 3*17=51.
    static void ExpandIndices(const uint8* src, uint8* dst, int32 width, int32 bitDepth) noexcept {
        switch(bitDepth){
            case 1:
                for(int32 i=0;i<width;++i)
            ...
            case 2:
                for(int32 i=0;i<width;++i)
                    dst[i]=(src[i/4]>>((3-(i%4))*2))&3;
                break;
            case 4:
                for(int32 i=0;i<width;++i)
                    dst[i]=(i&1)?(src[i/2]&0x0F):(src[i/2]>>4);
                break;
            case 8:
            default:
                NkCopy(dst,src,width);
    ...
    NkImage* NkPNGCodec::Decode(const uint8* data, usize size) noexcept {
        ...
        if(NkCompare(sig,PNG_SIG,8)!=0) return nullptr;
        ...
        uint8 palette[256*3]={};
        uint8 transIdx[256];NkSet(transIdx,255,256); // opaque par défaut
        int32 palSize=0;
        uint16 transR=0,transG=0,transB=0; // tRNS pour RGB/Gray
        bool hasTrans=false;

        // Collecte les données IDAT
        uint8* idatBuf=static_cast<uint8*>(NkAlloc(size)); if(!idatBuf) return nullptr;
        usize  idatLen=0;
        ...
        while(!s.IsEOF()&&!s.HasError()){
            ...
            if(chunkType==0x49484452u){ // IHDR
                ...
                bitDepth  = s.ReadU8();
                colorType = s.ReadU8();
                (void)s.ReadU8(); // compression
                (void)s.ReadU8(); // filter
                interlace = s.ReadU8();
                hasIHDR   = true;
            }
        ...
        if(!hasIHDR||width<=0||height<=0||idatLen==0){NkFree(idatBuf);return nullptr;}
        if(interlace!=0){NkFree(idatBuf);return nullptr;} // Adam7 non supporté
        ...
        uint8* raw=static_cast<uint8*>(NkAlloc(rawSize));
        if(!raw){NkFree(idatBuf);return nullptr;}

        usize written=0;
        if(!NkDeflate::Decompress(idatBuf,idatLen,raw,rawSize,written)){
            NkFree(idatBuf);NkFree(raw);return nullptr;
        }
        NkFree(idatBuf);
        ...
        const NkImagePixelFormat fmt =
            (outChannels==1)?NkImagePixelFormat::NK_GRAY8:
            (outChannels==2)?NkImagePixelFormat::NK_GRAY_A16:
            (outChannels==3)?NkImagePixelFormat::NK_RGB24:NkImagePixelFormat::NK_RGBA32;

        NkImage* img=NkImage::Alloc(width,height,fmt);
        if(!img){NkFree(raw);return nullptr;}

        // Buffer ligne précédente (pour le filtre)
        uint8* prevRow=static_cast<uint8*>(NkAllocZero(rowBytes+1,1));
        if(!prevRow){NkFree(raw);img->Free();return nullptr;}

        // Buffer temporaire pour l'expansion des bits
        uint8* tmpRow=static_cast<uint8*>(NkAlloc((rowBytes+4)*8));
        if(!tmpRow){NkFree(prevRow);NkFree(raw);img->Free();return nullptr;}
        ...
        for(int32 y=0;y<height;++y){
            ...
            if(colorType==3){ // indexé
                // Décompacte les indices de palette (1/2/4/8 bits) SANS normalisation
                ExpandIndices(filtRow,tmpRow,width,bitDepth);
                ...
                if(colorType==0){
                    ...
                    if(outChannels==3) NkCopy(dst,filtRow,width*3);
                    else {
                        for(int32 x=0;x<width;++x){
                    ...
                    NkCopy(dst,filtRow,width*outChannels);
                }
            }

            NkCopy(prevRow,filtRow,rowBytes);
        }

        NkFree(tmpRow); NkFree(prevRow); NkFree(raw);
        return img;
    }
    ...
    bool NkPNGCodec::Encode(const NkImage& img, uint8*& out, usize& outSize) noexcept {
        ...
        uint8* raw=static_cast<uint8*>(NkAlloc(rawSize));
        uint8* prevRow=static_cast<uint8*>(NkAllocZero(rowBytes,1));
        uint8* filtBuf=static_cast<uint8*>(NkAlloc(rowBytes));
        if(!raw||!prevRow||!filtBuf){NkFree(raw);NkFree(prevRow);NkFree(filtBuf);return false;}
        ...
        for(int32 y=0;y<h;++y){
            ...
            // Applique le filtre
            switch(ftype){
                case 0: NkCopy(raw+rawPos,row,rowBytes); break;
                case 1:
                    for(int32 i=0;i<rowBytes;++i)
            ...
            rawPos+=rowBytes;
            NkCopy(prevRow,row,rowBytes);
        }
        NkFree(filtBuf); NkFree(prevRow);
