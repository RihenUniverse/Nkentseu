...
NkMallocAllocator gMallocAllocator;
NkNewAllocator gNewAllocator;
NkVirtualAllocator gVirtualAllocator;
NkAllocator* gDefaultAllocator = &gMallocAllocator;
...
#endif
        gMallocAllocator.Deallocate(current);
        current = next;
    }
    mBlocks = nullptr;
}
...
void NkVirtualAllocator::TrackBlock(Pointer ptr, SizeType size) {
    auto* node = static_cast<NkVirtualBlock*>(
        gMallocAllocator.Allocate(sizeof(NkVirtualBlock), alignof(NkVirtualBlock)));
    if (!node) {
        return;
    }
    node->ptr = ptr;
...
nk_size NkVirtualAllocator::UntrackBlock(Pointer ptr) noexcept {
    ...
    while (cur) {
        if (cur->ptr == ptr) {
            ...
            gMallocAllocator.Deallocate(cur);
            return size;
        }
        prev = cur;
        cur = cur->next;
