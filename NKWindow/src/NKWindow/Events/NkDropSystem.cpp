...
// Win32 backend — OLE Drag & Drop avec fallback DragAcceptFiles
...
// Cocoa backend — NSView registerForDraggedTypes
...
// UIKit backend — UIDropInteraction
...
// Android backend — IntentFilter + ContentResolver
...
// XCB/XLib backend — XDND protocol (Freedesktop DND)
...
// WASM backend — HTML5 Drag & Drop API (ondrop, ondragover events)
...
// UWP backend — Windows.ApplicationModel.DataTransfer
class NkUWPDropTargetBackend : public IDropTargetBackend {
public:
    void Enable(void* nativeHandle) override {
        if (!nativeHandle) return;
        // CoreWindow or SwapChainPanel
        // Implementation: AllowDrop = true + DragOperationCompleted handler
        mDropTargets.insert(reinterpret_cast<std::uintptr_t>(nativeHandle));
    }
