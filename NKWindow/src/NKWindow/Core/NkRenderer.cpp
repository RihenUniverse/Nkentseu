...
std::unique_ptr<INkRendererImpl> CreateBuiltinRendererImpl(NkRendererApi api) {
	switch (api) {
		case NkRendererApi::NK_NONE:
			return nullptr;
		case NkRendererApi::NK_SOFTWARE:
			return std::make_unique<NkSoftwareRendererImpl>();
		case NkRendererApi::NK_VULKAN:
			return std::make_unique<NkVulkanRendererImpl>();
		case NkRendererApi::NK_OPENGL:
			return std::make_unique<NkOpenGLRendererImpl>();
		case NkRendererApi::NK_DIRECTX11:
			return std::make_unique<NkDX11RendererImpl>();
		case NkRendererApi::NK_DIRECTX12:
			return std::make_unique<NkDX12RendererImpl>();
		case NkRendererApi::NK_METAL:
			return std::make_unique<NkMetalRendererImpl>();
		default:
			return nullptr;
	}
}

std::unique_ptr<INkRendererImpl> CreateRendererImpl(NkRendererApi api) {
...
NkRenderer::NkRenderer(NkWindow &window, std::unique_ptr<INkRendererImpl> externalImpl, const NkRendererConfig &config) {
...
bool NkRenderer::Create(NkWindow &window, const NkRendererConfig &config) {
	return Create(window, std::unique_ptr<INkRendererImpl>{}, config);
}

bool NkRenderer::Create(NkWindow &window, std::unique_ptr<INkRendererImpl> externalImpl, const NkRendererConfig &config) {
	Shutdown();
...
bool NkRenderer::RegisterExternalRendererFactory(NkRendererApi api, NkRendererFactory factory) {
	if (api == NkRendererApi::NK_NONE || !factory)
		return false;
...
bool NkRenderer::HasExternalRendererFactory(NkRendererApi api) {
	...
	std::lock_guard<std::mutex> lock(ExternalRendererFactoryMutex());
	return ExternalRendererFactories().find(static_cast<NkU32>(api)) != ExternalRendererFactories().end();
}
...
NkRendererApi NkRenderer::GetApi() const {
	return mImpl ? mImpl->GetApi() : mConfig.api;
}
std::string NkRenderer::GetApiName() const {
	return mImpl ? mImpl->GetApiName() : NkRendererApiToString(mConfig.api);
}
...
void NkRenderer::BeginFrame(NkU32 clearColor) {
	...
	// Auto-resize: if the window changed size since last frame, resize the
	// framebuffer before clearing so the next Present() draws at the correct
	// resolution on all platforms (including Linux XLib).
...
bool NkRenderer::ResolveToExternalRenderTarget() {
	if (!mExternalTarget || !mImpl)
		return false;

	const NkFramebufferInfo &fb = mImpl->GetFramebufferInfo();
	if (!fb.pixels || !fb.width || !fb.height || !fb.pitch)
		return false;
...
NkU32 NkRenderer::PackColor(NkU8 r, NkU8 g, NkU8 b, NkU8 a) {
	return (static_cast<NkU32>(r) << 24) | (static_cast<NkU32>(g) << 16) | (static_cast<NkU32>(b) << 8) | a;
}
