...
namespace nkentseu {
	...
	class NkJiggleBoneSystem final : public NkSystem {
		...
		private:
			void UpdateJiggle(NkJiggleBone &jb, NkSkeleton &skeleton, const NkTransform &tf, float32 dt) noexcept;
	...
	class NkClothSystem final : public NkSystem {
		public:
			explicit NkClothSystem(NkIDevice *device = nullptr) noexcept : mDevice(device) {
			}
	...
	class NkHairSystem final : public NkSystem {
		public:
			explicit NkHairSystem(NkIDevice *device = nullptr) noexcept : mDevice(device) {
			}
	...
	class NkSoftBodySystem final : public NkSystem {
		public:
			explicit NkSoftBodySystem(NkIDevice *device = nullptr) noexcept : mDevice(device) {
			}
	...
	class NkMocapSystem final : public NkSystem {
		...
		private:
			void PlaybackMocap(NkMotionCapture &mc, NkSkeleton &sk, float32 dt) noexcept;
			NkMat4f InterpolateFrames(const NkMocapFrame &a, const NkMocapFrame &b, float32 t,
	...
	struct NkBlendShapeController {
			...
			void SetWeight(uint32 shapeIdx, float32 w) noexcept {
				for (uint32 i = 0; i < driverCount; ++i)
					if (drivers[i].shapeIdx == shapeIdx && drivers[i].type == DriverType::Manual) {
	...
	class NkBlendShapeSystem final : public NkSystem {
		...
		private:
			float32 ResolveDriver(const NkBlendShapeController::Driver &d, NkWorld &world, float32 dt) const noexcept;
	};
