...
namespace nkentseu {
	...
	struct NkClothSim {
			...
			bool AddBonePin(uint32 vertIdx, NkEntityId boneEnt, uint32 boneIdx, float32 weight = 1.f) noexcept {
				if (pinCount >= kMaxPins)
					return false;
	...
	struct NkRagdollComponent {
			...
			void BlendIn(float32 speed = 2.f) noexcept {
				state = State::Blending;
				blendSpeed = speed;
			}

			void BlendOut(float32 speed = 2.f) noexcept {
				blendSpeed = -speed;
			}
	};
	...
	struct NkMotionCapture {
			 ...
			 */
			void Retarget(const NkSkeleton &srcSkeleton, NkSkeleton &dstSkeleton) const noexcept;
	};
