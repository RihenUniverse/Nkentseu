...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    bool toggled = false;
    ImFolderTextureID::ImFolderTextureIDSource.OpenIt = false;
    if (!is_leaf)
    {
        if (pressed && g.DragDropHoldJustPressedId != id)
        {
            ...
            if ((flags & ImGuiTreeNodeFlags_OpenOnDoubleClick) && g.IO.MouseClickedCount[0] == 2) {
                toggled = true;
                ImFolderTextureID::ImFolderTextureIDSource.OpenIt = true;
            }
        }
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Folder) {
            if (is_open)
                printf("Open\n");
            RenderFolder(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col,
                         is_open ? ImGuiFolder_Open : ImGuiFolder_Close, 20.0f);
        } else if (flags & ImGuiTreeNodeFlags_FolderFile) {
            RenderFolder(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x - 18, text_pos.y), text_col, ImGuiFolder_File, 20.0f);
            text_pos.x -= text_offset_x / 2.0f;
